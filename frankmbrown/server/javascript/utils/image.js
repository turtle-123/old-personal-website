...
exports.handleImageUpload150_75 = exports.handleImageUpload100_100 = exports.getImageBackgroundColor = exports.isImageOpaque = exports.getOpaqueImageBackgroundColor = exports.getTransparentImageBackgroundColor = exports.checkPageAndTwitterImagesExist = exports.getInsertImageIntoImagesTable = exports.getImageFromUrl = exports.getImageInformation = exports.createTwitterAndLinkedInImages = exports.getTwitterImageUrlFromProfilePicture = exports.getPageImageUrlFromProfilePicture = exports.createDefaultProfilePicture = exports.createProfilePicture = exports.RESIZE_OPTIONS_BEFORE = exports.getMagick = void 0;
 ...
 */
async function createProfilePicture(url) {
    var base64;
    ...
    const firstMagick = await (0, cmd_1.executeWithArgument)('magick', [
        'convert', '-',
        '-gravity', 'center',
        '-crop', '200x200+0+0',
        '+repage',
    ...
    const secondMagick = await (0, cmd_1.executeWithArgument)('magick', [
        'convert', '-',
        '-alpha', 'set',
        '-background', 'none',
        '-fill', 'white',
 ...
 * @param username
 */
async function createDefaultProfilePicture(username) {
    var pathToOutput = '';
    try {
        const size = 200;
        ...
        const stdout = await (0, cmd_1.execute)(command, { cwd: __dirname });
        return { buffer: Buffer.from(stdout), background: color };
    }
    catch (e) {
        console.error(e);
        await (0, cmd_1.tryRemoveFile)(pathToOutput);
        throw new Error("Unable to create background image.");
    }
}
exports.createDefaultProfilePicture = createDefaultProfilePicture;
/**
 ...
 */
function getPageImageUrlFromProfilePicture(url) {
...
exports.getPageImageUrlFromProfilePicture = getPageImageUrlFromProfilePicture;
/**
 * Returns URL to page image (should be used as #META_OG_IMAGE[content])
 * @param url URL that's like https://image.storething.org/frankmbrown/[KEY]
 */
function getTwitterImageUrlFromProfilePicture(url) {
    const key = url.replace('https://image.storething.org/frankmbrown/', '');
    const name = key.split('.')[0];
    return 'https://image.storething.org/frankmbrown/twitter/'.concat(name).concat('.jpg');
}
exports.getTwitterImageUrlFromProfilePicture = getTwitterImageUrlFromProfilePicture;
/**
 ...
 * @param extension
 */
async function createTwitterAndLinkedInImages(image, background) {
    var background_new;
    if (!!!background) {
    ...
    const [twitter, linkedin] = await Promise.all([
        (0, cmd_1.executeWithArgument)(`magick`, ['convert', '-', '-thumbnail', '800x418', ...exports.RESIZE_OPTIONS_BEFORE, '-background', background_new, '-gravity', 'center', '-extent', '800x418', 'jpeg:-'], image),
        (0, cmd_1.executeWithArgument)(`magick`, ['convert', '-', '-thumbnail', '1200x627', ...exports.RESIZE_OPTIONS_BEFORE, '-background', background_new, '-gravity', 'center', '-extent', '1200x627', 'jpeg:-'], image)
    ]);
    return { twitter, linkedin };
}
exports.createTwitterAndLinkedInImages = createTwitterAndLinkedInImages;
/**
 * Parse output of magick identify
 * @param output
 * @returns
 */
function parseIdentifyOutput(output) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        else if (line.startsWith('Artifacts:') || line.startsWith('verbose:'))
            continue;
...
async function getImageInformation(image) {
    const magickProcess = (0, node_child_process_1.spawn)('magick', ['identify', '-verbose', '-']);
    let stdout = '';
    let stderr = '';
...
exports.getImageInformation = getImageInformation;
/**
 * Get an image from a URL
 * @param url
 */
async function getImageFromUrl(url) {
    const res = await axios_1.default.get(url, { responseType: "text", responseEncoding: "base64", timeout: 5000 });
    const base64 = Buffer.from(res.data, "base64");
    return base64;
}
exports.getImageFromUrl = getImageFromUrl;
function getInsertImageIntoImagesTable(url, db, user_id, imageInfo) {
    const arr = [
        url,
        user_id,
...
exports.getInsertImageIntoImagesTable = getInsertImageIntoImagesTable;
async function checkPageAndTwitterImagesExist(imageURL, image, background) {
    ...
    if (!!!twitterResp || !!!linkedinResp) {
        ...
        await Promise.all([
            (0, aws_implementation_1.uploadImageObject)(twitterKey, twitter, extension, {}, 3, 25),
            (0, aws_implementation_1.uploadImageObject)(linkedinKey, linkedin, extension, {}, 3, 25)
        ]);
    }
    return;
}
exports.checkPageAndTwitterImagesExist = checkPageAndTwitterImagesExist;
async function getTransparentImageBackgroundColor(buffer) {
    const res = await (0, cmd_1.executeWithArgument)("magick", ['convert', '-', '-colorspace', 'Gray', '-resize', '1x1\!', '-format', '', '-format', '"%[pixel:p{0,0}]"', 'info:'], buffer);
...
exports.getTransparentImageBackgroundColor = getTransparentImageBackgroundColor;
async function getOpaqueImageBackgroundColor(buffer) {
    const [pixels, output] = await Promise.all([
        (0, cmd_1.executeWithArgument)('magick', ['identify', '-format', '%[hex:u.p{0,0}],%[hex:u.p{w-1,0}],%[hex:u.p{0,h-1}],%[hex:u.p{w-1,h-1}]', '-'], buffer),
        (0, cmd_1.executeWithArgument)('magick', ['identify', '-verbose', '-'], buffer)
    ]);
...
exports.getOpaqueImageBackgroundColor = getOpaqueImageBackgroundColor;
async function isImageOpaque(buffer) {
    const isOpaqueResult = await (0, cmd_1.executeWithArgument)("magick", ['identify', '-format', '%[opaque]', '-'], buffer);
    const isOpaque = isOpaqueResult.toString('utf8').trim() === "True";
    return isOpaque;
}
exports.isImageOpaque = isImageOpaque;
/**
 * Determine the background color of an image
 * @param buffer
 * @returns
 */
async function getImageBackgroundColor(buffer) {
...
exports.getImageBackgroundColor = getImageBackgroundColor;
async function handleImageUpload100_100(imageURL) {
    ...
    const [widthHeightBUff, backgroundColor] = await Promise.all([
        (0, cmd_1.executeWithArgument)('magick', ['identify', '-format', '%wx%h', '-'], origImage),
    ...
    if (widthHeightArr.length !== 2)
        throw new Error("Something went wrong getting width and height of image.");
    const width = parseInt(widthHeightArr[0]);
    const height = parseInt(widthHeightArr[1]);
    var newImage;
    if (width !== 100 || height !== 100) {
        newImage = await (0, cmd_1.executeWithArgument)('magick', ['convert', '-', '-thumbnail', '100x100', ...exports.RESIZE_OPTIONS_BEFORE, '-gravity', 'center', '-background', backgroundColor, '-extent', '100x100', 'jpeg:-'], origImage);
        const key = imageURL.replace('https://image.storething.org/', '');
        const extension = String(key.split('.').at(-1));
        await (0, aws_implementation_1.uploadImageObject)(newKey, newImage, extension, { width: "100", height: "100" }, 3, 25);
...
exports.handleImageUpload100_100 = handleImageUpload100_100;
async function handleImageUpload150_75(imageURL) {
    const origImage = await getImageFromUrl(imageURL);
    const newKey = 'frankmbrown/' + crypto.randomUUID().concat('.jpg');
    const newURL = `https://image.storething.org/${newKey}`;
    const [widthHeightBUff, backgroundColor] = await Promise.all([
    ...
    if (width !== 150 || height !== 75) {
        newImage = await (0, cmd_1.executeWithArgument)('magick', ['convert', '-', '-gravity', 'center', '-thumbnail', '150x75', ...exports.RESIZE_OPTIONS_BEFORE, '-background', backgroundColor, '-extent', '150x75', 'jpeg:-'], origImage);
        const key = imageURL.replace('https://image.storething.org/', '');
        const extension = String(key.split('.').at(-1));
        await (0, aws_implementation_1.uploadImageObject)(newKey, newImage, extension, { width: "150", height: "75" }, 3, 25);
        await checkPageAndTwitterImagesExist(newURL, origImage, backgroundColor);
        return newURL;
    }
    else {
...
exports.handleImageUpload150_75 = handleImageUpload150_75;
