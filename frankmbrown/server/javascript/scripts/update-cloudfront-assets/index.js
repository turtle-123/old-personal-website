...
const desktop_bundle_path_client = node_path_1.default.resolve(__dirname, '..', '..', '..', '..', 'static', 'css', 'desktop-bundle.css');
const mobile_bundle_path_server = "frankmbrown/css/mobile-bundle.css";
const mobile_bundle_path_client = node_path_1.default.resolve(__dirname, '..', '..', '..', '..', 'static', 'css', 'mobile-bundle.css');
...
const invalidateCSS = new client_cloudfront_1.CreateInvalidationCommand({
 ...
 */
async function uploadJSFiles(arr) {
    const cloudfrontInvalidationKeys = [];
    const uploadObjects = [];
    for (let i = 0; i < arr.length; i++) {
        ...
        uploadObjects.push(new client_s3_1.PutObjectCommand({
            Bucket: AWS_CDN_BUCKET,
            Body: file,
            Key: key,
    ...
    await Promise.all([
        ...
        cloudfrontClient.send(new client_cloudfront_1.CreateInvalidationCommand({
            DistributionId: AWS_CLOUDFRONT_CDN_ID, // required
            InvalidationBatch: {
                Paths: {
