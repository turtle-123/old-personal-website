...
exports.registerCustomCodeNode = exports.createSvg = exports.createSelectButton = exports.INSERT_CODE_COMMAND = exports.CustomCodeNode = exports.$isCustomCodeNode = exports.$createCustomCodeNode = void 0;
/**
...
exports.createSelectButton = createSelectButton;
...
class CustomCodeNode extends lexicalLexical_js_1.DecoratorNode {
    static getType() {
        return 'custom-code';
    }
    static clone(node) {
    ...
    constructor({ code, autodetect, language }, key) {
        super(key);
    ...
    decorate(editor, config) {
        ...
        if (element) {
            ...
            if (this.getEditable()) {
                ...
                if (html)
                    html.dispatchEvent(new CustomEvent("NEW_CONTENT_LOADED"));
                return pre;
            }
            else {
                const div = document.createElement('div');
...
exports.CustomCodeNode = CustomCodeNode;
...
function $createCustomCodeNode(code = '', autodetect = true, language) {
    ...
    return (0, lexicalLexical_js_1.$applyNodeReplacement)(customCodeNode);
}
exports.$createCustomCodeNode = $createCustomCodeNode;
function $isCustomCodeNode(node) {
    return node instanceof CustomCodeNode;
}
exports.$isCustomCodeNode = $isCustomCodeNode;
const INSERT_CODE_COMMAND = (0, lexicalLexical_js_1.createCommand)('INSERT_CODE_COMMAND');
exports.INSERT_CODE_COMMAND = INSERT_CODE_COMMAND;
/**
 *
 * @param editor
 * @returns
...
function registerCustomCodeNode(editor) {
    return (0, lexical_utilsLexicalUtils_js_1.mergeRegister)(editor.registerCommand(lexicalLexical_js_1.SELECTION_CHANGE_COMMAND, () => {
        ...
        if ((0, lexicalLexical_js_1.$isNodeSelection)(selection)) {
            ...
            if ($isCustomCodeNode(nodes[0])) {
                ...
                Object.keys(decorators).forEach(key2 => {
                    if (key2 !== key && $isCustomCodeNode((0, lexicalLexical_js_1.$getNodeByKey)(key2)))
            ...
            for (let key of Object.keys(decorators)) {
                ...
                if ($isCustomCodeNode(node) && node.getEditable())
                    node.setEditable(false);
            }
        }
        return false;
    }, lexicalLexical_js_1.COMMAND_PRIORITY_HIGH), editor.registerCommand(lexicalLexical_js_1.KEY_ESCAPE_COMMAND, payload => {
        const decorators = editor.getDecorators();
        for (let key of Object.keys(decorators)) {
            const node = (0, lexicalLexical_js_1.$getNodeByKey)(key);
            if ($isCustomCodeNode(node) && node.getEditable())
        ...
        return false;
    }, lexicalLexical_js_1.COMMAND_PRIORITY_HIGH), editor.registerCommand(INSERT_CODE_COMMAND, () => {
        const codeNode = $createCustomCodeNode('', true, undefined);
        (0, lexicalLexical_js_1.$insertNodes)([codeNode]);
        if ((0, lexicalLexical_js_1.$isRootOrShadowRoot)(codeNode.getParentOrThrow())) {
            (0, lexical_utilsLexicalUtils_js_1.$wrapNodeInElement)(codeNode, lexicalLexical_js_1.$createParagraphNode).selectEnd();
        }
        codeNode.setEditable(true);
        return true;
    }, lexicalLexical_js_1.COMMAND_PRIORITY_EDITOR));
}
exports.registerCustomCodeNode = registerCustomCodeNode;
