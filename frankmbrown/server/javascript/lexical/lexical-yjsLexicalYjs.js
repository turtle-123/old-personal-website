...
exports.syncYjsChangesToLexical = exports.syncLexicalUpdateToYjs = exports.syncCursorPositions = exports.setLocalStateFocus = exports.initLocalState = exports.createUndoManager = exports.createBinding = exports.TOGGLE_CONNECT_COMMAND = exports.CONNECTED_COMMAND = void 0;
/**
...
const lexical_offsetLexicalOffset_js_1 = require("./lexical-offsetLexicalOffset.js");
/**
 * Copyright (c) Meta Platforms, Inc. and affiliates.
 *
 * This source code is licensed under the MIT license found in the
...
class CollabLineBreakNode {
    ...
    getNode() {
        const node = (0, lexicalLexical_js_1.$getNodeByKey)(this._key);
        return (0, lexicalLexical_js_1.$isLineBreakNode)(node) ? node : null;
    }
    getKey() {
        return this._key;
    }
...
class CollabTextNode {
    ...
    getPrevNode(nodeMap) {
        ...
        return (0, lexicalLexical_js_1.$isTextNode)(node) ? node : null;
    }
    getNode() {
        const node = (0, lexicalLexical_js_1.$getNodeByKey)(this._key);
        return (0, lexicalLexical_js_1.$isTextNode)(node) ? node : null;
...
function isExcludedProperty(name, node, binding) {
    ...
    else if ((0, lexicalLexical_js_1.$isElementNode)(node)) {
        if (elementExcludedProperties.has(name) || (0, lexicalLexical_js_1.$isRootNode)(node) && rootExcludedProperties.has(name)) {
            return true;
        }
    }
...
function $createCollabNodeFromLexicalNode(binding, lexicalNode, parent) {
    ...
    let collabNode;
    if ((0, lexicalLexical_js_1.$isElementNode)(lexicalNode)) {
        const xmlText = new yjs_1.XmlText();
    ...
    else if ((0, lexicalLexical_js_1.$isTextNode)(lexicalNode)) {
        // TODO create a token text node for token, segmented nodes.
        const map = new yjs_1.Map();
    ...
    else if ((0, lexicalLexical_js_1.$isLineBreakNode)(lexicalNode)) {
        const map = new yjs_1.Map();
        map.set('__type', 'linebreak');
        collabNode = $createCollabLineBreakNode(map, parent);
    }
    else if ((0, lexicalLexical_js_1.$isDecoratorNode)(lexicalNode)) {
        const xmlElem = new yjs_1.XmlElement();
...
function getNodeTypeFromSharedType(sharedType) {
    const type = sharedType instanceof yjs_1.Map ? sharedType.get('__type') : sharedType.getAttribute('__type');
    if (!(type != null)) {
...
function syncPropertiesFromYjs(binding, sharedType, lexicalNode, keysChanged) {
    const properties = keysChanged === null ? sharedType instanceof yjs_1.Map ? Array.from(sharedType.keys()) : Object.keys(sharedType.getAttributes()) : Array.from(keysChanged);
    let writableNode;
    for (let i = 0; i < properties.length; i++) {
        ...
        let nextValue = sharedType instanceof yjs_1.Map ? sharedType.get(property) : sharedType.getAttribute(property);
        if (prevValue !== nextValue) {
            if (nextValue instanceof yjs_1.Doc) {
                const yjsDocMap = binding.docMap;
                if (prevValue instanceof yjs_1.Doc) {
                    yjsDocMap.delete(prevValue.guid);
                }
                const nestedEditor = (0, lexicalLexical_js_1.createEditor)();
    ...
    for (let i = 0; i < properties.length; i++) {
        ...
        if (prevValue !== nextValue) {
            if (nextValue instanceof EditorClass) {
                ...
                const doc = prevDoc || new yjs_1.Doc();
                const key = doc.guid;
                // @ts-expect-error Lexical node
                nextValue._key = key;
            ...
            if (sharedType instanceof yjs_1.Map) {
...
function doesSelectionNeedRecovering(selection) {
    ...
    try {
        ...
        !anchorNode.isAttached() || !focusNode.isAttached() ||
            ...
            (0, lexicalLexical_js_1.$isTextNode)(anchorNode) && anchor.offset > anchorNode.getTextContentSize() || (0, lexicalLexical_js_1.$isTextNode)(focusNode) && focus.offset > focusNode.getTextContentSize()) {
            recoveryNeeded = true;
        }
    }
    catch (e) {
...
function createChildrenArray(element, nodeMap) {
    ...
    while (nodeKey !== null) {
        const node = nodeMap === null ? (0, lexicalLexical_js_1.$getNodeByKey)(nodeKey) : nodeMap.get(nodeKey);
...
class CollabDecoratorNode {
    ...
    getPrevNode(nodeMap) {
        ...
        const node = nodeMap.get(this._key);
        return (0, lexicalLexical_js_1.$isDecoratorNode)(node) ? node : null;
    }
    getNode() {
        const node = (0, lexicalLexical_js_1.$getNodeByKey)(this._key);
        return (0, lexicalLexical_js_1.$isDecoratorNode)(node) ? node : null;
...
class CollabElementNode {
    ...
    getPrevNode(nodeMap) {
        ...
        const node = nodeMap.get(this._key);
        return (0, lexicalLexical_js_1.$isElementNode)(node) ? node : null;
    }
    getNode() {
        const node = (0, lexicalLexical_js_1.$getNodeByKey)(this._key);
        return (0, lexicalLexical_js_1.$isElementNode)(node) ? node : null;
    ...
    syncChildrenFromYjs(binding) {
        ...
        for (let i = 0; i < collabChildrenLength; i++) {
            ...
            if (collabLexicalChildNode !== null && lexicalChildKey === collabKey) {
                const childNeedsUpdating = (0, lexicalLexical_js_1.$isTextNode)(collabLexicalChildNode);
                // Update
    ...
    _syncChildFromLexical(binding, index, key, prevNodeMap, dirtyElements, dirtyLeaves) {
        ...
        if (childCollabNode instanceof CollabElementNode && (0, lexicalLexical_js_1.$isElementNode)(nextChildNode)) {
        ...
        else if (childCollabNode instanceof CollabTextNode && (0, lexicalLexical_js_1.$isTextNode)(nextChildNode)) {
            childCollabNode.syncPropertiesAndTextFromLexical(binding, nextChildNode, prevNodeMap);
        }
        else if (childCollabNode instanceof CollabDecoratorNode && (0, lexicalLexical_js_1.$isDecoratorNode)(nextChildNode)) {
            childCollabNode.syncPropertiesFromLexical(binding, nextChildNode, prevNodeMap);
        }
    }
...
function createBinding(editor, provider, id, doc, docMap, excludedProperties) {
    ...
    const rootXmlText = doc.get('root', yjs_1.XmlText);
...
function createRelativePosition(point, binding) {
    ...
    return (0, yjs_1.createRelativePositionFromTypeIndex)(sharedType, offset);
}
function createAbsolutePosition(relativePosition, binding) {
    return (0, yjs_1.createAbsolutePositionFromRelativePosition)(relativePosition, binding.doc);
}
function shouldUpdatePosition(currentPos, pos) {
    ...
    else if (pos == null || !(0, yjs_1.compareRelativePositions)(currentPos, pos)) {
        return true;
    }
    return false;
}
...
function createCursorSelection(cursor, anchorKey, anchorOffset, focusKey, focusOffset) {
    ...
    name.style.cssText = `position:absolute;left:-2px;top:-16px;background-color:${color};color:#fff;line-height:12px;font-size:12px;padding:2px;font-family:Anuphan,sans-serif;font-weight:bold;white-space:nowrap;`;
...
function updateCursor(binding, cursor, nextSelection, nodeMap) {
    ...
    if (anchorNode === focusNode && (0, lexicalLexical_js_1.$isLineBreakNode)(anchorNode)) {
        const brRect = editor.getElementByKey(anchorKey).getBoundingClientRect();
        selectionRects = [brRect];
    }
    else {
        const range = (0, lexical_selectionLexicalSelection_js_1.createDOMRange)(editor, anchorNode, anchor.offset, focusNode, focus.offset);
        if (range === null) {
            return;
        }
        selectionRects = (0, lexical_selectionLexicalSelection_js_1.createRectsFromDOMRange)(editor, range);
    }
    const selectionsLength = selections.length;
...
function setPoint(point, key, offset) {
    if (point.key !== key || point.offset !== offset) {
        let anchorNode = (0, lexicalLexical_js_1.$getNodeByKey)(key);
        if (anchorNode !== null && !(0, lexicalLexical_js_1.$isElementNode)(anchorNode) && !(0, lexicalLexical_js_1.$isTextNode)(anchorNode)) {
            const parent = anchorNode.getParentOrThrow();
            key = parent.getKey();
            offset = anchorNode.getIndexWithinParent();
            anchorNode = parent;
        }
        point.set(key, offset, (0, lexicalLexical_js_1.$isElementNode)(anchorNode) ? 'element' : 'text');
    }
}
...
function syncLexicalSelectionToYjs(binding, provider, prevSelection, nextSelection) {
    ...
    if ((0, lexicalLexical_js_1.$isRangeSelection)(nextSelection)) {
...
function syncEvent(binding, event) {
    ...
    if (collabNode instanceof CollabElementNode && event instanceof yjs_1.YTextEvent) {
    ...
    else if (collabNode instanceof CollabTextNode && event instanceof yjs_1.YMapEvent) {
    ...
    else if (collabNode instanceof CollabDecoratorNode && event instanceof yjs_1.YXmlEvent) {
    ...
    editor.update(() => {
        ...
        if ((0, lexicalLexical_js_1.$isRangeSelection)(selection)) {
            ...
            if (doesSelectionNeedRecovering(selection)) {
                ...
                if ((0, lexicalLexical_js_1.$isRangeSelection)(prevSelection)) {
                    const prevOffsetView = (0, lexical_offsetLexicalOffset_js_1.$createOffsetView)(editor, 0, currentEditorState);
                    const nextOffsetView = (0, lexical_offsetLexicalOffset_js_1.$createOffsetView)(editor, 0, pendingEditorState);
                    ...
                    if (nextSelection !== null) {
                        ...
                        if (doesSelectionNeedRecovering(selection)) {
                            ...
                            // Fallback
                            (0, lexicalLexical_js_1.$getRoot)().selectEnd();
                        }
                    }
                }
                syncLexicalSelectionToYjs(binding, provider, prevSelection, (0, lexicalLexical_js_1.$getSelection)());
            }
            else {
                syncLocalCursorPosition(binding, provider);
            }
...
function handleNormalizationMergeConflicts(binding, normalizedNodes) {
    ...
    for (let i = 0; i < normalizedNodesKeys.length; i++) {
        ...
        const lexicalNode = (0, lexicalLexical_js_1.$getNodeByKey)(nodeKey);
        const collabNode = collabNodeMap.get(nodeKey);
        if (collabNode instanceof CollabTextNode) {
            if ((0, lexicalLexical_js_1.$isTextNode)(lexicalNode)) {
    ...
    syncWithTransaction(binding, () => {
        currEditorState.read(() => {
            ...
            if (dirtyElements.has('root')) {
                ...
                const nextLexicalRoot = (0, lexicalLexical_js_1.$getRoot)();
...
/** @module @lexical/yjs */
const CONNECTED_COMMAND = (0, lexicalLexical_js_1.createCommand)('CONNECTED_COMMAND');
exports.CONNECTED_COMMAND = CONNECTED_COMMAND;
const TOGGLE_CONNECT_COMMAND = (0, lexicalLexical_js_1.createCommand)('TOGGLE_CONNECT_COMMAND');
exports.TOGGLE_CONNECT_COMMAND = TOGGLE_CONNECT_COMMAND;
function createUndoManager(binding, root) {
    return new yjs_1.UndoManager(root, {
