...
exports.getTextWithBlockElementsEditorAIChat = exports.getAIChatHTML = exports.getMarkdownHTML = exports.getCodeEditorsHTML = void 0;
...
const lexicalImplementations_1 = require("./pages_helper/lexicalImplementations");
const database_1 = __importDefault(require("../database"));
const escape_html_1 = __importDefault(require("escape-html"));
const COMPONENTS_ROUTER = (0, express_1.Router)();
/* ----------------------- Navigation Sidebar --------------------------------- */
function getNavigationSidebar(req, res) {
    var _a, _b;
    return res.status(200).render('navigation', {
        layout: false,
        phone: ((_a = req.session.device) === null || _a === void 0 ? void 0 : _a.phone) || false,
        auth: Number((_b = req.session.auth) === null || _b === void 0 ? void 0 : _b.level) || 0
    });
}
COMPONENTS_ROUTER.get('/navigation-sidebar/?', getNavigationSidebar);
/* ----------------------- Settings Style --------------------------------- */
function getSettingsStyles(req, res) {
    var _a, _b, _c;
    const settingsString = ((_a = req.session.settings) === null || _a === void 0 ? void 0 : _a.settingsString) || DEFAULT_SETTINGS_1.default.settingsString;
    const navbarColor = (0, settingsHandler_1.getNavbarColor)(req.session.settings);
    const fillColor = (0, settingsHandler_1.getFillColor)(req.session.settings);
    const strokeColor = (0, settingsHandler_1.getStrokeColor)(req.session.settings);
    const mode = ((_c = (_b = req.session) === null || _b === void 0 ? void 0 : _b.settings) === null || _c === void 0 ? void 0 : _c.mode) || 'light';
    const html = /*html*/ `
    <head hx-merge="true">
...
function getCodeEditorsHTML(theme, state) {
...
exports.getCodeEditorsHTML = getCodeEditorsHTML;
function getArticleBuilderCode(req, res) {
    var _a;
    const theme = ((_a = req.session.settings) === null || _a === void 0 ? void 0 : _a.code_editor_theme) || 'vscode';
...
function getMarkdownHTML(theme, state) {
    const rand = node_crypto_1.default.randomUUID();
    const wrapperID = 'w_'.concat(rand);
    const wrapper4ID = 'w4_'.concat(rand);
...
exports.getMarkdownHTML = getMarkdownHTML;
function getArticleBuilderMarkdownHTML(req, res) {
    var _a;
    const theme = ((_a = req.session.settings) === null || _a === void 0 ? void 0 : _a.code_editor_theme) || 'vscode';
 ...
 */
function getAIChatHTML(rows, for_lexical = false) {
    var str = '';
    if (rows.length) {
...
async function getTextWithBlockElementsEditorAIChat(req, res) {
    ...
    const [messages, chat_names_rows_resp] = await Promise.all([
        db.query(`SELECT id, user_message, html, model, is_error AS error, chat_name FROM ai_chat WHERE chat_id=$1 ORDER BY id DESC LIMIT 100;`, [((_b = req.session.settings) === null || _b === void 0 ? void 0 : _b.ai_chat_id) || '']),
    ...
    const current_chat_id = (_c = req.session.settings) === null || _c === void 0 ? void 0 : _c.ai_chat_id;
    ...
    const chat_names_rows = chat_names_rows_resp.rows.filter((obj) => Boolean(obj.chat_name && obj.chat_id)).map((obj) => {
        ...
        return `<button type="button" tabindex="-1" class="select-option" role="option" aria-selected="${current_chat}" data-val="${(0, escape_html_1.default)(chat_id)}">${(0, escape_html_1.default)(chat_name)}</button>`;
    }).join('');
  ...
  <div class="select">
    ...
    <div tabindex="0" id="ai-chat-select-dropdown" role="listbox" class="select-menu o-xs" data-placement="bottom">
      <input type="text" hidden value="${(0, escape_html_1.default)(current_chat_id)}" name="ai-chat-select-input" id="ai-chat-select-input">
      ${chat_names_rows}
    </div>
  </div>
</div>`;
...
exports.getTextWithBlockElementsEditorAIChat = getTextWithBlockElementsEditorAIChat;
COMPONENTS_ROUTER.get('/ai-chat-prompt/?', getTextWithBlockElementsEditorAIChat);
exports.default = COMPONENTS_ROUTER;
