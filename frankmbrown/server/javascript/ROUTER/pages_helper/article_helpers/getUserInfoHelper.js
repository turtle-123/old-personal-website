...
const individualAnnotation = node_fs_1.default.readFileSync(node_path_1.default.resolve(BASE_PATH_1.default, 'views', 'partials', 'individual-annotation-user-activity.ejs'), { encoding: 'utf-8' });
const individualComment = node_fs_1.default.readFileSync(node_path_1.default.resolve(BASE_PATH_1.default, 'views', 'partials', 'individual-comment-user-activity.ejs'), { encoding: 'utf-8' });
...
async function getUserInformation(req, user_id) {
    var _a, _b, _c;
...
LIMIT 50;`;
  ...
  <div class="flex-row justify-between align-center">
        <span class="flex-row gap-2 align-center">
          ...
          <span class="caption">${(0, escape_html_1.default)(articleType)}</span>
          </span>
        ...
        if (commentsRows.length) {
            comments = commentsRows.map((obj) => {
                ...
                return ejs_1.default.render(individualComment, { ...obj, url: url.concat('#comments'), articleType, time_ago, svg_style });
            }).join('');
        }
        if (annotationsRows.length) {
            annotations = annotationsRows.map((obj) => {
                ...
                const randomColor = getRandomHexColor();
                const svg_style = `style="fill: ${randomColor}; color: ${randomColor};"`;
                return ejs_1.default.render(individualAnnotation, { ...obj, url: url.concat('?show-annotations=on&search-comments=&comment-search-type=default&comment-sort-by=best&search-annotations=&annotation-search-type=default'), articleType, time_ago, vote_count, svg_style });
            }).join('');
        }
