...
exports.getDeepCommentsHelper = exports.getGetCommentQuery = exports.getSearchCommentQueryAndArray = exports.getGetCommentsQueryAndArray = void 0;
const pg_1 = require("pgvector/pg");
const ai_1 = require("../../../ai");
const database_1 = __importDefault(require("../../../database"));
const handleArticleData_1 = require("./handleArticleData");
/**
 ...
 * @returns
 */
function getGetCommentsQueryAndArray(obj, type_id, type_html, is_logged_in, limit = 15, exclude, base_comment_parent, max_level = 6) {
    ...
    if (base_comment_parent)
        arr.push(base_comment_parent);
    if (exclude)
        arr.push(exclude);
    var order_by = '';
    if (obj.sort_by === "new") {
...
exports.getGetCommentsQueryAndArray = getGetCommentsQueryAndArray;
async function getSearchCommentQueryAndArray(obj, search_term, search_type, type_id, type_html, is_logged_in, limit = 100, exclude) {
    var query_var = 2;
    var exclude_var = 3;
    if (is_logged_in) {
        query_var += 1;
...
exports.getSearchCommentQueryAndArray = getSearchCommentQueryAndArray;
function getGetCommentQuery(obj, type_html) {
...
exports.getGetCommentQuery = getGetCommentQuery;
async function getDeepCommentsHelper(req, obj, type_html) {
    try {
        const exclude = [obj.id];
        const [query, arr] = getGetCommentQuery(obj, type_html);
        const original_comment_resp = await (0, database_1.default)().query(query, arr);
        ...
        const original_comment = original_comment_resp.rows[0];
        ...
        for (let comment of comments) {
            ...
            const comment_resp_row = comment.rows[0];
            exclude.push(comment_resp_row.id);
            const node = new handleArticleData_1.CommentNode(comment_resp_row);
            str_arr += node.render(req);
        }
        const base_node = new handleArticleData_1.CommentNode(original_comment);
        str_arr += base_node.render(req);
        return [str_arr, exclude];
    }
    catch (e) {
        console.error(e);
...
exports.getDeepCommentsHelper = getDeepCommentsHelper;
