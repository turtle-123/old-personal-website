...
export function getGetAnnotationsQueryAndArray(obj:{type_id:string,id:number,user_id:number|undefined},sort_by:'best'|'new'|'recommended-1'|'recommended-2',limit:number=100,exclude?:number[]) {
...
export async function searchAnnotationsQueryAndArray(obj:{type_id:string,id:number,user_id:number|undefined},sort_by:'best'|'new'|'recommended-1'|'recommended-2',search_term:string,search_type:'default'|'semantic',limit:number=100,exclude?:number[]) {
  const { type_id, id } = obj;
  var query_var = 2;
  var exclude_var = 3;
  if (Number.isInteger(obj.user_id)) {
