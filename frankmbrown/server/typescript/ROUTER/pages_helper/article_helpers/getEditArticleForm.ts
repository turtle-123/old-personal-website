...
function getArticleVersionSelect({ current_v, versions, rand, type, id, title }:{current_v:number, versions: number[], rand: string, type:'blog'|'note'|'idea', id: number, title: string }) {  
 ...
 * @returns 
 */
export function getArticleForm(req:Request,{ innerHTML, id, title, description, image, current_v, versions, type, desktop }:{innerHTML:string, id?:number, title: string, description: string, image: string|null, current_v?: number|undefined,  versions:number[], type:'blog'|'note'|'idea'|'diary', desktop: boolean }) {
  ...
  var deleteArticle = '';
  if (type!=="diary") {
    if (NUMBER.isValidInteger(current_v)&&versions.length>=1&&NUMBER.isValidInteger(id)) selectStr = getArticleVersionSelect({current_v,versions,rand, type, id, title});
    if (!!id) deleteArticle =  /*html*/`<div class="flex-row w-100 mt-2 justify-center align-center" style="padding-top:6px; border-top:1px solid var(--divider);">
  ...
  const articleBuilderLexical = getArticleBuilderImplementation(req,"article-body",innerHTML);
