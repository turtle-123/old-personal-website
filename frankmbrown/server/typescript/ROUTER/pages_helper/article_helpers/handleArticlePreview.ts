...
import { getErrorAlert, getWarningAlert } from '../../html';
...
<%}%>
`
type ArticlePreviewReturnRow = Partial<{id: string, date_created: string, last_edited: string, title: string, description: string, active_v: string, image: string, view_count: string, like_count: string, comment_count: string, annotation_count: string, article_type: string, rating?:number|null }>;

function renderArticlePreviewNew(url:string,isMobile:boolean,row:ArticlePreviewReturnRow,tz:TIME.ALLOWED_TIME_ZONE) {
  const id = row.id;
  ...
  const view_count=row.view_count!==null&&row.view_count!==undefined?formatNumberAttribute(String(row.view_count)):undefined;
  const like_count=row.like_count!==null&&row.like_count!==undefined?formatNumberAttribute(String(row.like_count)):undefined;
  const comment_count=row.comment_count!==null&&row.comment_count!==undefined?formatNumberAttribute(String(row.comment_count)):undefined;
  const annotation_count=row.annotation_count!==null&&row.annotation_count!==undefined?formatNumberAttribute(String(row.annotation_count)):undefined;
  ...
  if (date_created?.length||last_edited?.length) {
...
export function isReOrderRequest(req:Request) {
  return Boolean(req.get('hx-target')==="articles-inner");
}
export function updateTocOnReOrderScript(req:Request) {
...
type ArticleOrderByType = 'last-edit-new'|'last-edit-old'|'date-created-new'|'date-created-old'|'num-comments'|'num-views'|'num-annotations'|'num-likes'|'rating';
export const VALID_ARTICLE_PREVIEW_SORT_BY = new Set(['last-edit-new','last-edit-old','date-created-new','date-created-old','num-comments','num-views','num-annotations','num-likes','rating'] as const);
export const ARTICLE_SORT_BY_TO_STRING = {
...
export function getOrderBy(req:Request) {
  var order_by = Array.isArray(req.query['order-by-select']) ? String(req.query['order-by-select'][0]) : String(req.query['order-by-select']);
  if (isMemberOf(order_by,VALID_ARTICLE_PREVIEW_SORT_BY)) return { order_by_value: order_by, order_by_str: ARTICLE_SORT_BY_TO_STRING[order_by] } as const;
  else return { order_by_value: 'last-edit-new', order_by_str: "Last Edited (New)"} as const;
}
const GROUP_BY_MONTH_YEAR = new Set(['last-edit-new','last-edit-old','date-created-new','date-created-old'] as const);
export async function GET_ARTICLE_PREVIEWS(typ:'blog_id'|'idea_id'|'note_id'|'wikipedia_id'|'tex_notes_id'|'jupyter_notebooks_id'|'markdown_notes_id'|'goodreads_id'|'letterboxd_id'|'game_id'|'design_id'|'electronics_id',order_by:ArticleOrderByType,req:Request):Promise<{toc:TableOfContentsItem[], str:string}> {
  try {
    var gro_by_month = false;
    if (isMemberOf(order_by,GROUP_BY_MONTH_YEAR)) gro_by_month = true;
    if (order_by==="rating"&&typ!=='goodreads_id'&&typ!=='letterboxd_id') throw new Error("Ordering by rating is only available for goodreads and letterboxd");
    if (!!!VALID_TYPS_SET.has(typ)) throw new Error("Umable to get article preview query.");
    const HAS_ANNOTATION = !!!NOT_HAS_ANNOTATION.has(typ);
    if (!!!HAS_ANNOTATION&&order_by==="num-annotations") throw new Error("Cannot sort by the number of annotations when the ");
    if ((order_by==="last-edit-new"||order_by==="last-edit-old")&&typ==="jupyter_notebooks_id") throw new Error("You cannot sort by last edited with Jupyter Notebooks."); 
    ...
    const dbResp = await getDatabase().query(str);
    const rows = dbResp.rows as ArticlePreviewReturnRow[];
    if (!!!rows||!!!rows.length) {
      return { str: getWarningAlert("There are no article previews to show.",undefined,false), toc: [] };
    } else {
      var ret_str = '';
      if (gro_by_month) {
        const toc:TableOfContentsItem[] = [];
        var curr_month_year = (order_by==="date-created-new"||order_by==="date-created-old")?TIME.getMonthYearFromUnix(String(rows[0].date_created),req.session.settings?.timezone||'America/New_York'):TIME.getMonthYearFromUnix(String(rows[0].last_edited),req.session.settings?.timezone||'America/New_York');
...
`;
        ...
        for (let row of rows) {
          ...
          const month_year = (order_by==="date-created-new"||order_by==="date-created-old")?TIME.getMonthYearFromUnix(String(row.date_created),req.session.settings?.timezone||'America/New_York'):TIME.getMonthYearFromUnix(String(row.last_edited),req.session.settings?.timezone||'America/New_York');
          if (month_year===curr_month_year) {
            ret_str += renderArticlePreviewNew(url,!!!req.session.device?.desktop,row,req.session.settings?.timezone||'America/New_York');
          } else {
  ...
  <div class="mt-2">`;
    ...
    console.error(e);
    return { str: getErrorAlert("Something went wrong getting the article previews.",undefined,false), toc: []}
  }
}


/* -------------- Render Diary Preview ------------- */
export type PreviewDiaryType = {
  date_created: number,
  last_edited?: number,
  title: string,
  description: string,
  type: 'diary'|'blog'|'note'|'idea',
  image?:string,
  id: number,
  admin: boolean
};
export function renderDiaryPreview(obj:PreviewDiaryType,req:Request) {
  const {
    date_created, 
    last_edited,
    title, 
  ...
  const second_string = `${lastEditedString ? /*html*/`
    ...
    `:''}
`;
  return renderArticleComponentLink({ link: true, path, phone: Boolean(!!!req.session.device?.desktop), src: image || undefined, alt: title, title, description, date_created: dateCreatedString, second_string })
}
...
export function renderArticlePreview(obj:PreviewArticleType,req:Request) {
  const {
    date_created, 
    last_edited,
    title, 
...
export function renderMarkdownPreview(obj:{
  date_created: string;
  last_edited: string;
  title: string;
  description: string;
...
export function renderEditMarkdownNotePreview(obj:{
  date_created: string;
  last_edited: string;
  title: string;
  description: string;
...
export function renderJupyterPreview(obj:{ id:string, title:string, description:string, date_created:string, path: string, phone: boolean, image: string }) {
  return renderArticleComponentLink({
    link: true, 
    path:obj.path,
    phone:obj.phone,
...
export function renderDailyReadingCompletedPreview(obj:{ id: number, topic_name: string, what_interested_you: string, topic_image: string, date_submitted: number, phone: boolean }) {
...
export function renderDailyReadingToDoPreview(obj:{ id: number, topic_name: string, what_interested_you: string, topic_image: string, date_created: number, phone: boolean }) {
 ...
 * @returns 
 */
export function renderArticleComponentLink(obj:{link:boolean,path?:string,phone:boolean,src?:string,alt?:string,title:string,description:string,date_created:string,second_string?:string}) {
  return ejs.render(ejsTemplate,obj);
}

type RenderNotePreviewType = {
  id: number, 
  date_added: string, 
  ...
  note_image: string,
  published: boolean,
  flairs: {color: string, background_color: string, text: string}[]
}
export function renderTexNotePreview(req:Request,obj:RenderNotePreviewType) {
  var path = '';
  const IS_ADMIN = Number(req.session.auth?.level)>=3;
  if (obj.published) path = `/tex-notes/${encodeURIComponent(obj.id)}/${encodeURIComponent(obj.note_title)}`;
  ...
  <svg focusable="false" inert viewBox="0 0 24 24" tabindex="-1" title="Create"><path d="M3 17.25V21h3.75L17.81 9.94l-3.75-3.75L3 17.25zM20.71 7.04c.39-.39.39-1.02 0-1.41l-2.34-2.34a.9959.9959 0 0 0-1.41 0l-1.83 1.83 3.75 3.75 1.83-1.83z"></path></svg>
  <span></span>
</time>` : `<span class="bold">ID: ${escapeHTML(obj.id.toString())}</span>`;
  ...
  const ejsTemplateTeXNote = /*html*/`<%if(locals.published){%>
    <a hx-push-url="true" class="preview-article" href="<%=locals.path%>" hx-get="<%=locals.path%>" hx-target="#PAGE" hx-indicator="#page-transition-progress">
          <div style="background-color: black; flex-shrink: 0;" class="flex-row justify-center align-center">
            ...
            <div class="p-md flex-column justify-start align-start grow-1" style="min-width: 0px; paddding-bottom: 0px;">
  ...
  <%}%>`; 
  return ejs.render(ejsTemplateTeXNote,{ ...obj, date_created, second_string, path })
}