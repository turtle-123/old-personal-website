...
CREATE TABLE surveys (
  ...
  validated boolean default true, -- only admin creating surveys now
...
CREATE INDEX ON survey_description USING BTREE(survey_id);

CREATE TABLE survey_description_moderation (
  id serial PRIMARY KEY,
...
CREATE INDEX ON survey_description_moderation USING BTREE(survey_id);
...
CREATE INDEX ON survey_search USING BTREE(survey_id);
CREATE INDEX ON survey_search USING GIN (search);
CREATE INDEX ON survey_search USING hnsw (embedding vector_cosine_ops);
...
CREATE TABLE survey_questions (
  ...
  question_type valid_poll_question_types NOT NULL,
  editor_state jsonb NOT NULL,
  desktop_html TEXT NOT NULL,
  tablet_html TEXT NOT NULL,
  mobile_html TEXT NOT NULL,
...
CREATE INDEX ON survey_questions USING BTREE(survey_id);
ALTER TABLE survey_questions ADD CONSTRAINT unique_survey_question UNIQUE(survey_id, question_number);


CREATE TABLE survey_question_moderation (
  id serial PRIMARY KEY,
...
CREATE TABLE survey_question_search (
  id serial PRIMARY KEY,
...
CREATE INDEX ON survey_question_search USING BTREE(survey_question_id);
CREATE INDEX ON survey_question_search USING GIN (search);
CREATE INDEX ON survey_question_search USING hnsw (embedding vector_cosine_ops);

CREATE TABLE survey_short_answer_info (
  id serial PRIMARY KEY,
  survey_question_id int NOT NULL REFERENCES survey_questions(id) ON DELETE CASCADE,
  min_length int,
  max_length int,
...
CREATE INDEX ON survey_short_answer_info USING BTREE(survey_question_id);


CREATE TABLE survey_paragraph_info (
  id serial PRIMARY KEY,
  survey_question_id int NOT NULL REFERENCES survey_questions(id) ON DELETE CASCADE,
  min_length int, 
  max_length int,
...
CREATE INDEX ON survey_paragraph_info USING BTREE(survey_question_id);

CREATE TABLE survey_multiple_choice_options (
  id serial PRIMARY KEY,
  survey_question_id int NOT NULL REFERENCES survey_questions(id) ON DELETE CASCADE,
  option_type valid_option_types NOT NULL,
  option_number smallint NOT NULL,
  default_checked boolean NOT NULL,
  version smallint NOT NULL DEFAULT 1
);
CREATE INDEX ON survey_multiple_choice_options USING BTREE(survey_question_id);
ALTER TABLE survey_multiple_choice_options
ADD CONSTRAINT unique_mc_option
UNIQUE(survey_question_id,option_number);

-- COME BACK ADD CONSTRAINTS

CREATE TABLE survey_checkbox_info (
  id serial PRIMARY KEY, 
  survey_question_id int NOT NULL REFERENCES survey_questions(id) ON DELETE CASCADE,
  min_selected smallint,
  max_selected smallint
);
CREATE INDEX ON survey_checkbox_info USING BTREE(survey_question_id);


CREATE TABLE survey_checkbox_options (
  id serial PRIMARY KEY, 
  survey_question_id int NOT NULL REFERENCES survey_questions(id) ON DELETE CASCADE,
  option_type valid_option_types NOT NULL,
  option_number smallint NOT NULL,
...
CREATE INDEX ON survey_checkbox_options USING BTREE(survey_question_id);


CREATE TABLE survey_ranking_options (
  id serial PRIMARY KEY, 
  survey_question_id int NOT NULL REFERENCES survey_questions(id) ON DELETE CASCADE,
  option_type valid_option_types NOT NULL,
...
CREATE INDEX ON survey_ranking_options USING BTREE(survey_question_id);


CREATE TABLE survey_paragraph_option_type (
  id serial PRIMARY KEY,
  survey_multiple_choice_option_id int REFERENCES survey_multiple_choice_options(id) ON DELETE CASCADE,
  survey_checkbox_option_id int REFERENCES survey_checkbox_options(id) ON DELETE CASCADE,
  survey_ranking_option_id int REFERENCES survey_ranking_options(id) ON DELETE CASCADE,
  editor_state jsonb NOT NULL,
  desktop_html TEXT NOT NULL,
  tablet_html TEXT NOT NULL,
  mobile_html TEXT NOT NULL,
  lexical_uuid uuid DEFAULT gen_random_uuid(),
  CHECK (survey_multiple_choice_option_id IS NOT NULL OR survey_checkbox_option_id IS NOT NULL OR survey_ranking_option_id IS NOT NULL)
);
CREATE INDEX ON survey_paragraph_option_type USING BTREE(survey_multiple_choice_option_id);
CREATE INDEX ON survey_paragraph_option_type USING BTREE(survey_checkbox_option_id);
CREATE INDEX ON survey_paragraph_option_type USING BTREE(survey_ranking_option_id);

CREATE TABLE audio_option_type (
  id serial PRIMARY KEY,
  survey_multiple_choice_option_id int REFERENCES survey_multiple_choice_options(id) ON DELETE CASCADE,
  survey_checkbox_option_id int REFERENCES survey_checkbox_options(id) ON DELETE CASCADE,
  survey_ranking_option_id int REFERENCES survey_ranking_options(id) ON DELETE CASCADE,
...
CREATE INDEX ON audio_option_type USING BTREE(survey_multiple_choice_option_id);
CREATE INDEX ON audio_option_type USING BTREE(survey_checkbox_option_id);
CREATE INDEX ON audio_option_type USING BTREE(survey_ranking_option_id);

CREATE TABLE video_option_type (
  id serial PRIMARY KEY,
  survey_multiple_choice_option_id int REFERENCES survey_multiple_choice_options(id) ON DELETE CASCADE,
  survey_checkbox_option_id int REFERENCES survey_checkbox_options(id) ON DELETE CASCADE,
  survey_ranking_option_id int REFERENCES survey_ranking_options(id) ON DELETE CASCADE,
...
CREATE INDEX ON video_option_type USING BTREE(survey_multiple_choice_option_id);
CREATE INDEX ON video_option_type USING BTREE(survey_checkbox_option_id);
CREATE INDEX ON video_option_type USING BTREE(survey_ranking_option_id);

CREATE TABLE image_option_type (
  id serial PRIMARY KEY,
  survey_multiple_choice_option_id int REFERENCES survey_multiple_choice_options(id) ON DELETE CASCADE,
  survey_checkbox_option_id int REFERENCES survey_checkbox_options(id) ON DELETE CASCADE,
  survey_ranking_option_id int REFERENCES survey_ranking_options(id) ON DELETE CASCADE,
...
CREATE INDEX ON image_option_type USING BTREE(survey_multiple_choice_option_id);
CREATE INDEX ON image_option_type USING BTREE(survey_checkbox_option_id);
CREATE INDEX ON image_option_type USING BTREE(survey_ranking_option_id);


CREATE TABLE survey_range_info (
  id serial PRIMARY KEY, 
  survey_question_id int NOT NULL REFERENCES survey_questions(id) ON DELETE CASCADE,
  range_min real NOT NULL,
  range_max real NOT NULL,
  range_step real NOT NULL,
  version smallint NOT NULL DEFAULT 1
);
CREATE INDEX ON survey_range_info USING BTREE(survey_question_id);

CREATE TABLE survey_range_point_descriptions (
  id serial PRIMARY KEY, 
  survey_question_id int NOT NULL REFERENCES survey_questions(id) ON DELETE CASCADE,
  point real NOT NULL,
  editor_state jsonb NOT NULL,
...
CREATE INDEX ON survey_range_point_descriptions USING BTREE(survey_question_id);

CREATE TABLE survey_date_info (
  id serial PRIMARY KEY, 
  survey_question_id int NOT NULL REFERENCES survey_questions(id) ON DELETE CASCADE,
  min_date TEXT,
  max_date TEXT,
...
CREATE INDEX ON survey_date_info USING BTREE(survey_question_id);

CREATE TABLE survey_datetime_info (
  id serial PRIMARY KEY, 
  survey_question_id int NOT NULL REFERENCES survey_questions(id) ON DELETE CASCADE,
  min_datetime TEXT,
  max_datetime TEXT,
...
CREATE INDEX ON survey_datetime_info USING BTREE(survey_question_id);

CREATE TABLE survey_time_info (
  id serial PRIMARY KEY, 
  survey_question_id int NOT NULL REFERENCES survey_questions(id) ON DELETE CASCADE,
  min_time TEXT,
  max_time TEXT,
...
CREATE INDEX ON survey_time_info USING BTREE(survey_question_id);

CREATE TABLE survey_month_info (
  id serial PRIMARY KEY, 
  survey_question_id int NOT NULL REFERENCES survey_questions(id) ON DELETE CASCADE,
  min_month TEXT,
  max_month TEXT,
...
CREATE INDEX ON survey_month_info USING BTREE(survey_question_id);

CREATE TABLE survey_color_info (
  id serial PRIMARY KEY, 
  survey_question_id int NOT NULL REFERENCES survey_questions(id) ON DELETE CASCADE,
  default_color TEXT NOT NULL,
  version smallint NOT NULL DEFAULT 1
);
CREATE INDEX ON survey_color_info USING BTREE(survey_question_id);

CREATE TABLE survey_selection_info (
  id serial PRIMARY KEY, 
  survey_question_id int NOT NULL REFERENCES survey_questions(id) ON DELETE CASCADE,
  include_other boolean NOT NULL,
  version smallint NOT NULL DEFAULT 1
);
CREATE INDEX ON survey_selection_info USING BTREE(survey_question_id);

CREATE TABLE survey_selection_options (
  id serial PRIMARY KEY, 
  survey_question_id int NOT NULL REFERENCES survey_questions(id) ON DELETE CASCADE,
...
CREATE INDEX ON survey_selection_options USING BTREE(survey_question_id);

CREATE TABLE survey_number_info (
  id serial PRIMARY KEY, 
  survey_question_id int NOT NULL REFERENCES survey_questions(id) ON DELETE CASCADE,
  min_number real,
  max_number real,
...
CREATE INDEX ON survey_number_info USING BTREE(survey_question_id);


DELETE FROM surveys;
DELETE FROM survey_description;
DELETE FROM survey_description_moderation;
DELETE FROM survey_search;
DELETE FROM survey_questions;
DELETE FROM survey_question_moderation;
DELETE FROM survey_question_search;
DELETE FROM survey_short_answer_info;
DELETE FROM survey_paragraph_info;
DELETE FROM survey_multiple_choice_options;
DELETE FROM survey_checkbox_options;
DELETE FROM survey_ranking_options;
DELETE FROM survey_paragraph_option_type;
DELETE FROM audio_option_type;
DELETE FROM video_option_type;
DELETE FROM image_option_type;
DELETE FROM survey_range_info;
DELETE FROM survey_range_point_descriptions;
DELETE FROM survey_date_info;
DELETE FROM survey_datetime_info;
DELETE FROM survey_time_info;
DELETE FROM survey_month_info;
DELETE FROM survey_color_info;
DELETE FROM survey_selection_info;
DELETE FROM survey_selection_options;
DELETE FROM survey_number_info;

DROP TABLE surveys CASCADE;
DROP TABLE survey_description CASCADE;
DROP TABLE survey_description_moderation CASCADE;
DROP TABLE survey_search CASCADE;
DROP TABLE survey_questions CASCADE;
DROP TABLE survey_question_moderation CASCADE;
DROP TABLE survey_question_search CASCADE;
DROP TABLE survey_short_answer_info CASCADE;
DROP TABLE survey_paragraph_info CASCADE;
DROP TABLE survey_multiple_choice_options CASCADE;
DROP TABLE survey_checkbox_options CASCADE;
DROP TABLE survey_ranking_options CASCADE;
DROP TABLE survey_paragraph_option_type CASCADE;
DROP TABLE audio_option_type CASCADE;
DROP TABLE video_option_type CASCADE;
DROP TABLE image_option_type CASCADE;
DROP TABLE survey_range_info CASCADE;
DROP TABLE survey_range_point_descriptions CASCADE;
DROP TABLE survey_date_info CASCADE;
DROP TABLE survey_datetime_info CASCADE;
DROP TABLE survey_time_info CASCADE;
DROP TABLE survey_month_info CASCADE;
DROP TABLE survey_color_info CASCADE;
DROP TABLE survey_selection_info CASCADE;
DROP TABLE survey_selection_options CASCADE;
DROP TABLE survey_number_info CASCADE;
...
CREATE INDEX ON survey_views USING BTREE(survey_id);
...
CREATE TABLE survey_responses (
  ...
  ip_id int NOT NULL REFERENCES ip_info(id) ON DELETE CASCADE,
  survey_id int NOT NULL REFERENCES surveys(id) ON DELETE CASCADE,
  date_responded bigint NOT NULL
);
CREATE INDEX ON survey_responses USING BTREE(ip_id);
CREATE INDEX ON survey_responses USING BTREE(survey_id);

CREATE TABLE survey_audio_response (
  id serial PRIMARY KEY,
  survey_response_id int NOT NULL REFERENCES survey_responses(id) ON DELETE CASCADE,
  survey_question_id int NOT NULL REFERENCES survey_questions(id) ON DELETE CASCADE,
  src TEXT NOT NULL,
  title TEXT NOT NULL
);
CREATE INDEX ON survey_audio_response USING BTREE(survey_response_id);
CREATE INDEX ON survey_audio_response USING BTREE(survey_question_id);

CREATE TABLE survey_checkbox_response (
  id serial PRIMARY KEY,
  survey_response_id int NOT NULL REFERENCES survey_responses(id) ON DELETE CASCADE,
  survey_question_id int NOT NULL REFERENCES survey_questions(id) ON DELETE CASCADE,
  survey_checkbox_option_id int NOT NULL REFERENCES survey_checkbox_options(id) ON DELETE CASCADE
);
CREATE INDEX ON survey_checkbox_response USING BTREE(survey_response_id);
CREATE INDEX ON survey_checkbox_response USING BTREE(survey_question_id);
CREATE INDEX ON survey_checkbox_response USING BTREE(survey_checkbox_option_id);

CREATE TABLE survey_color_response (
  id serial PRIMARY KEY,
  survey_response_id int NOT NULL REFERENCES survey_responses(id) ON DELETE CASCADE,
  survey_question_id int NOT NULL REFERENCES survey_questions(id) ON DELETE CASCADE,
  color TEXT NOT NULL
);
CREATE INDEX ON survey_color_response USING BTREE(survey_response_id);
CREATE INDEX ON survey_color_response USING BTREE(survey_question_id);

CREATE TABLE survey_date_response (
  id serial PRIMARY KEY,
  survey_response_id int NOT NULL REFERENCES survey_responses(id) ON DELETE CASCADE,
  survey_question_id int NOT NULL REFERENCES survey_questions(id) ON DELETE CASCADE,
  date TEXT NOT NULL
);
CREATE INDEX ON survey_date_response USING BTREE(survey_response_id);
CREATE INDEX ON survey_date_response USING BTREE(survey_question_id);

CREATE TABLE survey_datetime_response (
  id serial PRIMARY KEY,
  survey_response_id int NOT NULL REFERENCES survey_responses(id) ON DELETE CASCADE,
  survey_question_id int NOT NULL REFERENCES survey_questions(id) ON DELETE CASCADE,
  datetime TEXT NOT NULL
);
CREATE INDEX ON survey_datetime_response USING BTREE(survey_response_id);
CREATE INDEX ON survey_datetime_response USING BTREE(survey_question_id);

CREATE TABLE survey_image_response (
  id serial PRIMARY KEY,
  survey_response_id int NOT NULL REFERENCES survey_responses(id) ON DELETE CASCADE,
  survey_question_id int NOT NULL REFERENCES survey_questions(id) ON DELETE CASCADE,
  src TEXT NOT NULL,
...
CREATE INDEX ON survey_image_response USING BTREE(survey_response_id);
CREATE INDEX ON survey_image_response USING BTREE(survey_question_id);

CREATE TABLE survey_month_response (
  id serial PRIMARY KEY,
  survey_response_id int NOT NULL REFERENCES survey_responses(id) ON DELETE CASCADE,
  survey_question_id int NOT NULL REFERENCES survey_questions(id) ON DELETE CASCADE,
  month TEXT NOT NULL
);
CREATE INDEX ON survey_month_response USING BTREE(survey_response_id);
CREATE INDEX ON survey_month_response USING BTREE(survey_question_id);

CREATE TABLE survey_multiple_choice_responses (
  id serial PRIMARY KEY,
  survey_response_id int NOT NULL REFERENCES survey_responses(id) ON DELETE CASCADE,
  survey_question_id int NOT NULL REFERENCES survey_questions(id) ON DELETE CASCADE,
  survey_multiple_choice_option_id int NOT NULL REFERENCES survey_multiple_choice_options(id) ON DELETE CASCADE
);
CREATE INDEX ON survey_multiple_choice_responses USING BTREE(survey_response_id);
CREATE INDEX ON survey_multiple_choice_responses USING BTREE(survey_question_id);

CREATE TABLE survey_number_response (
  id serial PRIMARY KEY,
  survey_response_id int NOT NULL REFERENCES survey_responses(id) ON DELETE CASCADE,
  survey_question_id int NOT NULL REFERENCES survey_questions(id) ON DELETE CASCADE,
  number real NOT NULL
);
CREATE INDEX ON survey_number_response USING BTREE(survey_response_id);
CREATE INDEX ON survey_number_response USING BTREE(survey_question_id);

CREATE TABLE survey_paragraph_response (
  id serial PRIMARY KEY,
  survey_response_id int NOT NULL REFERENCES survey_responses(id) ON DELETE CASCADE,
  survey_question_id int NOT NULL REFERENCES survey_questions(id) ON DELETE CASCADE,
  response TEXT NOT NULL
);
CREATE INDEX ON survey_paragraph_response USING BTREE(survey_response_id);
CREATE INDEX ON survey_paragraph_response USING BTREE(survey_question_id);

CREATE TABLE survey_range_response (
  id serial PRIMARY KEY,
  survey_response_id int NOT NULL REFERENCES survey_responses(id) ON DELETE CASCADE,
  survey_question_id int NOT NULL REFERENCES survey_questions(id) ON DELETE CASCADE,
  input real NOT NULL
);
CREATE INDEX ON survey_range_response USING BTREE(survey_response_id);
CREATE INDEX ON survey_range_response USING BTREE(survey_question_id);

CREATE TABLE survey_ranking_responses (
  id serial PRIMARY KEY,
  survey_response_id int NOT NULL REFERENCES survey_responses(id) ON DELETE CASCADE,
  survey_question_id int NOT NULL REFERENCES survey_questions(id) ON DELETE CASCADE,
  survey_ranking_option_id int NOT NULL REFERENCES survey_ranking_options(id) ON DELETE CASCADE,
  ranking smallint NOT NULL
);
CREATE INDEX ON survey_ranking_responses USING BTREE(survey_response_id);
CREATE INDEX ON survey_ranking_responses USING BTREE(survey_question_id);
CREATE INDEX ON survey_ranking_responses USING BTREE(survey_ranking_option_id);

-- Come back
CREATE TABLE survey_selection_response (
  id serial PRIMARY KEY,
  survey_response_id int NOT NULL REFERENCES survey_responses(id) ON DELETE CASCADE,
  survey_question_id int NOT NULL REFERENCES survey_questions(id) ON DELETE CASCADE,
  choice int REFERENCES survey_selection_options(id) ON DELETE CASCADE, 
  other_checked boolean NOT NULL,
  other_option TEXT
);
CREATE INDEX ON survey_selection_response USING BTREE(survey_response_id);
CREATE INDEX ON survey_selection_response USING BTREE(survey_question_id);
CREATE INDEX ON survey_selection_response USING BTREE(choice);

CREATE TABLE survey_short_answer_response (
  id serial PRIMARY KEY,
  survey_response_id int NOT NULL REFERENCES survey_responses(id) ON DELETE CASCADE,
  survey_question_id int NOT NULL REFERENCES survey_questions(id) ON DELETE CASCADE,
  response TEXT NOT NULL
);
CREATE INDEX ON survey_short_answer_response USING BTREE(survey_response_id);
CREATE INDEX ON survey_short_answer_response USING BTREE(survey_question_id);

CREATE TABLE survey_short_blog_response (
  id serial PRIMARY KEY,
  survey_response_id int NOT NULL REFERENCES survey_responses(id) ON DELETE CASCADE,
  survey_question_id int NOT NULL REFERENCES survey_questions(id) ON DELETE CASCADE,
  editor_state jsonb NOT NULL,
...
CREATE INDEX ON survey_short_blog_response USING BTREE(survey_response_id);
CREATE INDEX ON survey_short_blog_response USING BTREE(survey_question_id);

CREATE TABLE survey_time_response (
  id serial PRIMARY KEY,
  survey_response_id int NOT NULL REFERENCES survey_responses(id) ON DELETE CASCADE,
  survey_question_id int NOT NULL REFERENCES survey_questions(id) ON DELETE CASCADE,
  time TEXT NOT NULL
);
CREATE INDEX ON survey_time_response USING BTREE(survey_response_id);
CREATE INDEX ON survey_time_response USING BTREE(survey_question_id);

CREATE TABLE survey_video_response (
  id serial PRIMARY KEY,
  survey_response_id int NOT NULL REFERENCES survey_responses(id) ON DELETE CASCADE,
  survey_question_id int NOT NULL REFERENCES survey_questions(id) ON DELETE CASCADE,
  src TEXT NOT NULL,
...
CREATE INDEX ON survey_video_response USING BTREE(survey_response_id);
CREATE INDEX ON survey_video_response USING BTREE(survey_question_id);
*/
import { Request, Response } from 'express';
import { getErrorAlert, getErrorSnackbar, getInfoAlert, getWarningAlert, getWarningSnackbar } from '../html';
...
import { parseFullLexicalEditor, parseSurveyOptionEditor, parseSurveyQuestionEditor, parseSurveyRangeDescriptionEditor, parseSurveyShortBlogEditor } from '../../lexical';
import escape from 'escape-html';
import { AudioOptionType, AudioRankingOptionType, QuestionStateAny, ImageOptionType, ImageRankingOptionType, SurveyParagraphOptionType, SurveyParagraphRankingOptionType, VideoOptionType, VideoRankingOptionType, ParsedQuestionStates, ParsedSurveyParagraphOptionType, ParsedAudioOptionType, ParsedVideoOptionType, ParsedImageOptionType, ParsedSurveyParagraphRankingOptionType, ParsedAudioRankingOptionType, ParsedVideoRankingOptionType, ParsedImageRankingOptionType, ParsedSurveyColorQuestionState, ParsedSurveyAudioQuestionState, ParsedSurveyImageQuestionState, ParsedSurveyShortBlogQuestionState, ParsedSurveyVideoQuestionState, ParsedSurveyCheckboxQuestionState, ParsedSurveyDateQuestionState, ParsedSurveyDatetimeQuestionState, ParsedSurveyMonthQuestionState, ParsedSurveyMultipleChoiceQuestionState, ParsedSurveyNumberQuestionState, ParsedSurveyParagraphQuestionState, ParsedSurveyRangeQuestionState, ParsedSurveyRankingQuestionState, ParsedSurveySelectionQuestionState, ParsedSurveyShortAnswerQuestionState, ParsedSurveyTimeQuestionState, SurveyQuestionResponseRow, SurveyResponseRow, ParsedSurveyQuestion, SurveyDateDbResponse, SurveyDatetimeDbResponse, SurveyMonthDbResponse, SurveyMultipleChoiceDbResponse, SurveyCheckboxResponse, SurveyColorDbResponse, SurveyNumberDbResponse, SurveyParagraphDbResponse, SurveyRangeDbResponse, SurveyRankingDbResponse, SurveySelectionDbResponse, SurveyShortAnswerDbResponse, SurveyTimeDbResponse, SurveyResponse, VideoSurveyResponse, AudioSurveyResponse, CheckboxSurveyResponse, ColorSurveyResponse, DateSurveyResponse, DatetimeSurveyResponse, ImageSurveyResponse, MonthSurveyResponse, MultipleChoiceSurveyResponse, NumberSurveyResponse, ParagraphSurveyResponse, RangeSurveyResponse, RankingSurveyResponse, SelectionSurveyResponse, ShortAnswerSurveyResponse, ShortBlogResponse, TimeSurveyResponse, ValidSurveyQuestionTypes, ValidateSurveyResponseDbResponse, ParsedSurveyResponse, ValidateCheckboxDbResponse, ValidateDateDbResponse, ValidateDatetimeDbResponse, ValidateMonthDbResponse, ValidateMultipleChoiceDbResponse, ValidateNumberDbResponse, ValidateParagraphDbResponse, ValidateRangeDbResponse, ValidateRankingDbResponse, ValidateSelectionDbResponse, ValidateShortAnswerDbResponse, ValidateTimeDbResponse, ParsedAudioSurveyResponse, ParsedCheckboxSurveyResponse, ParsedVideoSurveyResponse, ParsedTimeSurveyResponse, ParsedShortBlogSurveyResponse, ParsedShortAnswerSurveyResponse, ParsedSelectionSurveyResponse, ParsedRankingSurveyResponse, ParsedRangeSurveyResponse, ParsedParagraphSurveyResponse, ParsedNumberSurveyResponse, ParsedMultipleChoiceSurveyResponse, ParsedMonthSurveyResponse, ParsedImageSurveyResponse, ParsedDatetimeSurveyResponse, ParsedDateSurveyResponse, ParsedColorSurveyResponse } from './surveyTypes';
import { formatDateFromUnixTime, getDateTimeStringFromUnix, getUnixTime } from '../../utils/time';
import escapeHTML from 'escape-html';
import { getAudioInformationLexical, getImagesInformationLexical, getVideoInformationLexical, renderAudioHTML, renderVideoHTML } from '../../aws/mediaHelpers';
import { getImageFromUrl, getImageInformation, handleImageUpload150_75 } from '../../utils/image';
...
import { EJS_LIKED_TEMPLATE, EJS_NOT_LIKED_TEMPLATE, GET_INSERT_INTO_MODERATION_TABLE, LIKED_ERROR_SPAN } from './article_helpers/handleArticleData';
...
import { VALID_HEX_REGEX } from '../../lexical/lexicalLexical';
import { formatNumberAttribute } from '../../utils/number';
import { uploadPageImageFromUrl } from './projects';
import {Decimal} from 'decimal.js';
const DECIMAL_ZERO = new Decimal(0);

const SURVEY_PREVIEW = fs.readFileSync(path.resolve(BASE_PATH,'views','partials','survey-preview.ejs'),{encoding:'utf-8'});
...
async function handleQuestionStateOptionTypePost(option:SurveyParagraphOptionType|AudioOptionType|VideoOptionType|ImageOptionType|SurveyParagraphRankingOptionType|AudioRankingOptionType|VideoRankingOptionType|ImageRankingOptionType,i:number,ranking:boolean=false) {
  const ret:{valid:boolean,error:string,question:number,questionState:Partial<ParsedSurveyParagraphOptionType|ParsedAudioOptionType|ParsedVideoOptionType|ParsedImageOptionType|ParsedSurveyParagraphRankingOptionType|ParsedAudioRankingOptionType|ParsedVideoRankingOptionType|ParsedImageRankingOptionType>} = { valid: true, error: '', question: i, questionState: {} }
  if (!!!ranking) {
    if (typeof (option as any).defaultChecked !== 'boolean') {
      ret.valid = false;
  ...
  switch (option.type) {
    case 'audio': {
      ...
      const audioInformation = await getAudioInformationLexical([src]);
      ...
      if (!!!ranking) {
        (ret.questionState as ParsedAudioOptionType).defaultChecked = (option as any).defaultChecked; 
      }
    ...
    case 'image': {
      ...
      if (longDescription.length) {
        if (caption!=='') caption+=': '.concat(longDescription)
        else caption = longDescription;
      }
      const imageInformation = await getImagesInformationLexical([{url: src, caption }]);
      ...
      if (width===0||height===0) {
        ...
        const ffmpegImageInfo = await getImageInformation(image)
      ...
      if (width!==0&&height!==0) {
        ...
        if (!!!ranking) {
          (ret.questionState as ParsedImageOptionType).defaultChecked = (option as any).defaultChecked; 
        }
      }
      return ret;
    }
    case 'video': {
      ...
      const videoInformation = await getVideoInformationLexical([src]);
      ...
      var height:number = heightClient;
      if (videoInformation[0]?.height) {
      ...
      if (!!!ranking) {
        (ret.questionState as ParsedVideoOptionType).defaultChecked = (option as any).defaultChecked; 
      }
      return ret;
    }
    case 'survey-paragraph': {
      ...
      try {
        const { editorState, innerText, desktop_html, tablet_html, mobile_html, able_to_validate, contains_nsfw, unvalidated_image_urls, unvalidated_audio_urls, unvalidated_video_urls } = await parseSurveyOptionEditor(JSON.parse(editorStateStr));
        ret.questionState = {
        ...
        if (!!!ranking) {
          (ret.questionState as ParsedSurveyParagraphOptionType).defaultChecked = (option as any).defaultChecked; 
        }
        return ret;
      } catch (e) {
        console.error(e);
...
async function validateQuestionStatesPost(questions:QuestionStateAny[]) {
  const ret_arr:ParsedQuestionStates[] = []
  for (let i=0; i < questions.length; i++) {
    const question = questions[i];
    const ret = { valid: true, error: '', question: i, questions: [] as ParsedQuestionStates[] };
    const to_push:Partial<ParsedQuestionStates> = { editorState: {}, desktop_html: '', tablet_html: '', mobile_html: '', innerText: '' }
    const questionState = JSON.parse(question.question);
    try {
      const { editorState, innerText, desktop_html, tablet_html, mobile_html, contains_nsfw, able_to_validate, unvalidated_image_urls, unvalidated_audio_urls, unvalidated_video_urls } = await parseSurveyQuestionEditor(questionState);
    ...
    switch (question.type) {
      case 'survey-audio': {
        ...
        ret_arr.push(to_push as ParsedSurveyAudioQuestionState);
        continue;
      }
      case 'survey-checkbox': {
        ...
        const optionStates = promiseArr.map((obj) => obj.questionState) as (ParsedAudioOptionType|ParsedVideoOptionType|ParsedImageOptionType|ParsedSurveyParagraphOptionType)[];
        (to_push as ParsedSurveyCheckboxQuestionState).min_selected_options = Number.isInteger(min_selected_options) ? Number(min_selected_options) : null;
        (to_push as ParsedSurveyCheckboxQuestionState).max_selected_options = Number.isInteger(max_selected_options) ? Number(max_selected_options) : null;
        (to_push as ParsedSurveyCheckboxQuestionState).options = optionStates;
        ret_arr.push(to_push as ParsedSurveyCheckboxQuestionState);
        continue;
      }
      case 'survey-color': {
        ...
        (to_push as ParsedSurveyColorQuestionState).defaultColor = defaultColor;
        ret_arr.push(to_push as ParsedSurveyColorQuestionState);
        continue;
      }
      case 'survey-date': {
        ...
        (to_push as ParsedSurveyDateQuestionState).minDate = minDate;
        (to_push as ParsedSurveyDateQuestionState).maxDate = maxDate;
        (to_push as ParsedSurveyDateQuestionState).step = step;
        ret_arr.push(to_push as ParsedSurveyDateQuestionState);
        continue;
      }
      case 'survey-datetime': {
        ...
        (to_push as ParsedSurveyDatetimeQuestionState).minDatetime = minDatetime;
        (to_push as ParsedSurveyDatetimeQuestionState).maxDatetime = maxDatetime;
        (to_push as ParsedSurveyDatetimeQuestionState).step = step;
        ret_arr.push(to_push as ParsedSurveyDatetimeQuestionState);
        continue;
      }
      case 'survey-image': {
        to_push.type = 'survey-image';
        ret_arr.push(to_push as ParsedSurveyImageQuestionState);
        continue;
      }
      case 'survey-month': {
        ...
        (to_push as ParsedSurveyMonthQuestionState).minMonth = minMonth;
        (to_push as ParsedSurveyMonthQuestionState).maxMonth = maxMonth;
        (to_push as ParsedSurveyMonthQuestionState).step = step;
        ret_arr.push(to_push as ParsedSurveyMonthQuestionState);
        continue;
      }
      case 'survey-multiple-choice': {
        ...
        const optionStates = promiseArr.map((obj) => obj.questionState) as (ParsedAudioOptionType|ParsedVideoOptionType|ParsedImageOptionType|ParsedSurveyParagraphOptionType)[];
        (to_push as ParsedSurveyMultipleChoiceQuestionState).options = optionStates;
        ret_arr.push(to_push as ParsedSurveyMultipleChoiceQuestionState);
        continue;
      }
      case 'survey-number': {
        ...
        (to_push as ParsedSurveyNumberQuestionState).min = min;
        (to_push as ParsedSurveyNumberQuestionState).max = max;
        (to_push as ParsedSurveyNumberQuestionState).step = step;
        ret_arr.push(to_push as ParsedSurveyNumberQuestionState);
        continue;
      }
      // Check
      case 'survey-paragraph': {
        ...
        (to_push as ParsedSurveyParagraphQuestionState).minLength = minLength;
        (to_push as ParsedSurveyParagraphQuestionState).maxLength = maxLength;
        ret_arr.push(to_push as ParsedSurveyParagraphQuestionState);
        continue;
      }
      // Check
      case 'survey-range': {
        ...
        (to_push as ParsedSurveyRangeQuestionState).rangeMin = rangeMin;
        (to_push as ParsedSurveyRangeQuestionState).rangeMax = rangeMax;
        (to_push as ParsedSurveyRangeQuestionState).rangeStep = rangeStep;
        ...
        try {
          const promiseArr = await Promise.all(rangePointDescriptions.map((obj) => parseSurveyRangeDescriptionEditor(JSON.parse(obj.description))));
          const arr:{point: number, editorState: any, innerText: string, desktop_html: string, mobile_html: string, tablet_html: string, contains_nsfw: boolean, able_to_validate: boolean, unvalidated_image_urls: string[], unvalidated_audio_urls: string[], unvalidated_video_urls: string[],}[] = [];
          for (let i=0;i<promiseArr.length;i++){
            const { point:pointNumber } = rangePointDescriptions[i];
            if (pointNumber < rangeMin || pointNumber > rangeMax || (!!!Decimal.mod(pointNumber - rangeMin,rangeStep).equals(DECIMAL_ZERO))) { // MOD
              ret.valid = false;
              ret.error = "Invalid range point value.";
              return ret;
            }
            const obj = promiseArr[i];
            delete (obj as any).tableOfContents;
          ...
          (to_push as ParsedSurveyRangeQuestionState).rangePointDescriptions = arr;
        } catch (e) {
          console.error(e);
          ret.valid = false;
      ...
      case 'survey-ranking': {
        ...
        const arr:(ParsedAudioRankingOptionType|ParsedVideoRankingOptionType|ParsedImageRankingOptionType|ParsedSurveyParagraphRankingOptionType)[] = [];
        for (let i=0;i<options.length;i++) {
          ...
          const obj = promiseArr[i];
          arr.push({...obj.questionState, original_order: option.original_order} as ParsedAudioRankingOptionType|ParsedVideoRankingOptionType|ParsedImageRankingOptionType|ParsedSurveyParagraphRankingOptionType);
        }
        (to_push as ParsedSurveyRankingQuestionState).options = arr;
        ret_arr.push(to_push as ParsedSurveyRankingQuestionState);
        continue;
      }
      // Check
      case 'survey-selection': {
        ...
        (to_push as ParsedSurveySelectionQuestionState).include_other = question.include_other;
        (to_push as ParsedSurveySelectionQuestionState).selectionText = selectionText;
        (to_push as ParsedSurveySelectionQuestionState).options = options;
        ret_arr.push(to_push as ParsedSurveySelectionQuestionState);
        continue;
      }
      // Check
      case 'survey-short-answer': {
        ...
        (to_push as ParsedSurveyShortAnswerQuestionState).minLength = minLength;
        (to_push as ParsedSurveyShortAnswerQuestionState).maxLength = maxLength;
        ret_arr.push(to_push as ParsedSurveyShortAnswerQuestionState);
        continue;
      }
      case 'survey-short-blog': {
        to_push.type = 'survey-short-blog';
        ret_arr.push(to_push as ParsedSurveyShortBlogQuestionState);
        continue;
      }
      // Check
      case 'survey-time': {
        ...
        (to_push as ParsedSurveyTimeQuestionState).minTime = minTime;
        (to_push as ParsedSurveyTimeQuestionState).maxTime = maxTime;
        (to_push as ParsedSurveyTimeQuestionState).step = step;
        ret_arr.push(to_push as ParsedSurveyShortAnswerQuestionState);
        continue;
      }
      case 'survey-video': {
        to_push.type = 'survey-video';
        ret_arr.push(to_push as ParsedSurveyVideoQuestionState);
        continue;
      }
      case 'survey-week': {
...
async function handleQuestionStateOptionTypeSave(option:SurveyParagraphOptionType|AudioOptionType|VideoOptionType|ImageOptionType|SurveyParagraphRankingOptionType|AudioRankingOptionType|VideoRankingOptionType|ImageRankingOptionType,i:number,ranking:boolean=false) {
  ...
  switch (option.type) {
    ...
    case 'survey-paragraph': {
      const { option:editorStateStr } = option;
      try {
        const { editorState } = await parseSurveyOptionEditor(JSON.parse(editorStateStr));
      } catch (e) {
        console.error(e);
        ret.valid = false;
        ret.error = "Video description must be a string less than 200 characters.";
 ...
 */
async function validateQuestionStatesSave(questions:QuestionStateAny[]) {
  for (let i=0; i < questions.length; i++) {
    const question = questions[i];
    const ret = { valid: true, error: '', question: i };
    const questionState = JSON.parse(question.question);
    try {
      await parseSurveyQuestionEditor(questionState);
    } catch (e) {
      console.error(e);
      ret.valid = false;
      ret.error = "Invalid question lexical state.";
...
function renderSurveyPreviews(rows:GetSurveyPreviewRow[],req:Request) {
  return rows.map((obj) => {
    ...
    const like_count = formatNumberAttribute(String(like_count_str));
    const view_count = formatNumberAttribute(String(view_cont_str));
    const annotation_count = formatNumberAttribute(String(annotation_count_str));
    const response_count = formatNumberAttribute(String(response_count_str));
    ...
    const date_created_datetime = getDateTimeStringFromUnix(Number(obj.date_created));
    const tz = req.session.settings?.timezone||'America/New_York';
    const date_created_string = formatDateFromUnixTime('MM/DD/YYYY',tz,Number(obj.date_created));
    const deadline_datetime = getDateTimeStringFromUnix(Number(obj.deadline));
    const deadline_string = formatDateFromUnixTime('MM/DD/YYYY, h:m:s A',tz,Number(obj.deadline));
    return ejs.render(SURVEY_PREVIEW,{id,title,image,description_string,question_count,date_created_datetime,date_created_string,deadline_string,deadline_datetime,results:false,like_count,view_count,annotation_count,response_count});
  }).join('');
}
export async function getSurveys(req:Request,res:Response) {
  try {
    ...
    const title = "Surveys";
    const breadcrumbs: Breadcrumbs = [{ name: 'Home', item: '/', position: 1 }, { name: 'Surveys', item: '/surveys', position: 2 }];
    const keywords: string[] = ["Frank McKee Brown", "frankmbrown.net", "Frank Brown","Surveys", "Surveys", "Login Functionality has not yet been implemented", "Blog", "Notes"];
    const description = '';
    ...
    const SEO: SEO = {breadcrumbs, keywords, title, description, tableOfContents, path: '/surveys'};
    const requiredVariables = getRequiredVariables(req,SEO);
    var open_surveys = getWarningAlert("There are no open suveys to show.",undefined,false); 
    const survey_query = await getDatabase().query(`WITH surveys_question_count AS (
  ...
  SELECT count(id) as response_count, survey_id FROM survey_responses GROUP BY survey_id
    ...
    const rows = survey_query.rows as GetSurveyPreviewRow[];
    if (rows.length) {
      open_surveys = renderSurveyPreviews(rows,req);
    } 
    const closed_surveys = getWarningAlert("There are no closed suveys to show.",undefined,false); 
    ...
    console.error(e);
    return redirect(req,res,'/',400,{ severity: 'error', message: 'Something went wrong getting the surveys!' });
  }
}
const getDeviceCacheKey = (req:Request,id:number) => {
  if (req.session.device?.desktop) return `frankmbrown-survey-${id}-desktop`;
  else if (req.session.device?.tablet) return `frankmbrown-survey-${id}-tablet`;
  else return `frankmbrown-survey-${id}-mobile`;
}
const getDeviceHTML = (req:Request) => {
  if (req.session.device?.desktop) return `desktop_html`;
  else if (req.session.device?.tablet) return `tablet_html`;
  else return `mobile_html`;
}
const timeToNumber = (s:string)=> {
...
const getMultipleChoiceCheckboxRankingQuery = (type_html:ReturnType<typeof getDeviceHTML>,typ:'multiple-choice'|'checkbox'|'ranking') => {
  var table;
...
const getRenderQuestionTopHTML = (question: ParsedSurveyQuestion,results:boolean) => {
  const question_wrapper_id = 'qw_'.concat(crypto.randomUUID());
  return `<section class="mt-3 survey-question" id="question-${escapeHTML(question.question_number.toString())}" data-question-id="${escapeHTML(question.id.toString())}" data-question-type="${escapeHTML(question.question_type)}"> 
  <div class="flex-row justify-between align-center navbar-sticky-transition" style="background-color: var(--background); opacity: 1; position: sticky; top: var(--navbar-height); z-index: 3;"><h2 class="h3 bold"><a class="same-page bold" href="#question-${escapeHTML(question.question_number.toString())}">Question ${escapeHTML(question.question_number.toString())}</a></h2><button type="button" class="small icon-text text warning" data-hide-show="" data-el="#${question_wrapper_id}">
...
const getRenderQuestionBottomToHTML = (question:ParsedSurveyQuestion) => {
  var str = `</div>`;
...
function renderSurveyQuestions(req:Request,questions:ParsedSurveyQuestion[],results:boolean) {
  var survey_body = '';
  for (let i=0;i<questions.length;i++) {
    const question = questions[i];
    ...
    switch (question.question_type) {
      ...
      case 'survey-checkbox': {
        const arr = question.questionState as any as SurveyCheckboxResponse[];
        ...
        for (let obj of arr) {
          ...
          switch (obj.option_type) {
            case 'image': {
              survey_body+=`<img src="${obj.image_src}" alt="${escapeHTML(obj.short_description)}" data-text="${escapeHTML(obj.long_description)}" style="max-height: 200px; width: auto; height: auto; max-width: 100%; margin: 0px !important;"/>`
              break;
            }
            case 'survey-paragraph': {
            ...
            case 'audio': {
              survey_body+=renderAudioHTML(obj.audio_src.replace('https://audio.storething.org/',''),obj.audio_title,Boolean(req.session.device?.desktop));
              break;
            }
            case 'video': {
              survey_body+=renderVideoHTML(obj.video_src.replace('https://video.storething.org/',''),obj.video_title,obj.video_description,Boolean(req.session.device?.desktop),Math.min(250,obj.video_height));
              break;
            }
            default: {
      ...
      case 'survey-color': {
        const uuid = crypto.randomUUID();
        const arr = question.questionState as any as SurveyColorDbResponse[];
        const questionState = arr[0] as SurveyColorDbResponse;
      ...
      case 'survey-date': {
        const uuid = crypto.randomUUID();
        const arr = question.questionState as any as SurveyDateDbResponse[];
        const questionState = arr[0] as SurveyDateDbResponse;
        survey_body+=`<div class="input-group block p-md" data-hover="false"><label for="date_resp_${uuid}">Input Date:</label><div class="flex-row align-center gap-2 mt-1"><button data-launch-datetime="" type="button" class="icon medium" aria-label="Launch Date Input Menu"><svg focusable="false" inert="" viewBox="0 0 24 24" tabindex="-1" title="DateRange"><path d="M9 11H7v2h2v-2zm4 0h-2v2h2v-2zm4 0h-2v2h2v-2zm2-7h-1V2h-2v2H8V2H6v2H5c-1.11 0-1.99.9-1.99 2L3 20c0 1.1.89 2 2 2h14c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2zm0 16H5V9h14v11z"></path></svg></button><input type="date" id="date_resp_${uuid}" name="date_resp_${uuid}" step="${questionState.step}" placeholder="yyyy-mm-dd" ${questionState.min_date?`min="${escapeHTML(questionState.min_date)}"`:``} ${questionState.max_date?`max="${escapeHTML(questionState.max_date)}"`:``}></div>${questionState.min_date?`<p class="block body2 mt-1"><span class="bold">Min Date: </span>${escapeHTML(questionState.min_date)}</p>`:``}${questionState.max_date?`<p class="block body2 mt-1"><span class="bold">Max Date: </span>${escapeHTML(questionState.max_date)}</p>`:``}</div>`
        break;
      }
      case 'survey-datetime': {
        const uuid = crypto.randomUUID();
        const arr = question.questionState as any as SurveyDatetimeDbResponse[];
        const questionState = arr[0] as SurveyDatetimeDbResponse;
      ...
      case 'survey-month': {
        const uuid = crypto.randomUUID();
        const arr = question.questionState as any as SurveyMonthDbResponse[];
        const questionState = arr[0] as SurveyMonthDbResponse;
      ...
      case 'survey-multiple-choice': {
        const arr = question.questionState as any as SurveyMultipleChoiceDbResponse[];
      ...
      case 'survey-number': {
        const uuid = crypto.randomUUID();
        const arr = question.questionState as any as SurveyNumberDbResponse[];
        const questionState = arr[0] as SurveyNumberDbResponse;
      ...
      case 'survey-paragraph': {
        const uuid = crypto.randomUUID();
        const arr = question.questionState as any as SurveyParagraphDbResponse[];
        const questionState = arr[0] as SurveyParagraphDbResponse;
      ...
      case 'survey-range': {
        const uuid = crypto.randomUUID();
        const arr = question.questionState as any as SurveyRangeDbResponse[];
        ...
        for (let obj of arr) {
          if(Number.isInteger(obj.point)&&obj.html&&obj.lexical_uuid) {
            ...
            var placement = 'top';
            if (Number(obj.point) < one_third) placement='top-start';
            else if (Number(obj.point) > two_third) placement = 'top-end';
            const temp_uuid = crypto.randomUUID();
      ...
      case 'survey-ranking': {
        const uuid = crypto.randomUUID();
        const arr = question.questionState as any as SurveyRankingDbResponse[];
      ...
      case 'survey-selection': {
        const uuid = crypto.randomUUID();
        const arr = question.questionState as any as SurveySelectionDbResponse[];
      ...
      case 'survey-short-answer': {
        const uuid = crypto.randomUUID();
        const arr = question.questionState as any as SurveyShortAnswerDbResponse[];
        const questionState = arr[0] as SurveyShortAnswerDbResponse;
...
const getSurveyLikeViewQuery = (logged_in:boolean) => {
...
export async function getSurvey(req:Request,res:Response) {
  try {
    ...
    if (!!!Number.isInteger(parseInt(id))) {
      return redirect(req,res,'/surveys',400,{ severity: 'error', message: 'Something went wrong getting the survey!' });
    }
    ...
    var description_id = '';
    var date_created = 0;
    var breadcrumbs: Breadcrumbs = [{ name: 'Home', item: '/', position: 1 }, { name: 'Surveys', item: '/surveys', position: 2 }];
    ...
    const survey_like_view = await getDatabase().query(survey_like_view_query,[parseInt(id),user_id,req.session.ip_id,getUnixTime()]);
    const survey_like_view_row = survey_like_view.rows[0];
    has_liked = survey_like_view_row?.user_liked || false;
    like_count = survey_like_view_row?.likes || 0;
    view_count = survey_like_view_row?.views || 0;
    ...
    try {
      ...
      const survey = surveyQuery.rows[0] as SurveyResponseRow;
      ...
      deadline_datetime = getDateTimeStringFromUnix(deadline,true);
      deadline_string = formatDateFromUnixTime('MM/DD/YYYY, h:m:s A',tz,deadline);
      ...
      const questions = survey_questions.rows as SurveyQuestionResponseRow[];
      ...
      const parsed_survey_questions:ParsedSurveyQuestion[] = [];
      for (let i=0;i<questions.length;i++) {
        const newQuestion:Partial<ParsedSurveyQuestion> = questions[i];
        newQuestion.questionState = rows[i];
        parsed_survey_questions.push(newQuestion as ParsedSurveyQuestion);
      }
      ...
      if (!!!IS_DEVELOPMENT) await req.cache.set(device_cache_key,JSON.stringify({ title, description: description_html, description_text: survey.description_text, image, survey_uuid, survey_body, table_of_contents, description_id }));
    } catch (e) {
      console.error(e);
      try {
    ...
    const requiredVariables = getRequiredVariables(req,SEO,{ includeAnnotation: true, includeAnnotationFormOnly: true });
    const date_created_datetime = getDateTimeStringFromUnix(date_created,true);
    const date_created_string = formatDateFromUnixTime('MM/DD/YYYY',tz,date_created);
    return res.status(200).render(view,{
      layout: requiredVariables.fullPageRequest ? 'layout' : false,
      ...requiredVariables,
      description_html,
...
export async function postSurvey(req:Request,res:Response) {
  try { 
    ...
    const unvalidated_audio_urls:Set<string> = new Set();
    const unvalidated_video_urls:Set<string> = new Set();
    const unvalidated_image_urls:Set<string> = new Set();
    var able_to_validate:boolean = true;
    var contains_nsfw:boolean = false;
    const descriptionLexicalState = JSON.parse(descriptionStringified);
    const { editorState:descriptionState, innerText:description_text, desktop_html, mobile_html, tablet_html, tableOfContents, unvalidated_audio_urls:unvalidated_desc_audio_urls, unvalidated_video_urls:unvalidated_desc_video_urls, unvalidated_image_urls:unvalidated_desc_image_urls, able_to_validate:able_to_validate_description, contains_nsfw:contains_nsfw_description  } = await parseFullLexicalEditor(descriptionLexicalState);
    addItemsToSet(unvalidated_desc_audio_urls,unvalidated_audio_urls);
    addItemsToSet(unvalidated_desc_image_urls,unvalidated_image_urls);
    addItemsToSet(unvalidated_desc_video_urls,unvalidated_video_urls);
    ...
    const AT_LEAST_ONE_REQUIRED = original_questions.filter((ques:any) => ques.required).length>=1;
    if (!!!AT_LEAST_ONE_REQUIRED) {
      return res.status(400).json({error: "At least one survey question must be required."});
    }
    const [obj,image_new] = await Promise.all([validateQuestionStatesPost(original_questions),handleImageUpload150_75(image)]);
    await uploadPageImageFromUrl(req,image_new);
    if (!!!obj.valid) {
    ...
    try {
      ...
      const [descriptionEmbeddings,descriptionModeration] = await Promise.all([
        Promise.all(descriptionChunks.map((s) => createEmbedding(s,'text-embedding-ada-002'))),
        Promise.all(descriptionChunks.map((s) => moderateOpenAI(s)))
      ]);
      ...
      const question_moderation_query = GET_INSERT_INTO_MODERATION_TABLE('survey_description_moderation','survey_id');
      ...
      for (let i=0;i<descriptionChunks.length;i++) {
        ...
        db_connection.query(`INSERT INTO survey_search (document,embedding,survey_id) VALUES ($1,$2,$3);`,[s,toSql(descriptionEmbeddings[i]),survey_id]);
      }
      const embed_question_arr:string[][] = [];
      const promiseArrQuestions:Promise<any>[] = [];
      for (let i=0; i < questions.length; i++) {
        const question = questions[i];
        var arr:any[] = [];
        var curr_index = 0;
  ...
  INSERT INTO survey_questions (survey_id,question_number,required,question_type,editor_state,desktop_html,tablet_html,mobile_html) VALUES ($1,$2,$3,$4,$5,$6,$7,$8) RETURNING id, question_type
        ...
        const { 
          ...
          innerText:questionInnerText, 
          desktop_html:question_desktop_html,
          tablet_html:question_tablet_html,
          mobile_html:question_mobile_html,
          type:questionType,
          unvalidated_audio_urls:question_unvalidated_audio_urls,
          unvalidated_image_urls:question_unvalidated_image_urls,
          unvalidated_video_urls:question_unvalidated_video_urls,
          able_to_validate:question_able_to_validate,
          contains_nsfw:question_contains_nsfw,
          required
        } = question;  
        addItemsToSet(question_unvalidated_audio_urls,unvalidated_audio_urls);
        addItemsToSet(question_unvalidated_image_urls,unvalidated_image_urls);
        addItemsToSet(question_unvalidated_video_urls,unvalidated_video_urls);
  ...
  SELECT true AS column_name
          ...
          case 'survey-checkbox': {
            const obj = question as ParsedSurveyCheckboxQuestionState;
  ...
  SELECT id FROM ${query_str_3}
              ...
              switch (optionType) {
                case 'audio': {
                  const option_obj = option as ParsedAudioOptionType;
  ...
  INSERT INTO audio_option_type (survey_checkbox_option_id,src,title) SELECT id, $${curr_index}, $${curr_index+1} FROM ${query_str} 
                ...
                case 'image': {
                  const option_obj = option as ParsedImageOptionType;
query+=`, ${query_str_2} AS (
  INSERT INTO image_option_type (survey_checkbox_option_id,src,short_description,long_description,width,height) SELECT id, $${curr_index}, $${curr_index+1}, $${curr_index+2}, $${curr_index+3}, $${curr_index+4} FROM ${query_str} 
                ...
                case 'survey-paragraph': {
                  const option_obj = option as ParsedSurveyParagraphOptionType;
query+=`, ${query_str_2} AS (
  INSERT INTO survey_paragraph_option_type (survey_checkbox_option_id,editor_state,desktop_html,tablet_html,mobile_html) SELECT id, $${curr_index}, $${curr_index+1}, $${curr_index+2}, $${curr_index+3} FROM ${query_str} 
                  ...
                  addItemsToSet(option_obj.unvalidated_audio_urls,unvalidated_audio_urls);
                  addItemsToSet(option_obj.unvalidated_image_urls,unvalidated_image_urls);
                  addItemsToSet(option_obj.unvalidated_video_urls,unvalidated_video_urls);
                  curr_index+=4;
                  options_str += `${option_obj.innerText}\n`
                  break;
                }
                case 'video': {
                  const option_obj = option as ParsedVideoOptionType;
query+=`, ${query_str_2} AS (
  INSERT INTO video_option_type (survey_checkbox_option_id,src,title,description,height) SELECT id, $${curr_index}, $${curr_index+1}, $${curr_index+2}, $${curr_index+3} FROM ${query_str} 
          ...
          case 'survey-color': {
            const obj = question as ParsedSurveyColorQuestionState;
            query+=`, second_query AS ( 
  INSERT INTO survey_color_info (survey_question_id,default_color,version) SELECT id, $${curr_index},$${curr_index+1} FROM question_cte 
          ...
          case 'survey-date': {
            const obj = question as ParsedSurveyDateQuestionState;
            query+=`, second_query AS ( 
  INSERT INTO survey_date_info (survey_question_id,min_date,max_date,step,version) SELECT id, $${curr_index},$${curr_index+1},$${curr_index+2},$${curr_index+3} FROM question_cte 
          ...
          case 'survey-datetime': {
            const obj = question as ParsedSurveyDatetimeQuestionState;
            query+=`, second_query AS ( 
  ...
  SELECT true AS column_name
          ...
          case 'survey-month': {
            const obj = question as ParsedSurveyMonthQuestionState;
            query+=`, second_query AS (
 INSERT INTO survey_month_info (survey_question_id,min_month,max_month,step,version) SELECT id, $${curr_index},$${curr_index+1},$${curr_index+2},$${curr_index+3} FROM question_cte 
          ...
          case 'survey-multiple-choice': {
            const obj = question as ParsedSurveyMultipleChoiceQuestionState;
            const { options, VERSION } = obj;
            var options_str = '';
            for (let j = 0; j < options.length; j++) {
              const option = options[j];
...
query+=`, ${query_str_2} AS (
  INSERT INTO video_option_type (survey_multiple_choice_option_id,src,title,description,height) SELECT id, $${curr_index}, $${curr_index+1}, $${curr_index+2}, $${curr_index+3} FROM ${query_str} 
          ...
          case 'survey-number': {
            const obj = question as ParsedSurveyNumberQuestionState;
            query+=`, second_query AS (
 INSERT INTO survey_number_info (survey_question_id,min_number,max_number,step,version) SELECT id, $${curr_index},$${curr_index+1},$${curr_index+2},$${curr_index+3} FROM question_cte 
          ...
          case 'survey-paragraph': {
            const obj = question as ParsedSurveyParagraphQuestionState;
            query+=`, second_query AS ( 
  INSERT INTO survey_paragraph_info (survey_question_id,min_length,max_length,version) SELECT id, $${curr_index},$${curr_index+1},$${curr_index+2} FROM question_cte
          ...
          case 'survey-range': {
            const obj = question as ParsedSurveyRangeQuestionState;
  ...
  INSERT INTO  survey_range_point_descriptions (survey_question_id,point,editor_state,desktop_html,tablet_html,mobile_html) SELECT id, $${curr_index}, $${curr_index+1}, $${curr_index+2}, $${curr_index+3}, $${curr_index+4} FROM question_cte
          ...
          case 'survey-ranking': {
            const obj = question as ParsedSurveyRankingQuestionState;
            const { options, VERSION } = obj;
            var options_str = '';
            for (let j = 0; j < options.length; j++) {
              const option = options[j];
  ...
  SELECT id FROM ${query_str_3}
              ...
              switch (optionType) {
                case 'audio': {
                  const option_obj = option as ParsedAudioRankingOptionType;
                  query+=`, ${query_str_2} AS (
  INSERT INTO audio_option_type (survey_ranking_option_id,src,title) SELECT id, $${curr_index}, $${curr_index+1} FROM ${query_str} 
                ...
                case 'image': {
                  const option_obj = option as ParsedImageRankingOptionType;
query+=`, ${query_str_2} AS (
  INSERT INTO image_option_type (survey_ranking_option_id,src,short_description,long_description,width,height) SELECT id, $${curr_index}, $${curr_index+1}, $${curr_index+2}, $${curr_index+3}, $${curr_index+4} FROM ${query_str} 
                ...
                case 'survey-paragraph': {
                  const option_obj = option as ParsedSurveyParagraphRankingOptionType;
query+=`, ${query_str_2} AS (
  INSERT INTO survey_paragraph_option_type (survey_ranking_option_id,editor_state,desktop_html,tablet_html,mobile_html) SELECT id, $${curr_index}, $${curr_index+1}, $${curr_index+2}, $${curr_index+3} FROM ${query_str} 
                ...
                case 'video': {
                  const option_obj = option as ParsedVideoRankingOptionType;
query+=`, ${query_str_2} AS (
  INSERT INTO video_option_type (survey_ranking_option_id,src,title,description,height) SELECT id, $${curr_index}, $${curr_index+1}, $${curr_index+2}, $${curr_index+3} FROM ${query_str} 
          ...
          case 'survey-selection': {
            const obj = question as ParsedSurveySelectionQuestionState;
  ...
  INSERT INTO survey_selection_options (survey_question_id,option,selected,index) SELECT id, $${curr_index}, $${curr_index+1}, $${curr_index+2} FROM question_cte
          ...
          case 'survey-short-answer': {
            const obj = question as ParsedSurveyShortAnswerQuestionState;
            query += `, second_query AS ( 
  ...
  SELECT true AS column_name
          ...
          case 'survey-time': {
            const obj = question as ParsedSurveyTimeQuestionState;
            query += `, second_query AS (
  ...
  SELECT true AS column_name
      ...
      const [question_ids_and_types,promiseEmbedQuestions,promiseModerateQuestions] = await Promise.all([
        ...
        Promise.all(embed_question_arr.map((arr) => Promise.all(arr.map((s) => createEmbedding(s,'text-embedding-ada-002'))))),
        Promise.all(embed_question_arr.map((arr) => Promise.all(arr.map((s) => moderateOpenAI(s)))))
      ]);
      const moderation_query = GET_INSERT_INTO_MODERATION_TABLE('survey_question_moderation','survey_question_id');
      
      for (let i = 0; i < question_ids_and_types.length; i++) {
        const question_id = question_ids_and_types[i]?.rows?.[0].id;
        ...
        const moderationParsedQuestion:ParsedModerationResp[] = [];
        for (let moder of moderationArr) {
          moderationParsedQuestion.push(parseOpenAIModerationResponse(moder));
        }
        for (let j = 0; j < toEmbedArray.length; j++) {
          const document = textChunks[j];
          db_connection.query(`INSERT INTO survey_question_search (survey_question_id,document,embedding) VALUES ($1,$2,$3);`,[question_id,document,toSql(toEmbedArray[j])]);
        }
      ...
      await db_connection.query('COMMIT;');
      db_connection.release();
      return res.status(200).json({ error: getWarningSnackbar("I haven't implemented this yet.") });
    } catch (e) {
      console.error(e)
      try {
        await db_connection.query('ROLLBACK;');
      ...
      db_connection.release();
      return res.status(400).json({ error: getWarningSnackbar("I haven't implemented this yet.") });
    }

  } catch (e) {
    console.error(e);
    return res.status(400).json({error:getErrorSnackbar("Something went wrong posting the survey!")});
  }
}
async function validateSurveyResponses(survey_id:number,responses:SurveyResponse) {
  const date_responded = getUnixTime();
  const dbqsQuery = await getDatabase().query(`SELECT id, required, question_type FROM survey_questions WHERE survey_questions.survey_id=$1 ORDER BY question_number ASC;`,[survey_id]);
  const db_questions = dbqsQuery.rows as { id: number, required: boolean, question_type: ValidSurveyQuestionTypes}[];
...
WHERE survey_selection_info.survey_question_id=$1;`,[question.id]));
  ...
  const validateDbResponsesRows = validateDBResponsesQueryRes.map((obj) => obj===null?null: obj.rows[0]) as ValidateSurveyResponseDbResponse[];
  const parsed_survey_responses:ParsedSurveyResponse[] = [];
  for (let i = 0; i < responses.length; i++) {
    const parsed_survey_response:Partial<ParsedSurveyResponse> = {};
    const response = responses[i];
    const required = response.required;
    const confirm_response = response.confirmed_response;
    ...
    switch (response.question_type) {
      case 'survey-audio': {
        const { src, title } = response.response as AudioSurveyResponse;
        if (title.length>50||title.length<1) {
      ...
      case 'survey-checkbox': {
        const { choices, min_selected, max_selected } = response.response as CheckboxSurveyResponse;
        const validatedObject = validateDbResponsesRows[i] as ValidateCheckboxDbResponse;
      ...
      case 'survey-color': {
        const { color } = response.response as ColorSurveyResponse;
        if (!!!VALID_HEX_REGEX.test(color)) {
      ...
      case 'survey-date': {
        const { date } = response.response as DateSurveyResponse;
        const validatedObject = validateDbResponsesRows[i] as ValidateDateDbResponse;
        ...
        if (min_date&&isValidDateString(min_date)) {
          ...
          if (!!!Decimal.mod((d_time - min_d_time),step*86_400_000).equals(DECIMAL_ZERO)) { // MOD
        ...
        if (max_date&&isValidDateString(max_date)) {
          ...
          if (!!!valid_step) {
            if (!!!Decimal.mod((max_d.getTime()-d.getTime()),step*86_400_000).equals(DECIMAL_ZERO)) { // MOD
              throw new Error("Date does not comply with step input.");
            }
          }
        }
      ...
      case 'survey-datetime': {
        const { datetime } = response.response as DatetimeSurveyResponse;
        const validatedObject = validateDbResponsesRows[i] as ValidateDatetimeDbResponse;
        ...
        if (min_datetime&&isValidDateString(min_datetime)) {
          ...
          const min_d_time = min_d.getTime();
          const d_time = d.getTime();
          if (!!!Decimal.mod((d_time - min_d_time),step*60).equals(DECIMAL_ZERO)) { // MOD
        ...
        if (max_datetime&&isValidDateString(max_datetime)) {
          ...
          if (!!!valid_step) {
            if (!!!Decimal.mod((max_d.getTime()-d.getTime()),step*60).equals(DECIMAL_ZERO)) { // MOD
              throw new Error("Datetime does not comply with step input.");
            }
          }
        }
      ...
      case 'survey-image': {
        const { src, short_description, long_description } = response.response as ImageSurveyResponse;
        if (!!!src.startsWith('https://image.storething.org/frankmbrown')) {
      ...
      case 'survey-month': {
        const { month } = response.response as MonthSurveyResponse;
        const validatedObject = validateDbResponsesRows[i] as ValidateMonthDbResponse;
        ...
        if (min_month&&isValidDateString(min_month)) {
          ...
          if (!!!Decimal.mod(months,step).equals(DECIMAL_ZERO)) { // MOD
      ...
      case 'survey-multiple-choice': {
        const { choice } = response.response as MultipleChoiceSurveyResponse;
        const validatedObject = validateDbResponsesRows[i] as ValidateMultipleChoiceDbResponse;
      ...
      case 'survey-number': {
        const { number } = response.response as NumberSurveyResponse;
        const validatedObject = validateDbResponsesRows[i] as ValidateNumberDbResponse;
        ...
        if (typeof min_number==="number") {
          if (!!!Decimal.mod((number - min_number),step).equals(DECIMAL_ZERO)) { // MOD
            throw new Error("Number input does not comply with step requirement.");
          }
        } else if (typeof max_number==="number") {
          if (!!!Decimal.mod((max_number - number),step).equals(DECIMAL_ZERO)) { // MOD
            throw new Error("Number input does not comply with step requirement.");
          }
        }
        parsed_survey_response.response = {
      ...
      case 'survey-paragraph': {
        const { response:paragraphResponse } = response.response as ParagraphSurveyResponse;
        const validatedObject = validateDbResponsesRows[i] as ValidateParagraphDbResponse;
      ...
      case 'survey-range': {
        const { input } = response.response as RangeSurveyResponse;
        const validatedObject = validateDbResponsesRows[i] as ValidateRangeDbResponse;
        const { range_min, range_max, range_step } = validatedObject;
        if (!!!Decimal.mod((input - range_min),range_step).equals(DECIMAL_ZERO)) { // MOD
      ...
      case 'survey-ranking': {
        const { ordered } = response.response as RankingSurveyResponse;
        const validatedObject = validateDbResponsesRows[i] as ValidateRankingDbResponse;
      ...
      case 'survey-selection': {
        const { other_checked } = response.response as SelectionSurveyResponse;
        const other_option = (response.response as SelectionSurveyResponse).other_option;
        const choice = (response.response as SelectionSurveyResponse).choice;
        const validatedObject = validateDbResponsesRows[i] as ValidateSelectionDbResponse;
      ...
      case 'survey-short-answer': {
        const { response:shortAnswerResponse } = response.response as ShortAnswerSurveyResponse;
        const validatedObject = validateDbResponsesRows[i] as ValidateShortAnswerDbResponse;
        const { min_length, max_length } = validatedObject;
      ...
      case 'survey-short-blog': {
        const { editorState } = response.response as ShortBlogResponse;
        const editorStateParsed = JSON.parse(editorState);
        const { desktop_html, mobile_html, tablet_html, editorState:editor_state, innerText } = await parseSurveyShortBlogEditor(editorStateParsed);
        parsed_survey_response.response = {
          editor_state,
          desktop_html,
          mobile_html,
      ...
      case 'survey-time': {
        const { time } = response.response as TimeSurveyResponse;
        const validatedObject = validateDbResponsesRows[i] as ValidateTimeDbResponse;
        ...
        if (isValidTimeString(min_time)) {
          ...
          if (!!!Decimal.mod((time_num - min_time),step).equals(DECIMAL_ZERO)) { // MOD
        ...
        if (isValidTimeString(max_time)) {
          ...
          if (!!!valid_step) {
            if (!!!Decimal.mod((max_time - time_num),step).equals(DECIMAL_ZERO)) { // MOD
              throw new Error("Time does not comply with step value.");
            }
          }
        }
      ...
      case 'survey-video': {
        const { src, title, description, height } = response.response as VideoSurveyResponse;
        if (!!!src.startsWith('https://video.storething.org/frankmbrown')) {
    ...
    parsed_survey_responses.push(parsed_survey_response as ParsedSurveyResponse);
  }
  return parsed_survey_responses;
}
export async function postSurveyResponse(req:Request,res:Response) {
  try {
    ...
    try {
      ...
      const survey_response_query = await db_connection.query(`INSERT INTO survey_responses (ip_id, survey_id, date_responded) VALUES ($1,$2,$3) RETURNING id;`,[ip_id,parseInt(id),getUnixTime()]);
      ...
      for (let i = 0; i < parsedSurveyResponses.length; i++) {
        ...
        switch (parsedSurveyResponses[i].question_type) {
          case 'survey-audio': {
            const { src,title } = qR as ParsedAudioSurveyResponse;
            arr.push(src,title);
          ...
          case 'survey-checkbox': {
            const { choices } = qR as ParsedCheckboxSurveyResponse;
          ...
          case 'survey-color': {
            const { color } = qR as ParsedColorSurveyResponse;
            arr.push(color);
          ...
          case 'survey-date': {
            const { date } = qR as ParsedDateSurveyResponse;
            arr.push(date);
          ...
          case 'survey-datetime': {
            const { datetime } = qR as ParsedDatetimeSurveyResponse;
            arr.push(datetime);
          ...
          case 'survey-image': {
            const { src, short_description, long_description } = qR as ParsedImageSurveyResponse;
          ...
          case 'survey-month': {
            const { month } = qR as ParsedMonthSurveyResponse;
            arr.push(month);
          ...
          case 'survey-multiple-choice': {
            const { choice } = qR as ParsedMultipleChoiceSurveyResponse;
            arr.push(choice);
          ...
          case 'survey-number': {
            const { number } = qR as ParsedNumberSurveyResponse;
            arr.push(number);
          ...
          case 'survey-paragraph': {
            const { response:paragraphResponse } = qR as ParsedParagraphSurveyResponse;
          ...
          case 'survey-range': {
            const { input } = qR as ParsedRangeSurveyResponse;
            arr.push(input);
          ...
          case 'survey-ranking': {
            const { ordered } = qR as ParsedRankingSurveyResponse;
          ...
          case 'survey-selection': {
            const { choice, other_checked, other_option } = qR as ParsedSelectionSurveyResponse;
            if (choice!==null) {
              arr.push()
          ...
          case 'survey-short-answer': {
            const { response:shortAnswerResponse } = qR as ParsedShortAnswerSurveyResponse;
          ...
          case 'survey-short-blog': {
            const { editor_state, innerText, desktop_html, tablet_html, mobile_html } = qR as ParsedShortBlogSurveyResponse;
          ...
          case 'survey-time': {
            const { time } = qR as ParsedTimeSurveyResponse;
            arr.push(time);
          ...
          case 'survey-video': {
            const { src, title, description, height } = qR as ParsedVideoSurveyResponse;
    ...
    console.error(e);
    return res.status(400).json({ error: getErrorSnackbar("Unable to post survey response! Try reloading the page.")});
  }
}
export async function searchSurveys(req:Request,res:Response) {
  try {
    ...
    if (typeof surveySearch !=='string') {
      return res.status(200).send(getErrorAlert("Search term must be a string.",undefined,false));
    }
    if (surveySearch.length===0) {
      return res.status(200).send(getInfoAlert("Enter the name or description of a survey to search for in the search box above to search for saved surveys.",undefined,false));
    }
    const tz = req.session.settings?.timezone||'America/New_York';
    const user_id = req.session.auth?.userID;
    const rows = (await getDatabase().query(`WITH query AS ( SELECT websearch_to_tsquery('english',$1) term ) SELECT id, title, description_text, cardinality(questions) questions_length, date_created FROM saved_surveys WHERE (SELECT term FROM query) @@ search AND ${Number.isInteger(user_id)?`user_id`:`ip_id`}=$2 ORDER BY ts_rank(search,(SELECT term FROM query)) DESC, date_created DESC LIMIT 20;`,[surveySearch,user_id?user_id:req.session.ip_id])).rows;
    if (!!!rows.length) {
      return res.status(200).send(getWarningAlert("No search results found.",undefined,false));
    }
    const html = rows.map((obj) => {
      const num_questions = obj.questions_length;
      const date_created_str = formatDateFromUnixTime('MMM D, YYYY',tz,Number(obj.date_created)); 
      const datetime_str = getDateTimeStringFromUnix(Number(obj.date_created),true);
...
<label class="radio grow-0 large"><input type="radio" name="download-survey-radio" class="secondary" value="${escape(obj.id)}"><span hidden=""></span></label>
<div class="grow-1">
  <p class="h6 bold block">${escape(obj.title)}</p>
  <p class="body1 mt-1">${escape(obj?.description_text?.slice(0,150).concat('...'))}</p>
  <div class="flex-row align-center justify-between">
      ...
      <span>${escapeHTML(date_created_str)}</span>
    </time>
  </div>
</div>
</div>`
    ...
    console.error(e);
    return res.status(200).send(getErrorAlert("Something went wrong searching for surveys.",undefined,false));
  }
}

export async function postSaveSurvey(req:Request,res:Response) {
  try {
    ...
    const descriptionLexicalState = JSON.parse(descriptionStringified);
    const { editorState:description, innerText:description_text } = await parseFullLexicalEditor(descriptionLexicalState);
    if (!!!image.startsWith('https://image.storething.org/frankmbrown/')) {
      return res.status(400).json({error:"Invalid survey image."});
    }
    if (!!!isValidDateString(deadline)) {
...
export async function downloadSurvey(req:Request,res:Response) {
  try {
    const { id } = req.params;
    if (!!!Number.isInteger(parseInt(id))) {
    ...
    const user_id = req.session.auth?.userID || null;
    const ip_id = req.session.ip_id;
    const surveyRows = (await getDatabase().query(`SELECT title, description, image, questions, survey_uuid, deadline FROM saved_surveys WHERE id=$1 AND ${user_id?`user_id`:`ip_id`}=$2`,[parseInt(id),user_id?user_id:ip_id])).rows;
...
export async function searchSurveysAndQuestions(req:Request,res:Response) {
  try {
    ...
    if (typeof search_input !== 'string' || !!!search_input.length) {
      return res.status(200).send(getErrorAlert("Search Input must be at least 1 character long.",undefined,false));
    }
    if (survey_search_type!=='default'&&survey_search_type!=='semantic' && survey_search_type!=='by-title') {
      return res.status(200).send(getErrorAlert("Search Survey Type must be either 'default' or 'semantic'.",undefined,false));
    }
  ...
  SELECT count(id) as response_count, survey_id FROM survey_responses GROUP BY survey_id
      ...
      const dbResp = await getDatabase().query(query,[search_input]);
      const rows = dbResp.rows;
      if (!!!rows.length) {
        return res.status(200).send(getWarningAlert("No survey titles match the search term.",undefined,false));
      } else {
        return res.status(200).send(renderSurveyPreviews(rows,req));
      }
    } else if (survey_search_type==="semantic") {
      const embedding = await createEmbedding(search_input,'text-embedding-ada-002');
      const query = `WITH surveys_question_count AS (
  SELECT survey_id, COUNT(id) AS question_count FROM survey_questions GROUP BY survey_questions.survey_id 
), survey_like_count AS (
  SELECT count(like_id) like_count, survey_id FROM article_likes WHERE survey_id IS NOT NULL GROUP BY survey_id
  ...
  SELECT count(id) as response_count, survey_id FROM survey_responses GROUP BY survey_id
    ...
    const dbResp = await getDatabase().query(query,[toSql(embedding)]);
    const rows = dbResp.rows;
    if (!!!rows.length) {
      return res.status(200).send(getWarningAlert("No Results were found for the provided query.",undefined,false));
    } else {
      return res.status(200).send(renderSurveyPreviews(rows,req));
    }
    } else {
  ...
  SELECT count(id) as response_count, survey_id FROM survey_responses GROUP BY survey_id
    ...
    console.error(e);
    return res.status(200).send(getErrorAlert("Something went wrong searching surveys and questions.",undefined,false));
  }
}