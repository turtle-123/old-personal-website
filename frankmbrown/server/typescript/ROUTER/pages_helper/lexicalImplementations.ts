...
export function getArticleBuilderImplementation(req:Request,id:string="article",innerHTML:string='<p>Insert article content here...</p>',obj:{minHeight?: number, wrapperClass: string}={minHeight: 600, wrapperClass: 'block lexical-wrapper mt-3' }) {
  const desktop = Boolean(req.session.device?.desktop===true);
...
<div class="${obj.wrapperClass}" id="${id}" data-current="true" data-save-state>
  ...
  <div contenteditable="true" data-rich-text-editor="" data-type="full" data-editable="true" spellcheck="true" style="min-height: ${obj?.minHeight||600}px; user-select: text; white-space: pre-wrap; word-break: break-word;" data-lexical-editor="true">${innerHTML}${END_PADDING}</div>
</div>`;
}
...
export function getCommentImplementation(req:Request,id:string="comment",disabled:boolean=false,initialHTML="<p>Insert comment here...</p>",options:CommentOptions={ wrapperStyle: 'block lexical-wrapper mt-3', type:'comment', rteStyle: 'max-height: 400px; min-height: 150px; overflow-y:auto;'}) {
...
export function getTextOnlyImplementation(req:Request,id:string='text-implementation',noColor:boolean=false) {
  const obj = getFullLexicalRenderObj();
  (obj as any).initialHTML = '<p>Insert comment here...</p>';
  (obj as any).settingsMode = req.session?.settings?.mode==="light" ? "lightMode":"darkMode";
  (obj as any).settings = req.session.settings;
...
<div data-click-capture class="floating-menu o-xs" style="min-width: 150px;" data-placement="bottom-end"
  ...
  return ejs.render(/*html*/`<div class="block lexical-wrapper mt-3" id="${id}" data-current="false">
  <div role="toolbar" class="toggle-button-group hz-scroll">
...
export function getTextWithLinksImplementation(req:Request,id:string="text-links-implementation",noColor:boolean=false) {
  const obj = getFullLexicalRenderObj();
  (obj as any).initialHTML = '<p>Insert comment here...</p>';
  (obj as any).settingsMode = req.session?.settings?.mode==="light" ? "lightMode":"darkMode";
  (obj as any).settings = req.session.settings;
...
export function getTextBlockTextImplementation(req:Request,id:string="text-blocks-implementation",noColor:boolean=false,max_height=400) {
  const obj = getFullLexicalRenderObj();
  (obj as any).initialHTML = '<p>Insert comment here...</p>';
  (obj as any).settingsMode = req.session?.settings?.mode==="light" ? "lightMode":"darkMode";
  (obj as any).settings = req.session.settings;
