...
import { parseMarkdown, purifyAndUpdateJupyterNotebookMarkdownHTML } from './markdown';



export async function markdownApi(req:Request,res:Response) {
  try {
    ...
    if (typeof markdownStrPre!=='string' || markdownStrPre.length<10) return res.status(200).send(getErrorAlert('Markdown Input must be at least 10 characters long.'));
    const markdownRespPre = await parseMarkdown(markdownStrPre,{ design_system, breaks, gfm, pedantic });
    const markdownResp = await purifyAndUpdateJupyterNotebookMarkdownHTML(markdownRespPre);
    const resp = /*html*/`
...
`;
    ...
    console.error(error);
    return res.status(200).send(getErrorAlert('Unable to generate HTML for the markdown provided.'));
  }
}
