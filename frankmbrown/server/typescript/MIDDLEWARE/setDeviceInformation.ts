...
import { isMobileDevice, isMobileOrTablet } from '../utils/deviceDetection';
import env from '../utils/env';

export type TempDeviceObject = {
  phone: boolean,
  tablet: boolean,
  desktop: boolean,
 ...
 * @param next 
 */
function setDeviceInformation(req: Request ,res: Response,next: NextFunction) {
  if (!!!req.session.device) {
    const userAgent = req.get('user-agent');
    const ua = new uap(req.get('user-agent'));
    ...
    if (env.PHONE_ONLY==="true") {
      ...
      if (!Boolean(typeof userAgent === 'string')) {
          ...
          phone = isMobileDevice(userAgent as string);
          touchEnabled = !!!phone ? isMobileOrTablet(userAgent as string) : true;
      }
    }
...
export default setDeviceInformation;