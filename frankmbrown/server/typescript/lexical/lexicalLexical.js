 ...
 */
import { ElementNodeWithBlockStyle as ElementNodeWithBlockStyle$1, getCssTextForElementNodeWithBlockStyle as getCssTextForElementNodeWithBlockStyle$1, $isRootNode as $isRootNode$1, extractStylesFromCssText as extractStylesFromCssText$1, ElementNode as ElementNode$1, COMMAND_PRIORITY_CRITICAL as COMMAND_PRIORITY_CRITICAL$1, isHTMLElement as isHTMLElement$1, $applyNodeReplacement as $applyNodeReplacement$1, $createParagraphNode as $createParagraphNode$1, TextNode as TextNode$1, $isTextNode as $isTextNode$1 } from './lexicalLexical.js';
...
class LexicalNode {
  ...
  /** @internal */
  /** @internal */
  __first_element = false;
  __last_element = false;
  ...
  return {
    forChild: lexicalNode => {
      if ($isTextNode(lexicalNode) && !lexicalNode.hasFormat(format)) {
        ...
        if (format === "subscript" && lexicalNode.hasFormat('superscript')) lexicalNode.toggleFormat('superscript');
        if (format === "superscript" && lexicalNode.hasFormat('subscript')) lexicalNode.toggleFormat('subscript');
      }
      return lexicalNode;
    },
    node: null
...
const IS_REAL_BROWSER$1 = Object.getOwnPropertyDescriptor(globalThis, 'window')?.get?.toString().includes('[native code]') ?? false;
...
class ElementNodeWithBlockStyle extends ElementNode {
  __blockStyle = structuredClone(BLOCK_STYLE_DEFAULT);
  __version = CURRENT_VERSION$5;
...
function isEqualStringOrStringArray(s1, s2) {
  if (typeof s1 !== typeof s2) return false;else if (s1.length !== s2.length) return false;else if (typeof s1 === 'object') {
...
function getCssTextForElementNodeWithBlockStyle(node1, type) {
  ...
  // Margin - Always Set
  if (Array.isArray(style.margin)) cssText += 'margin:'.concat(style.margin.join(' ')).concat(';');else cssText += 'margin:'.concat(style.margin).concat(';');
  // Padding - Always set
  if (LIST) {
  ...
  if (!!!isEqualStringOrStringArray(style.borderRadius, BLOCK_STYLE_DEFAULT.borderRadius)) {
    if (Array.isArray(style.borderRadius)) cssText += 'border-radius: '.concat(style.borderRadius.join(' ')).concat(';');else cssText += 'border-radius:'.concat(style.borderRadius).concat(';');
  }
  // Border Color
  if (!!!isEqualStringOrStringArray(style.borderColor, BLOCK_STYLE_DEFAULT.borderColor)) {
    if (Array.isArray(style.borderColor)) cssText += 'border-color:'.concat(style.borderColor.join(' ')).concat(';');else cssText += 'border-color:'.concat(style.borderColor).concat(';');
  }
  // Border Width
  if (!!!isEqualStringOrStringArray(style.borderWidth, BLOCK_STYLE_DEFAULT.borderWidth)) {
    if (Array.isArray(style.borderWidth)) cssText += 'border-width:'.concat(style.borderWidth.join(' ')).concat(';');else cssText += 'border-width:'.concat(style.borderWidth).concat(';');
  }
  // Border Type
  if (!!!isEqualStringOrStringArray(style.borderType, BLOCK_STYLE_DEFAULT.borderType)) {
    if (Array.isArray(style.borderType)) cssText += 'border-style:'.concat(style.borderType.join(' ')).concat(';');else cssText += 'border-style:'.concat(style.borderType).concat(';');
  }
...
function clampSize(str, min, max) {
  ...
  if (num < min) return min.toString().concat('px');else if (num > max) return max.toString().concat('px');else return str;
}
function parseSizeStr(str, defaultStr, minMax, validLength) {
  if (typeof str === 'string') {
    ...
    for (let i = 0; i < arr.length; i++) {
      ...
      } else if (Array.isArray(defaultStr) && defaultStr[i]) arr[i] = defaultStr[i];else if (Array.isArray(defaultStr)) arr[i] = defaultStr[0];else arr[i] = defaultStr[0];
    }
    if (validLength) {
    ...
    if (arr.length === 1) return arr[0];else if (arr.length === 2) return arr;else if (arr.length === 4) return arr;else return defaultStr;
  } else return defaultStr;
}
...
function parseBorderTypeStr(s) {
  ...
  for (let i = 0; i < str.length && i < 4; i++) {
    if (ALLOWED_BORDER_STYLES.has(str[i])) continue;else {
      str[i] = 'none';
    }
  }
  if (str.length === 1) return str[0];else if (str.length === 3) {
    str.push(str[1]);
    return str;
  } else {
    return str;
...
function parseBorderColorStr(s) {
  ...
  for (let i = 0; i < str.length; i++) {
    if (str[i] === 'r') startColorIndexes.push(i);else if (str[i] === '#') startColorIndexes.push(i);
  }
  for (let index of startColorIndexes) {
    if (str[index] === '#') {
      const color = str.slice(index, str.indexOf(' ', index) + 1);
      if (VALID_HEX_REGEX.test(color)) colors.push(color);else colors.push('transparent');
    } else if (str[index] === 'r') {
  ...
  if (colors.length === 1) return colors[0];else if (colors.length === 2) return colors;else if (colors.length === 3) return [colors[0], colors[1], colors[2], colors[1]];else if (colors.length === 4) return colors;else return 'transparent';
}
function parseBorderCssText(obj) {
  ...
  if (obj['border']) {
    ...
    if (borderArr.length === 1 && ALLOWED_BORDER_STYLES.has(borderArr[0])) retObj.borderType = borderArr[0];else if (borderArr.length === 2) {
...
/** @noInheritDoc */
class ParagraphNode extends ElementNodeWithBlockStyle$1 {
  ...
  createDOM(config) {
    ...
    const cssText = getCssTextForElementNodeWithBlockStyle$1(this.getLatest(), "p");
  ...
  updateDOM(prevNode, dom, config) {
    const nextStyle = getCssTextForElementNodeWithBlockStyle$1(this, "p");
    dom.style.cssText = nextStyle;
    return false;
  }
  static importDOM() {
...
function convertParagraphElement(element) {
  ...
  if (element.style.cssText) {
    const obj = extractStylesFromCssText$1(element.style.cssText, 'p');
...
function createEditor(editorConfig) {
  ...
  const editor = new LexicalEditor(editorState, parentEditor, registeredNodes, {
    ...
    namespace,
    theme
  }, onError ? onError : console.error, initializeConversionCache(registeredNodes, html ? html.import : undefined), isEditable, editorConfig?.editorType);
...
class LexicalEditor {
  ...
  constructor(editorState, parentEditor, nodes, config, onError, htmlConversions, editable, editorType) {
    ...
    this._editorType = editorType?.length ? editorType : 'full';
  }
...
/** @noInheritDoc */
class TableWrapperNode extends ElementNode$1 {
  constructor(key) {
    super(key);
    this.__first = null;
    this.__last = null;
      ...
      div: node => {
        if (node.classList.contains('table-wrapper')) {
          return {
            ...
            priority: COMMAND_PRIORITY_CRITICAL$1
          };
        } else {
          return null;
        }
  ...
  exportDOM(editor) {
    ...
    if (element && isHTMLElement$1(element)) {
      if (this.isEmpty()) element.append(document.createElement('br'));
...
function $createTableWrapperNode() {
  return $applyNodeReplacement$1(new TableWrapperNode());
}
...
class ExtendedTextNode extends TextNode$1 {
  constructor(text, key) {
    super(text, key);
  ...
  static importDOM() {
    const importers = TextNode$1.importDOM();
    return {
      ...importers,
      code: () => ({
  ...
  static importJSON(serializedNode) {
    return TextNode$1.importJSON(serializedNode);
  }
  isSimpleText() {
...
export { $addUpdateTag, $applyNodeReplacement, $copyNode, $createExtendedTextNode, $createLineBreakNode, $createNodeSelection, $createParagraphNode, $createRangeSelection, $createTabNode, $createTableWrapperNode, $createTextNode, $getAdjacentNode, $getNearestNodeFromDOMNode, $getNearestRootOrShadowRoot, $getNodeByKey, $getPreviousSelection, $getRoot, $getSelection, $getTextContent, $hasAncestor, $hasUpdateTag, $insertNodes, $isBlockElementNode, $isDecoratorNode, $isElementNode, $isExtendedTextNode, $isInlineElementOrDecoratorNode, $isLeafNode, $isLineBreakNode, $isNodeSelection, $isParagraphNode, $isRangeSelection, $isRootNode, $isRootOrShadowRoot, $isTabNode, $isTableWrapperNode, $isTextNode, $nodesOfType, $normalizeSelection as $normalizeSelection__EXPERIMENTAL, $parseSerializedNode, $selectAll, $setCompositionKey, $setSelection, $splitNode, ALLOWED_BORDER_STYLES, BLOCK_STYLE_DEFAULT, BLUR_COMMAND, CAN_REDO_COMMAND, CAN_UNDO_COMMAND, CLEAR_EDITOR_COMMAND, CLEAR_HISTORY_COMMAND, CLICK_COMMAND, COMMAND_PRIORITY_CRITICAL, COMMAND_PRIORITY_EDITOR, COMMAND_PRIORITY_HIGH, COMMAND_PRIORITY_LOW, COMMAND_PRIORITY_NORMAL, CONTROLLED_TEXT_INSERTION_COMMAND, COPY_COMMAND, CUT_COMMAND, DELETE_CHARACTER_COMMAND, DELETE_LINE_COMMAND, DELETE_WORD_COMMAND, DEPRECATED_$computeGridMap, DEPRECATED_$createGridSelection, DEPRECATED_$getGridCellNodeRect, DEPRECATED_$getNodeTriplet, DEPRECATED_$isGridCellNode, DEPRECATED_$isGridNode, DEPRECATED_$isGridRowNode, DEPRECATED_$isGridSelection, DEPRECATED_GridCellNode, DEPRECATED_GridNode, DEPRECATED_GridRowNode, DRAGEND_COMMAND, DRAGOVER_COMMAND, DRAGSTART_COMMAND, DROP_COMMAND, DecoratorNode, ElementNode, ElementNodeWithBlockStyle, ExtendedTextNode, FOCUS_COMMAND, FORMAT_ELEMENT_COMMAND, FORMAT_TEXT_COMMAND, INDENT_CONTENT_COMMAND, INSERT_LINE_BREAK_COMMAND, INSERT_PARAGRAPH_COMMAND, INSERT_TAB_COMMAND, KEY_ARROW_DOWN_COMMAND, KEY_ARROW_LEFT_COMMAND, KEY_ARROW_RIGHT_COMMAND, KEY_ARROW_UP_COMMAND, KEY_BACKSPACE_COMMAND, KEY_DELETE_COMMAND, KEY_DOWN_COMMAND, KEY_ENTER_COMMAND, KEY_ESCAPE_COMMAND, KEY_MODIFIER_COMMAND, KEY_SPACE_COMMAND, KEY_TAB_COMMAND, LineBreakNode, MAX_INDENT_PARAGRAPH_LEVEL, MOVE_TO_END, MOVE_TO_START, OUTDENT_CONTENT_COMMAND, PASTE_COMMAND, ParagraphNode, REDO_COMMAND, REMOVE_TEXT_COMMAND, RootNode, SELECTION_CHANGE_COMMAND, SELECT_ALL_COMMAND, TabNode, TableWrapperNode, TextNode, UNDO_COMMAND, VALID_HEX_REGEX, createCommand, createEditor, errorOnReadOnly, extractStylesFromCssText, getCssTextForElementNodeWithBlockStyle, getElementOuterTag, getNearestEditorFromDOMNode, isHTMLAnchorElement, isHTMLElement, isSelectionCapturedInDecoratorInput, isSelectionWithinEditor, parseColorStr };
