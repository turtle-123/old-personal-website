 ...
 */
import { SELECTION_CHANGE_COMMAND, $getSelection, $isRangeSelection, COMMAND_PRIORITY_CRITICAL, $getPreviousSelection, $isParagraphNode, $isBlockElementNode, VALID_HEX_REGEX as VALID_HEX_REGEX$1, MAX_INDENT_PARAGRAPH_LEVEL, getElementOuterTag, $getRoot, REDO_COMMAND, UNDO_COMMAND, ElementNode, BLUR_COMMAND, $isElementNode, $getNodeByKey, COMMAND_PRIORITY_EDITOR, $setSelection, $isTextNode, INSERT_TAB_COMMAND, $insertNodes, $createTabNode, KEY_TAB_COMMAND, FORMAT_TEXT_COMMAND, DecoratorNode, $nodesOfType, TextNode, $createTextNode, COMMAND_PRIORITY_NORMAL, KEY_ARROW_RIGHT_COMMAND, COMMAND_PRIORITY_LOW, $addUpdateTag, createCommand, $createParagraphNode, FORMAT_ELEMENT_COMMAND, $isRootNode, CLICK_COMMAND, $getTextContent, OUTDENT_CONTENT_COMMAND, INDENT_CONTENT_COMMAND, INSERT_LINE_BREAK_COMMAND, CLEAR_EDITOR_COMMAND } from './lexicalLexical.js';
import { mergeRegister, $findMatchingParent, segmentNodesForToolbarUpdate } from './lexical-utilsLexicalUtils.js';
import { $getSelectionStyleValueForProperty, $isAtNodeEnd, $patchStyleText, $setBlocksType } from './lexical-selectionLexicalSelection.js';
import { getCurrentEditor, setColorInput, setSelectInput, closeDialog, getDefaultBackground, getDefaultTextColor, getDefaultFontFamily, getEditorButtons, disableAndSelect, setSelectButtonText, setSelectOptionsStyle, setMarkRadioGroupChecked, setHeadingNodeButtons, getNoColor, isMobileDevice, getEditorInstances, TEXT_COLOR_CHANGE, BACKGROUND_COLOR_CHANGE, CLEAR_TEXT_FORMATTING, MAKE_TRANSPARENT, q, addSwipeRightListener, getCurrentEditorID, isTouchDevice, getDOMRangeRectAbsolute, editorCanChangeSize, handleOpenLexDialog, handleCloseLexDialog, openDialog, REGISTER_TOOLBAR_LISTENERS, getDeviceType, getCsrfToken } from './lexical-sharedLexicalShared.js';
import { offset, shift, flip, computePosition, autoUpdate } from '@floating-ui/dom';
import { exportFile } from './lexical-fileLexicalFile.js';
import { HeadingNode, $createHeadingNode, $createQuoteNode, $isHeadingNode } from './lexical-rich-textLexicalRichText.js';
import { $getLexicalContent, $generateNodesFromSerializedNodes } from './lexical-clipboardLexicalClipboard.js';
import { $createLoadingNode, $isLoadingNode, AiChatNode, INSERT_AI_CHAT_NODE, $createAiChatNode, ImageNode, AudioNode, VideoNode, YouTubeNode, NewsNode, TwitterNode, TikTokNode, InstagramNode, HtmlNode } from './lexical-decoratorsLexicalDecorators.js';
import { TableNode } from './lexical-tableLexicalTable.js';
import { MathNode } from './lexical-mathLexicalMath.js';
import { $createLinkNode } from './lexical-linkLexicalLink.js';
...
function getBlockStyleForm() {
  ...
  var cssStr = "";
  const backgroundColor = obj['backgroundColorTransparent'] ? 'transparent' : VALID_HEX_REGEX$1.test(obj['backgroundColor']) ? obj['backgroundColor'] : 'transparent';
  cssStr += 'background-color:'.concat(backgroundColor).concat(';');
  const textColor = obj['textColorInherit'] ? 'inherit' : VALID_HEX_REGEX$1.test(obj['textColor']) ? obj['textColor'] : 'inherit';
  ...
  const borderTopColor = obj['borderTopColorTransparent'] ? 'transparent' : VALID_HEX_REGEX$1.test(obj['borderTopColor']) ? obj['borderTopColor'] : 'transparent';
  const borderRightColor = obj['borderRightColorTransparent'] ? 'transparent' : VALID_HEX_REGEX$1.test(obj['borderRightColor']) ? obj['borderRightColor'] : 'transparent';
  const borderBottomColor = obj['borderBottomColorTransparent'] ? 'transparent' : VALID_HEX_REGEX$1.test(obj['borderBottomColor']) ? obj['borderBottomColor'] : 'transparent';
  const borderLeftColor = obj['borderLeftColorTransparent'] ? 'transparent' : VALID_HEX_REGEX$1.test(obj['borderLeftColor']) ? obj['borderLeftColor'] : 'transparent';
  ...
  const boxShadowColor = obj['boxShadowColorTransparent'] ? 'transparent' : VALID_HEX_REGEX$1.test(obj['boxShadowColor']) ? obj['boxShadowColor'] : 'transparent';
...
function handleBlockStyleGeneralInputChange(e) {
  if (e.target) {
    ...
    if (id === 'lex-block-border-color') {
      ...
      /* @ts-ignore */
      const text = Array.from(input?.parentElement?.querySelectorAll('button[role="option"]')).filter(a => a.getAttribute('data-val') === value).map(el => el.innerText);
      sides.forEach(side => {
  ...
  if (dialog) {
    ...
    if (editor) {
      editor.update(() => {
        ...
        var selection;
        if (!!!firstSelection) selection = SELECTION[editor._config.namespace];else selection = firstSelection;
        if ($isRangeSelection(selection)) {
          const nodes = selection.getNodes();
          const blockNodes = [];
          for (let node of nodes) {
            const tempNode = $findMatchingParent(node, n => Boolean($isParagraphNode(n) || isListNode(n) || isHeadingNode$1(n)));
            if (tempNode) blockNodes.push(tempNode);
          }
          blockNodes.forEach(node => {
            if ($isParagraphNode(node) || isListNode(node) || isHeadingNode$1(node)) {
              if ($isParagraphNode(node) || isHeadingNode$1(node)) {
                ...
                while (isNested) {
                  ...
                  if (tempListNode) topLevelList = tempListNode;else isNested = false;
                }
...
function setSelectButton(b, innerText, val) {
  ...
  if (dropdown && dropdown.classList.contains('select-menu')) {
    ...
    selectButtons.forEach(b => {
      if (b.getAttribute('data-val') === val) b.setAttribute('aria-selected', 'true');else b.setAttribute('aria-selected', 'false');
    });
  }
}
...
function resolveArray(arr1, arr2, defaultArr, colorArr) {
  ...
  var defaultArrToUse = ['', '', '', ''];
  const ret = ['', '', '', ''];
  if (typeof arr1 === 'string') arr1ToUse = [arr1, arr1, arr1, arr1];else if (arr1.length === 2) arr1ToUse = [arr1[0], arr1[1], arr1[0], arr1[1]];else if (arr1.length === 4) arr1ToUse = arr1;
  if (typeof arr2 === 'string') arr2ToUse = [arr2, arr2, arr2, arr2];else if (arr2.length === 2) arr2ToUse = [arr2[0], arr2[1], arr2[0], arr2[1]];else if (arr2.length === 4) arr2ToUse = arr2;
  if (typeof defaultArr === 'string') defaultArrToUse = [defaultArr, defaultArr, defaultArr, defaultArr];else if (defaultArr.length === 2) defaultArrToUse = [defaultArr[0], defaultArr[1], defaultArr[0], defaultArr[1]];else if (defaultArr.length === 4) defaultArrToUse = defaultArr;
...
function resolveColor(color1, color2, defaultColor) {
  if (color1 !== color2) return defaultColor;else return color1;
}
function resolveValue(v1, v2, def) {
  if (v1 !== v2) return def;else return v1;
}
    ...
    if (selection && $isRangeSelection(selection)) {
        ...
        for (let i = 0; i < blockStyleKeys.length; i++) {
          switch (blockStyleKeys[i]) {
            case 'backgroundColor':
              {
                if (j === 0) currentStyles['backgroundColor'] = resolveColor(blockStylesToUse[j]['backgroundColor'], blockStylesToUse[j]['backgroundColor'], DEFAULT_BLOCK_STYLES.backgroundColor);else currentStyles['backgroundColor'] = resolveColor(blockStylesToUse[j]['backgroundColor'], currentStyles['backgroundColor'], DEFAULT_BLOCK_STYLES.backgroundColor);
                break;
              }
            case 'borderColor':
              {
                if (j === 0) currentStyles['borderColor'] = resolveArray(blockStylesToUse[j]['borderColor'], blockStylesToUse[j]['borderColor'], DEFAULT_BLOCK_STYLES.borderColor, true);else currentStyles['borderColor'] = resolveArray(blockStylesToUse[j]['borderColor'], currentStyles['borderColor'], DEFAULT_BLOCK_STYLES.borderColor, true);
                break;
              }
            case 'borderRadius':
              {
                if (j === 0) currentStyles['borderRadius'] = resolveArray(blockStylesToUse[j]['borderRadius'], blockStylesToUse[j]['borderRadius'], DEFAULT_BLOCK_STYLES.borderRadius, false);else currentStyles['borderRadius'] = resolveArray(blockStylesToUse[j]['borderRadius'], currentStyles['borderRadius'], DEFAULT_BLOCK_STYLES.borderRadius, false);
                break;
              }
            case 'borderType':
              {
                if (j === 0) currentStyles['borderType'] = resolveArray(blockStylesToUse[j]['borderType'], blockStylesToUse[j]['borderType'], DEFAULT_BLOCK_STYLES.borderType, false);else currentStyles['borderType'] = resolveArray(blockStylesToUse[j]['borderType'], currentStyles['borderType'], DEFAULT_BLOCK_STYLES.borderType, false);
                break;
              }
            case 'borderWidth':
              {
                if (j === 0) currentStyles['borderWidth'] = resolveArray(blockStylesToUse[j]['borderWidth'], blockStylesToUse[j]['borderWidth'], DEFAULT_BLOCK_STYLES.borderWidth, false);else currentStyles['borderWidth'] = resolveArray(blockStylesToUse[j]['borderWidth'], currentStyles['borderWidth'], DEFAULT_BLOCK_STYLES.borderWidth, false);
                break;
              }
            case 'boxShadowBlurRadius':
              {
                if (j === 0) currentStyles['boxShadowBlurRadius'] = resolveValue(blockStylesToUse[j]['boxShadowBlurRadius'], blockStylesToUse[j]['boxShadowBlurRadius'], DEFAULT_BLOCK_STYLES.boxShadowBlurRadius);else currentStyles['boxShadowBlurRadius'] = resolveValue(blockStylesToUse[j]['boxShadowBlurRadius'], currentStyles['boxShadowBlurRadius'], DEFAULT_BLOCK_STYLES.boxShadowBlurRadius);
                break;
              }
            case 'boxShadowColor':
              {
                if (j === 0) currentStyles['boxShadowColor'] = resolveColor(blockStylesToUse[j]['boxShadowColor'], blockStylesToUse[j]['boxShadowColor'], DEFAULT_BLOCK_STYLES.boxShadowColor);else currentStyles['boxShadowColor'] = resolveColor(blockStylesToUse[j]['boxShadowColor'], currentStyles['boxShadowColor'], DEFAULT_BLOCK_STYLES.boxShadowColor);
                break;
              }
            case 'boxShadowInset':
              {
                if (j === 0) currentStyles['boxShadowInset'] = resolveValue(blockStylesToUse[j]['boxShadowInset'], blockStylesToUse[j]['boxShadowInset'], DEFAULT_BLOCK_STYLES.boxShadowInset);else currentStyles['boxShadowInset'] = resolveValue(blockStylesToUse[j]['boxShadowInset'], currentStyles['boxShadowInset'], DEFAULT_BLOCK_STYLES.boxShadowInset);
                break;
              }
            case 'boxShadowOffsetX':
              {
                if (j === 0) currentStyles['boxShadowOffsetX'] = resolveValue(blockStylesToUse[j]['boxShadowOffsetX'], blockStylesToUse[j]['boxShadowOffsetX'], DEFAULT_BLOCK_STYLES.boxShadowOffsetX);else currentStyles['boxShadowOffsetX'] = resolveValue(blockStylesToUse[j]['boxShadowOffsetX'], currentStyles['boxShadowOffsetX'], DEFAULT_BLOCK_STYLES.boxShadowOffsetX);
                break;
              }
            case 'boxShadowOffsetY':
              {
                if (j === 0) currentStyles['boxShadowOffsetY'] = resolveValue(blockStylesToUse[j]['boxShadowOffsetY'], blockStylesToUse[j]['boxShadowOffsetY'], DEFAULT_BLOCK_STYLES.boxShadowOffsetY);else currentStyles['boxShadowOffsetY'] = resolveValue(blockStylesToUse[j]['boxShadowOffsetY'], currentStyles['boxShadowOffsetY'], DEFAULT_BLOCK_STYLES.boxShadowOffsetY);
                break;
              }
            ...
            case 'margin':
              {
                if (j === 0) currentStyles['margin'] = resolveArray(blockStylesToUse[j]['margin'], blockStylesToUse[j]['margin'], DEFAULT_BLOCK_STYLES.margin, false);else currentStyles['margin'] = resolveArray(blockStylesToUse[j]['margin'], currentStyles['margin'], DEFAULT_BLOCK_STYLES.margin, false);
                break;
              }
            case 'padding':
              {
                if (j === 0) currentStyles['padding'] = resolveArray(blockStylesToUse[j]['padding'], blockStylesToUse[j]['padding'], DEFAULT_BLOCK_STYLES.padding, false);else currentStyles['padding'] = resolveArray(blockStylesToUse[j]['padding'], currentStyles['padding'], DEFAULT_BLOCK_STYLES.padding, false);
                break;
              }
            case 'textAlign':
              {
                if (j === 0) currentStyles['textAlign'] = resolveValue(blockStylesToUse[j]['textAlign'], blockStylesToUse[j]['textAlign'], DEFAULT_BLOCK_STYLES.textAlign);else currentStyles['textAlign'] = resolveValue(currentStyles['textAlign'], blockStylesToUse[j]['textAlign'], DEFAULT_BLOCK_STYLES.textAlign);
                break;
              }
            case 'textColor':
              {
                if (j === 0) currentStyles['textColor'] = resolveColor(blockStylesToUse[j]['textColor'], blockStylesToUse[j]['textColor'], DEFAULT_BLOCK_STYLES.textColor);else currentStyles['textColor'] = resolveColor(blockStylesToUse[j]['textColor'], currentStyles['textColor'], DEFAULT_BLOCK_STYLES.textColor);
                break;
              }
          }
        }
      ...
      blockStyleInputs.forEach(obj => {
        if (obj.id === 'lex-block-background-color') {
          ...
          if (el) {
            if (typeof currentStyles['margin'] === 'string') el.value = parseInt(currentStyles['margin']).toString();else if (allArrayElementsEqual(currentStyles['margin'])) el.value = parseInt(currentStyles['margin'][0]).toString();else el.value = parseInt('0').toString();
            el.dispatchEvent(new Event('change'));
          }
        } else if (obj.id === 'lex-block-padding') {
          const el = document.getElementById('lex-block-padding');
          if (el) {
            if (typeof currentStyles['padding'] === 'string') el.value = parseInt(currentStyles['padding']).toString();else if (allArrayElementsEqual(currentStyles['padding'])) el.value = parseInt(currentStyles['padding'][0]).toString();else el.value = parseInt('0').toString();
            el.dispatchEvent(new Event('change'));
          }
          ...
          if (el) {
            if (typeof currentStyles['borderColor'] === 'string') el.value = currentStyles['borderColor'];else if (allArrayElementsEqual(currentStyles['borderColor'])) el.value = currentStyles['borderColor'][0];else el.value = getDefaultTextColor();
            el.dispatchEvent(new Event('change'));
          }
        } else if (obj.id === 'lex-block-border-color-transparent') {
          const el = document.getElementById('lex-block-border-color-transparent');
          if (el) {
            if (typeof currentStyles['borderColor'] === 'string') el.checked = Boolean(currentStyles['borderColor'] === 'transparent');else if (currentStyles['borderColor'][0] === 'transparent' && allArrayElementsEqual(currentStyles['borderColor'])) el.checked = true;else el.checked = false;
            el.dispatchEvent(new Event('change'));
          }
        } else if (obj.id === 'border-style-value') {
          const el = document.getElementById('border-style-value');
          if (el) {
            if (typeof currentStyles['borderType'] === 'string') setSelectInput(el, currentStyles['borderType'], currentStyles['borderType']);else if (allArrayElementsEqual(currentStyles['borderType'])) setSelectInput(el, currentStyles['borderType'][0], currentStyles['borderType'][0]);else setSelectInput(el, 'none', 'None');
            el.dispatchEvent(new Event('change'));
          }
        } else if (obj.id === 'lex-block-border-width') {
          const el = document.getElementById('lex-block-border-width');
          if (el) {
            if (typeof currentStyles['borderWidth'] === 'string') el.value = parseInt(currentStyles['borderWidth']).toString();else if (allArrayElementsEqual(currentStyles['borderWidth'])) el.value = parseInt(currentStyles['borderWidth'][0]).toString();else el.value = parseInt('0').toString();
            el.dispatchEvent(new Event('change'));
          }
        } else if (obj.id === 'lex-block-border-radius') {
          const el = document.getElementById('lex-block-border-radius');
          if (el) {
            if (typeof currentStyles['borderRadius'] === 'string') el.value = parseInt(currentStyles['borderRadius']).toString();else if (allArrayElementsEqual(currentStyles['borderRadius'])) el.value = parseInt(currentStyles['borderRadius'][0]).toString();else el.value = parseInt('0').toString();
            el.dispatchEvent(new Event('change'));
          }
        }
        // Border Top
        ...
        // Border Bottom
          ...
          if (el) {
            el.value = Boolean(currentStyles['borderColor'][2] === 'transparent') ? getDefaultTextColor() : currentStyles['borderColor'][2];
            el.dispatchEvent(new Event('change'));
          }
        ...
        // Border Left
          ...
          if (el) {
            el.value = Boolean(currentStyles['borderColor'][3] === 'transparent') ? getDefaultTextColor() : currentStyles['borderColor'][3];
            el.dispatchEvent(new Event('change'));
          }
...
function registerBlockStyleCustomization(editor) {
  ...
  return mergeRegister(editor.registerCommand(SELECTION_CHANGE_COMMAND, () => {
    const selection = $getSelection();
    if ($isRangeSelection(selection)) SELECTION[editor._config.namespace] = selection;else SELECTION[editor._config.namespace] = undefined;
    return false;
  }, COMMAND_PRIORITY_CRITICAL), function () {
...
const objectsEqual = (obj1, obj2) => {
  try {
    ...
    for (let key of keys) {
      if (typeof obj1[key] === 'object' || typeof obj2[key] === 'object') throw new Error('Object values should not be objects.');else if (obj1[key] !== obj2[key]) return false;else continue;
    }
...
function disableAndSelect2(button, disable, select) {
  if (disable) button.setAttribute('disabled', '');else button.removeAttribute('disabled');
  if (select) button.setAttribute('aria-selected', 'true');else button.setAttribute('aria-selected', 'false');
}
...
function $updateToolbar(editor) {
  ...
  if (RANGE_SELECTION) {
     ...
     */
    var textColor;
    if (!!!VALID_HEX_REGEX.test(backgroundColorTemp)) backgroundColor = defaultBackgroundColor;else backgroundColor = backgroundColorTemp;
    if (!!!VALID_HEX_REGEX.test(textColorTemp)) textColor = defaultTextColor;else textColor = textColorTemp;
    ...
    for (let node of SEGMENTED_NODES.blockNodes) {
      if (align !== null) {
        ...
        if (align === undefined && alignment && alignment !== 'start' && alignment !== 'end') align = alignment;else if (align === undefined) align = null;else if (align !== undefined && alignment !== align) align = null;
      }
    ...
    if (wrapper) {
      ...
      if (HAS_LINK_NODE && removeLinkButton) removeLinkButton.removeAttribute('hidden');else if (removeLinkButton) removeLinkButton.setAttribute('hidden', '');
      ...
      if (HAS_ABBR_NODE && removeAbbreviationButton) removeAbbreviationButton.removeAttribute('hidden');else if (removeAbbreviationButton) removeAbbreviationButton.setAttribute('hidden', '');
      ...
      if (textColorTransparent) {
        if (textColorTemp === "undefined") textColorTransparent.checked = true;else textColorTransparent.checked = false;
      }
      if (backgroundColorInput) setColorInput(backgroundColorInput, backgroundColor);
      if (backgroundColorTransparent) {
        if (backgroundColorTemp === "undefined") backgroundColorTransparent.checked = true;else backgroundColorTransparent.checked = false;
      }
  ...
  if (obj) {
    ...
    if (menu) {
      ...
      if (backgroundColorButtton) {
        if (!!!getNoColor(editor)) disableAndSelect(backgroundColorButtton, obj.style.background.disable, false);else disableAndSelect(backgroundColorButtton, true, false);
      }
      if (backgroundColorInput) {
        if (!!!getNoColor(editor)) setColorInput(backgroundColorInput, obj.style.background.color);else setColorInput(backgroundColorInput, getDefaultBackground());
      }
      if (backgroundColorTransparent) {
        if (obj.style.background.transparent || getNoColor(editor)) backgroundColorTransparent.checked = true;else backgroundColorTransparent.checked = false;
      }
      if (textColorButtton) {
        if (!!!getNoColor(editor)) disableAndSelect(textColorButtton, obj.style.text.disable, false);else disableAndSelect(textColorButtton, true, false);
      }
      if (textColorInput) {
        if (!!!getNoColor(editor)) setColorInput(textColorInput, obj.style.text.color);else setColorInput(textColorInput, getDefaultTextColor());
      }
      if (textColorTransparent) {
        if (obj.style.text.transparent || getNoColor(editor)) textColorTransparent.checked = true;else textColorTransparent.checked = false;
      }
    }
  }
}
...
function registerSpeechToText(editor) {
  const SUPPORT_SPEECH_RECOGNITION = Boolean(typeof globalThis?.window !== undefined && ('SpeechRecognition' in globalThis?.window || 'webkitSpeechRecognition' in globalThis?.window) && !!!isMobileDevice(window.navigator.userAgent));
  if (SUPPORT_SPEECH_RECOGNITION) {
    const SpeechRecognition = window.SpeechRecognition || window.webkitSpeechRecognition;
    const wrapper_id = editor._config.namespace;
    const wrapper = document.getElementById(wrapper_id) || editor._rootElement?.closest('div.lexical-wrapper');
    if (wrapper) {
      const speechToTextButton = wrapper.querySelector('button[data-lexical-speech-to-text]');
      if (!!!SpeechRecognition && speechToTextButton) speechToTextButton.setAttribute('disabled', '');else if (speechToTextButton) speechToTextButton.addEventListener('click', handleStartStopSpeechRecognition);
    }
...
class AnnotationNode extends ElementNode {
  __version = ANNOTATION_NODE_VERSION;
  ...
  exportDOM(editor) {
    const {
      element
    } = super.exportDOM(editor); // calls createDOM
    return {
      element
    };
  }
  static importJSON(serializedNode) {
    ...
    var obj = undefined;
    if (text_color && background_color) obj = {
      text_color,
      background_color
    };
            ...
            case 'write':
              {
                ...
                if (lastNodeEditor) {
                  ...
                  if (!!!obj.end) {
                    const loading = $createLoadingNode({
                      size: 'small',
                      inline: true
                    });
...
function registerAIWritingHelperEditable(editor) {
  ...
  return mergeRegister(editor.registerCommand(SELECTION_CHANGE_COMMAND, () => {
    ...
    return false;
  }, COMMAND_PRIORITY_CRITICAL), editor.registerCommand(BLUR_COMMAND, () => {
    setTimeout(() => {
      if (AI_WRITER_HELPER_STATE.timeout) {
        clearTimeout(AI_WRITER_HELPER_STATE.timeout);
        AI_WRITER_HELPER_STATE.timeout = null;
...
function $updatePollQuestionToolbar(el) {
  ...
  if ($isRangeSelection(selection)) {
    ...
    if (backgroundColorInput && VALID_HEX_REGEX$1.test(backgroundColor)) backgroundColorInput.value = backgroundColor;else if (backgroundColorInput) backgroundColorInput.value = getDefaultBackground();
    if (backgroundColorTransparent) {
      if (backgroundColorTemp === 'undefined') backgroundColorTransparent.checked = true;else backgroundColorTransparent.checked = false;
    }
    if (textColorButton) disableAndSelect(textColorButton, false, false);
    if (textColorInput && VALID_HEX_REGEX$1.test(textColor)) textColorInput.value = textColor;else if (textColorInput) textColorInput.value = getDefaultTextColor();
    if (textColorTransparent) {
      if (textColorTemp === 'undefined') textColorTransparent.checked = true;else textColorTransparent.checked = false;
    }
...
function dispatchOnClick(e) {
  ...
  if (wrapper && dispatchStr) {
    ...
    if (id) {
      ...
      if (payload && dispatch) editor.dispatchCommand(dispatch, payload);else if (dispatch) editor.dispatchCommand(dispatch, undefined);
    }
  }
}
...
function onTextColorChange() {
  ...
  if (editor && VALID_HEX_REGEX$1.test(value)) {
    if (DEBOUNCE_COLOR_CHANGE$1) clearTimeout(DEBOUNCE_COLOR_CHANGE$1);
    ...
    if (rootElement) {
      ...
      if (wrapper) {
        if (this.checked) {
          ...
          editor.dispatchCommand(MAKE_TRANSPARENT, {
            type: 'text',
            color: null
          });
        } else {
          ...
          if (colorInput) editor.dispatchCommand(MAKE_TRANSPARENT, {
            type: 'text',
...
function registerPollQuestionToolbar(editor) {
  ...
  if (rootElement) {
    const wrapper = rootElement.closest('div.lexical-wrapper');
    if (wrapper) setToolbarListeners(wrapper);
  }
  return mergeRegister(editor.registerCommand(SELECTION_CHANGE_COMMAND, () => {
    const rootElement = editor.getRootElement();
    if (rootElement) {
    ...
    return false;
  }, COMMAND_PRIORITY_EDITOR), editor.registerCommand(TEXT_COLOR_CHANGE, textColor => {
    const selection = $getSelection();
    if ($isRangeSelection(selection)) {
      const obj = {
        "color": textColor,
    ...
    return false;
  }, COMMAND_PRIORITY_EDITOR), editor.registerCommand(BACKGROUND_COLOR_CHANGE, backgroundColor => {
    const selection = $getSelection();
    if ($isRangeSelection(selection)) {
    ...
    return false;
  }, COMMAND_PRIORITY_EDITOR), editor.registerCommand(CLEAR_TEXT_FORMATTING, () => {
    const selection = $getSelection();
    if ($isRangeSelection(selection)) {
      const nodes = selection.getNodes();
      for (let node of nodes) {
    ...
    return true;
  }, COMMAND_PRIORITY_EDITOR), editor.registerCommand(MAKE_TRANSPARENT, payload => {
    const selection = $getSelection();
    if ($isRangeSelection(selection)) {
...
/** @noInheritDoc */
class MaxHeightButtonNode extends DecoratorNode {
...
function registerAutocompleteEditable(editor) {
  ...
  return mergeRegister(editor.registerNodeTransform(AutocompleteNode, $handleAutocompleteNodeTransform), editor.registerUpdateListener(getHandleUpdate(editor)), editor.registerCommand(KEY_TAB_COMMAND, $handleKeypressCommand, COMMAND_PRIORITY_NORMAL), editor.registerCommand(KEY_ARROW_RIGHT_COMMAND, $handleKeypressCommand, COMMAND_PRIORITY_LOW), ...(rootElem !== null ? [addSwipeRightListener(rootElem, getHandleSwipeRightListener(editor))] : []), getUnmountSuggestion(editor));
}

const LAUNCH_ARTICLE_SELECTION_MENU = createCommand('LAUNCH_ARTICLE_SELECTION_MENU');
const ADD_COMMENT_ARTICLE_COMMAND = createCommand('ADD_COMMENT_ARTICLE_COMMAND');
const RECEIVE_COMMENT_ARTICLE_COMMAND = createCommand('RECEIVE_COMMENT_ARTICLE_COMMAND');
const ADD_ANNOTATE_ARTICLE_COMMAND = createCommand('ADD_ANNOTATE_ARTICLE_COMMAND');
const RECEIVE_ANNOTATE_ARTICLE_COMMAND = createCommand('RECEIVE_ANNOTATE_ARTICLE_COMMAND');
const SCROLL_TO_NODE = createCommand('SCROLL_TO_NODE');
const COPY_LINK_SELECTION = createCommand('COPY_LINK_SELECTION');
const UPDATE_TOOLBAR_COMMAND = createCommand('UPDATE_TOOLBAR_COMMAND');
const FONT_FAMILY_CHANGE = createCommand('FONT_FAMILY_CHANGE');
const FONT_WEIGHT_CHANGE = createCommand('FONT_WEIGHT_CHANGE');
const FONT_SIZE_CHANGE = createCommand('FONT_SIZE_CHANGE');
const LAUNCH_TABLE_MENU = createCommand('LAUNCH_TABLE_MENU');
const LAUNCH_SELECTION_MENU = createCommand('LAUNCH_SELECTION_MENU');
const ASK_AI_COMMAND = createCommand('ASK_AI_COMMAND');
...
function getFontFamilyChange(editor) {
  const handleFontFamilyChange = function () {
    if (ALLOWED_FONTS.has(this.value) || this.value === 'Inherit') editor.dispatchCommand(FONT_FAMILY_CHANGE, this.value);
  };
  return handleFontFamilyChange;
}
function getFontWeightChange(editor) {
  const handleFontWeightChange = function () {
    if (ALLOWED_FONT_WEIGHTS.has(this.value) || this.value === 'Inherit') editor.dispatchCommand(FONT_WEIGHT_CHANGE, this.value);
  };
  return handleFontWeightChange;
}
function getFontSizeChange(editor) {
  const handleFontWeightChange = function () {
    if (ALLOWED_FONT_SIZES.has(this.value) || this.value === 'Inherit') editor.dispatchCommand(FONT_SIZE_CHANGE, this.value);
  };
  return handleFontWeightChange;
}
function getTextColorChange(editor) {
  const handleTextColorChange = function () {
    ...
    if (VALID_HEX_REGEX$1.test(value)) {
...
function getBackgroundColorChange(editor) {
  const handleBackgroundColorChange = function (_) {
    if (VALID_HEX_REGEX$1.test(this.value)) {
      if (DEBOUNCE_COLOR_CHANGE) clearTimeout(DEBOUNCE_COLOR_CHANGE);
      const value = this.value;
      const parent = this.parentElement;
      if (parent) {
  ...
  if (menu && currentEditorID === editor._config.namespace) {
    ...
    const updatePopover = function () {
      ...
      if (selection && root && selection.rangeCount > 0) {
        const rect = getDOMRangeRectAbsolute(selection, root);
        if (getPortalEl()) {
          if (!!!IS_MOBILE_DEVICE && !!!IS_EDGE) this.style.top = `calc(${rect.top.toString().concat('px') + ` + var(--safe-area-inset-top-0px) + var(--virtual-keyboard-height)`})`;else this.style.top = `calc(${rect.top.toFixed(0).concat('px') + ` + ${rect.height.toFixed(0).concat('px')}` + ` + 3rem + var(--safe-area-inset-top-0px) + var(--virtual-keyboard-height)`})`;
          this.style.left = ((rect.left + rect.right) / 2).toString().concat('px');
        }
      } else return;
      const options = {
...
function dispatchFormatTextCommand(e) {
  ...
  if (payload && VALID_FORMAT_TEXT_COMMAND_SET.has(payload)) {
    const editor = getCurrentEditor();
    if (editor) editor.dispatchCommand(FORMAT_TEXT_COMMAND, payload);
  }
}
...
async function onUploadAIChat(e) {
  ...
  if (dialog && selected) {
    ...
    try {
      ...
      const editor = getCurrentEditor();
      if (editor) editor.dispatchCommand(INSERT_AI_CHAT_NODE, {
        html: str,
        chat_id: chat_id_new,
        chat_name
      });
    ...
    if (file && outputLexicalInstance && outputLexicalInstance.id) {
      try {
        ...
        if (outputEditor) {
          ...
          if (ouputEditorEl) document.dispatchEvent(new CustomEvent("NEW_CONTENT_LOADED", {
            detail: {
              el: ouputEditorEl
            }
          }));
...
function fontFamilyChangeCallback(selection, fontWithSemiColon) {
  if (fontWithSemiColon === 'Inherit') $patchStyleText(selection, {
    'font-family': null
...
function fontWeightChangeCallback(selection, weight) {
  if (weight === 'Inherit') $patchStyleText(selection, {
    'font-weight': null
  });else $patchStyleText(selection, {
    'font-weight': weight
  });
...
function registerToolbarEditable(editor) {
  ...
  document.addEventListener('OPEN_LEXICAL_DIALOG', handleOpenLexDialog);
  document.addEventListener('CLOSE_LEXICAL_DIALOG', handleCloseLexDialog);
  const arr = [editor.registerCommand(FONT_FAMILY_CHANGE, fontWithSemiColon => {
    ...
    return false;
  }, COMMAND_PRIORITY_EDITOR), editor.registerCommand(FONT_WEIGHT_CHANGE, weight => {
    const selection = $getSelection();
    if ($isRangeSelection(selection)) {
    ...
    return false;
  }, COMMAND_PRIORITY_EDITOR), editor.registerCommand(FONT_SIZE_CHANGE, size => {
    const selection = $getSelection();
    if ($isRangeSelection(selection)) {
    ...
    editor.dispatchCommand(UPDATE_TOOLBAR_COMMAND, undefined);
    return false;
  }, COMMAND_PRIORITY_EDITOR), editor.registerCommand(UPDATE_TOOLBAR_COMMAND, () => {
    ...
    return false;
  }, COMMAND_PRIORITY_LOW), editor.registerCommand(LAUNCH_SELECTION_MENU, obj => {
    ...
    return true;
  }, COMMAND_PRIORITY_EDITOR), registerSpeechToText(editor), () => {
    const wrapper = document.getElementById(editor._config.namespace) || editor._rootElement?.closest('div.lexical-wrapper');
    ...
    handleFocusOut();
    return false;
  }, COMMAND_PRIORITY_EDITOR), editor.registerCommand(ASK_AI_COMMAND, ({
    url,
    selection,
    context
  }) => {
  ...
  if (editor.hasNode(AiChatNode)) {
    arr.push(editor.registerCommand(INSERT_AI_CHAT_NODE, obj => {
  ...
  if (!!!getNoColor(editor)) {
    arr.push(editor.registerCommand(TEXT_COLOR_CHANGE, textColor => {
      const selection = $getSelection();
      ...
      return false;
    }, COMMAND_PRIORITY_EDITOR), registerBlockStyleCustomization(editor));
  }
...
function shareLink() {
  if (SHARE_ARTICLE_STATE["link"]) {
    navigator.clipboard.writeText(SHARE_ARTICLE_STATE["link"].payload).then(() => {
      if (SHARE_ARTICLE_STATE["link"]?.type === "article") document.dispatchEvent(new CustomEvent('DISPATCH_SNACKBAR', {
        detail: {
          severity: 'success',
          message: "Copied link to article selection to clipboard!"
        }
      }));else document.dispatchEvent(new CustomEvent('DISPATCH_SNACKBAR', {
        detail: {
          severity: 'success',
...
function onPinterestClick() {
  if (SHARE_ARTICLE_STATE['pinterest']) {
    const {
      ...
      image,
      title
    } = SHARE_ARTICLE_STATE['pinterest'].payload;
    const a = document.createElement('a');
...
function onWhatsAppClick() {
  if (SHARE_ARTICLE_STATE['whats-app']) {
    ...
    const {
      url,
      title
    } = SHARE_ARTICLE_STATE['whats-app'].payload;
...
function onTelegramClick() {
  if (SHARE_ARTICLE_STATE['telegram']) {
    ...
    const {
      url,
      title
    } = SHARE_ARTICLE_STATE['telegram'].payload;
...
function onTumblrClick() {
  if (SHARE_ARTICLE_STATE['tumblr']) {
    ...
    const {
      ...
      title,
      description
    } = SHARE_ARTICLE_STATE['tumblr'].payload;
...
function onPocketClick() {
  if (SHARE_ARTICLE_STATE['pocket']) {
    ...
    const {
      url,
      title
    } = SHARE_ARTICLE_STATE['pocket'].payload;
...
function onBufferClick() {
  if (SHARE_ARTICLE_STATE['buffer']) {
    ...
    const {
      url,
      title
    } = SHARE_ARTICLE_STATE['buffer'].payload;
...
function onDiggClick() {
  if (SHARE_ARTICLE_STATE['digg']) {
    ...
    const {
      url,
      title
    } = SHARE_ARTICLE_STATE['digg'].payload;
...
function onVKontakteClick() {
  if (SHARE_ARTICLE_STATE['vkonate']) {
    ...
    const {
      ...
      description,
      image
    } = SHARE_ARTICLE_STATE['vkonate'].payload;
...
function onEvernoteClick() {
  if (SHARE_ARTICLE_STATE['evernote']) {
    ...
    const {
      url,
      title
    } = SHARE_ARTICLE_STATE['evernote'].payload;
...
function onHackerClick() {
  if (SHARE_ARTICLE_STATE['hacker']) {
    ...
    const {
      url,
      title
    } = SHARE_ARTICLE_STATE['hacker'].payload;
...
function onFlipboardClick() {
  if (SHARE_ARTICLE_STATE['flipboard']) {
    ...
    const {
      url,
      title
    } = SHARE_ARTICLE_STATE['flipboard'].payload;
...
function onBloggerClick() {
  if (SHARE_ARTICLE_STATE['blogger']) {
    ...
    const {
      ...
      title,
      description
    } = SHARE_ARTICLE_STATE['blogger'].payload;
...
function onGoogleClick() {
  if (SHARE_ARTICLE_STATE['google']) {
    ...
    const {
      ...
      title,
      description
    } = SHARE_ARTICLE_STATE['google'].payload;
...
function onRenrenClick() {
  if (SHARE_ARTICLE_STATE['renren']) {
    ...
    const {
      ...
      title,
      description
    } = SHARE_ARTICLE_STATE['renren'].payload;
...
function onWeiboClick() {
  if (SHARE_ARTICLE_STATE['weibo']) {
    ...
    const {
      url,
      title
    } = SHARE_ARTICLE_STATE['weibo'].payload;
...
function onBaiduClick() {
  if (SHARE_ARTICLE_STATE['baidu']) {
    ...
    const {
      url,
      title
    } = SHARE_ARTICLE_STATE['baidu'].payload;
...
function registerCommentArticleNotEditable(editor, annotationOnly) {
  ...
  if (menu) {
    ...
    if (shareLexSelectionButton) {
      if (isTouchDevice()) shareLexSelectionButton.addEventListener('touchstart', setShareArticleSelection);else shareLexSelectionButton.addEventListener('mousedown', setShareArticleSelection);
    }
    if (askAiSelection) askAiSelection.addEventListener('click', onAskAiSelection);
  }
  const arr = [editor.registerCommand(SELECTION_CHANGE_COMMAND, () => {
    ...
    closeFloatingArticleMenu();
    return false;
  }, COMMAND_PRIORITY_EDITOR), editor.registerCommand(CLICK_COMMAND, e => {
    const selection = $getSelection();
    if (!!!$isRangeSelection(selection) || selection.getNodes().length < 1 || selection.isCollapsed()) {
      closeFloatingArticleMenu();
      return true;
    ...
    return false;
  }, COMMAND_PRIORITY_EDITOR), editor.registerCommand(COPY_LINK_SELECTION, () => {
    ...
    return false;
  }, COMMAND_PRIORITY_EDITOR), editor.registerCommand(SCROLL_TO_NODE, obj => {
    const rootElement = editor._rootElement;
    if (rootElement) {
      const {
        range,
        nodes
      } = getRange(obj, rootElement);
      if (nodes.length) {
        ...
        editor.update(() => {
          ...
          for (let nodeType of nodeArray) {
            ...
            if (nodeType === "MathNode") {
              /* @ts-ignore */
              nodesOfTypeArray.push(...$nodesOfType(NodeClass).filter(node => !!!node.isInline()));
            } else {
              /* @ts-ignore */
...
function registerCommentEditable(editor, nodeRestrictions = DEFAULT_NODE_RESTRICTION) {
  ...
  arr.push(editor.registerCommand(UPDATE_TOOLBAR_COMMAND, () => {
    const wrapper = document.getElementById(editor_id) || editor._rootElement?.closest('div.lexical-wrapper');
    if (wrapper) {
      const {
        imageButton,
        gifButton,
...
function registerCommentNotEditable(editor) {
  ...
  if (rootElement) {
    ...
    if (comment) {
      ...
      if (window.commentObserver) window.commentObserver.observe(comment);
      ...
      if (shareButton && username) {
        if (isTouchDevice()) shareButton.addEventListener('touchstart', getOnShareComment(editor, id, username.innerText.trim()));else shareButton.addEventListener('mousedown', getOnShareComment(editor, id, username.innerText.trim()));
      }
    }
  }
  return () => {};
...
function registerAnnotationNotEditable(editor) {
  ...
  if (rootElement) {
    ...
    if (annotationWrapper) {
      if (window.annotationObserver) window.annotationObserver.observe(annotationWrapper);
...
export { $createAnnotationNode, $isAnnotationNode, ADD_ANNOTATE_ARTICLE_COMMAND, ADD_COMMENT_ARTICLE_COMMAND, ALLOWED_FONT_SIZES, ALLOWED_FONT_WEIGHTS, ASK_AI_COMMAND, AnnotationNode, AutocompleteNode, COPY_LINK_SELECTION, FONT_FAMILY_CHANGE, FONT_SIZE_CHANGE, FONT_SIZE_TO_CSS, FONT_WEIGHT_CHANGE, LAUNCH_ARTICLE_SELECTION_MENU, LAUNCH_SELECTION_MENU, LAUNCH_TABLE_MENU, MaxHeightButtonNode, RECEIVE_ANNOTATE_ARTICLE_COMMAND, RECEIVE_COMMENT_ARTICLE_COMMAND, SCROLL_TO_NODE, UPDATE_TOOLBAR_COMMAND, appendRichTextEditorToOutput, backgroundColorChangeCallback, clearTextFormattingCallback, closeAIWriterMenu, closeFloatingArticleMenu, closeFloatingContextMenu, convertAnnotationNodeElement, fontFamilyChangeCallback, fontSizeChangeCallback, fontWeightChangeCallback, getOnDocumentClickArticle, getPortalEl, handleHeadingKeyUp, makeTransparentCallback, onAfterReloadHistoryDialogOpen, onAnnotateArticle, onAskAiSelection, onCommentArticle, onShareArticle, openAIWriterMenu, registerAIWritingHelperEditable, registerAnnotationEditable, registerAnnotationNotEditable, registerAutocompleteEditable, registerBlockStyleCustomization, registerCommentArticleNotEditable, registerCommentEditable, registerCommentNotEditable, registerLoadUnloadToolbarEditable, registerMaxHeightNode, registerNoToolbar, registerPollQuestionToolbar, registerSaveAndReloadHistoryEditable, registerToolbarEditable, setShareArticleSelection, textColorChangeCallback };
