 ...
 */
import { createRectsFromDOMRange, $cloneWithProperties } from './lexical-selectionLexicalSelection.js';
import { $getSelection, $isRangeSelection, TextNode, $getRoot, $isElementNode, $isTextNode, $setSelection, $getPreviousSelection, $isRootOrShadowRoot, $splitNode, $isNodeSelection, DEPRECATED_$isGridSelection, $createParagraphNode, RootNode, ElementNode, $isLineBreakNode } from './lexicalLexical.js';
export { $splitNode, isHTMLAnchorElement, isHTMLElement } from './lexicalLexical.js';
import { DecoratorBlockNode } from './lexical-decoratorsLexicalDecorators.js';
...
function getBlockNodesInSelection(selection) {
  ...
  for (let elementNode of elementNodes) {
    if (elementNode.isInline()) ret.add(elementNode);
  }
  return ret;
}
...
function segmentNodesForToolbarUpdate(selection) {
  const nodes = selection.getNodes();
  const closestRoot = $findMatchingParent(nodes[0], n => Boolean(n instanceof RootNode || n.isShadowRoot && n.isShadowRoot()));
  const ret = {
    blockNodes: new Set(),
  ...
  const findActiveCell = node => {
    ...
    if (temp && isTableCellNode(temp)) ret.activeCell = temp;else return;
  };
  const handleElementNode = node => {
    ret.types.add(node.getType());
    SEEN_NODES.add(node.getKey());
    ret.elementNodes.add(node);
    if (!!!node.isInline()) ret.blockNodes.add(node);else {
      const nearestBlockNode = $findMatchingParent(node, n => $isElementNode(n) && !!!n.isInline());
      if (nearestBlockNode && !!!SEEN_NODES.has(nearestBlockNode.getKey())) handleElementNode(nearestBlockNode);
    }
    ...
    if (!!!ret.activeCell) findActiveCell(node);
  };
  ...
  for (let i = 0; i < nodes.length; i++) {
    ...
    if (SEEN_NODES.has(NODE_KEY)) continue;
    ret.types.add(nodes[i].getType());
    SEEN_NODES.add(NODE_KEY);
    if (nodes[i] instanceof TextNode) {
      if ($isTextNode(nodes[i])) ret.textNodes.add(nodes[i]);
      nodes[i].getParentOrThrow();
      const nearestElementNode = $findMatchingParent(nodes[i], n => n instanceof ElementNode);
      if (nearestElementNode) handleElementNode(nearestElementNode);
    } else if (nodes[i] instanceof ElementNode) handleElementNode(nodes[i]);else if (nodes[i] instanceof DecoratorBlockNode) handleDecoratorNode(nodes[i]);else if ($isLineBreakNode(nodes[i])) {
      const parent = nodes[i].getParentOrThrow();
      if (parent instanceof ElementNode) handleElementNode(parent);
    }
  }
  return ret;
}

export { $dfs, $filter, $findMatchingParent, $getNearestBlockElementAncestorOrThrow, $getNearestNodeOfType, $insertFirst, $insertNodeToNearestRoot, $restoreEditorState, $wrapNodeInElement, addClassNamesToElement, getBlockNodesInSelection, getElementNodesInSelection, isMimeType, markSelection, mediaFileReader, mergeRegister, objectKlassEquals, positionNodeOnRange, registerNestedElementResolver, removeClassNamesFromElement, segmentNodesForToolbarUpdate };
