 ...
 */
import { $getSelection, $isRangeSelection, DEPRECATED_$isGridSelection, $isRootOrShadowRoot, $isElementNode, $isLeafNode, $createParagraphNode, $isParagraphNode, $applyNodeReplacement, ElementNode, ElementNodeWithBlockStyle, getCssTextForElementNodeWithBlockStyle, $createTextNode, extractStylesFromCssText, COMMAND_PRIORITY_LOW, INSERT_PARAGRAPH_COMMAND, INDENT_CONTENT_COMMAND, COMMAND_PRIORITY_CRITICAL, KEY_TAB_COMMAND, $getRoot, $createTabNode, $setSelection, $isTextNode, INSERT_TAB_COMMAND, KEY_ARROW_DOWN_COMMAND, KEY_ARROW_UP_COMMAND, KEY_ESCAPE_COMMAND, KEY_SPACE_COMMAND, $getNearestNodeFromDOMNode, KEY_ARROW_LEFT_COMMAND, COMMAND_PRIORITY_EDITOR, createCommand } from './lexicalLexical.js';
import { $getNearestNodeOfType, removeClassNamesFromElement, addClassNamesToElement, isHTMLElement, mergeRegister, $findMatchingParent, getElementNodesInSelection } from './lexical-utilsLexicalUtils.js';
import { REGISTER_TOOLBAR_LISTENERS, isTouchDevice, calculateZoomLevel } from './lexical-sharedLexicalShared.js';
...
/** @noInheritDoc */
class ListNode extends ElementNodeWithBlockStyle {
  /** @internal */
  ...
  if (wrapper) {
    ...
    if (insertOrderListButton) {
      ...
      if (dataMouseDown !== null) {
        if (!!!isTouchDevice()) insertOrderListButton.addEventListener('mousedown', getDispatchLexicalCommand(editor, INSERT_ORDERED_LIST_COMMAND));else insertOrderListButton.addEventListener('touchstart', getDispatchLexicalCommand(editor, INSERT_ORDERED_LIST_COMMAND));
      } else {
    ...
    if (insertCheckListButton) {
      ...
      if (dataMouseDown !== null) {
        if (!!!isTouchDevice()) insertCheckListButton.addEventListener('mousedown', getDispatchLexicalCommand(editor, INSERT_CHECK_LIST_COMMAND));else insertCheckListButton.addEventListener('touchstart', getDispatchLexicalCommand(editor, INSERT_CHECK_LIST_COMMAND));
      } else {
...
function registerListEditable(editor, maxIndent = 5) {
  if (editor.hasNodes([ListNode, ListItemNode])) {
    ...
    return mergeRegister(editor.registerCommand(INSERT_ORDERED_LIST_COMMAND, () => {
      ...
      return true;
    }, COMMAND_PRIORITY_LOW), editor.registerCommand(INSERT_CHECK_LIST_COMMAND, () => {
      ...
      return false;
    }, COMMAND_PRIORITY_LOW), editor.registerCommand(INDENT_CONTENT_COMMAND, () => !!!isIndentPermitted(maxIndent), COMMAND_PRIORITY_CRITICAL), editor.registerCommand(KEY_TAB_COMMAND, e => {
      const selection = $getSelection();
...
export { $createListItemNode, $createListNode, $getListDepth, $handleListInsertParagraph, $isListItemNode, $isListNode, INSERT_CHECK_LIST_COMMAND, INSERT_ORDERED_LIST_COMMAND, INSERT_UNORDERED_LIST_COMMAND, ListItemNode, ListNode, REMOVE_LIST_COMMAND, insertList, registerListEditable, removeList };
