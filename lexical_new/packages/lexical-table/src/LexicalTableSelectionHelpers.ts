...
import { $createCellParagraphNode, $isCellParagraphNode } from './CellParagraphNode';
import invariant from 'shared/invariant';

import {$isTableCellNode} from './LexicalTableCellNode';
import {$isTableNode} from './LexicalTableNode';
import {TableSelection} from './LexicalTableSelection';
import { $getTableNodeFromLexicalNodeOrThrow } from './LexicalTableUtils';
...
function getTableElementFromGrid(editor:LexicalEditor,grid: Grid) {
  const {cells} = grid;
  const row = cells[0]
  if (row) {
    const cell = row[0];
    if (cell) {
      ...
      if (lexicalNode) {
        ...
        if (table) {
          ...
          if (tableElement) return tableElement as HTMLTableElement;
        }
      }
    }
  }
...
function $addHighlightToDOM(editor: LexicalEditor, cell: Cell): void {
  ...
  if (backgroundColor === null) {
    ...
    element.style.setProperty(
      'background-image',
      `linear-gradient(to right, var(--select-button-hover), var(--select-button-hover)`,
    );
  }
...
function $handleArrowKey(
  ...
  if ($isRangeSelection(selection) && selection.isCollapsed()) {
    ...
    const edgeChild =
      Boolean(direction === 'up' || direction==="backward")
    ...
    const isExiting =
    Boolean(direction==="up"||direction==="down") ? (
      direction === 'up'
      ...
      direction === "backward"
        ? (edgeRect.left<=edgeSelectionRect.left&&selection.isCollapsed()&&selection.anchor.offset===0&&selection.focus.offset===0)
        : edgeSelectionRect.right >= edgeRect.right
    );
