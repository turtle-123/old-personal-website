...
export function $createTableNodeWithDimensions(
  ...
  const tableNode = $createTableNode({ caption, bandedRows, bandedCols, align });
...
export function $insertTableRow__EXPERIMENTAL(insertAfter = true,numOfRows:number=1,includeRowHeader:boolean): void {
  ...
  if (insertAfter) {
    ...
    const rowsToAdd: TableRowNode[] = [];
    for (let j = 0; j <numOfRows; j++){
      const newRow = $createTableRowNode();
      for (let i = 0; i < columnCount; i++) {
        const {cell, startRow} = focusEndRowMap[i];
        if (startRow + cell.__rowSpan - 1 <= focusEndRow) {
          newRow.append($createTableCellNode(
            Boolean(includeRowHeader && i===0) ? 2 :
            TableCellHeaderStates.NO_STATUS
            ).append($createCellParagraphNode()));
        } else {
...
export function $insertTableColumn__EXPERIMENTAL(insertAfter = true,columns:number=1,hasHeaderColumn:boolean=false): void {
  const selection = $getSelection();
  invariant(
    $isRangeSelection(selection) || DEPRECATED_$isGridSelection(selection),
    'Expected a RangeSelection or GridSelection',
  ...
  function $createTableCellNodeForInsertTableColumn(firstCell: boolean) {
    const cell = $createTableCellNode(
      Boolean(firstCell&&hasHeaderColumn) ? 3 :
      TableCellHeaderStates.NO_STATUS).append($createCellParagraphNode());
  ...
  for (let j = 0; j < columns; j++) {
    let loopRow: DEPRECATED_GridRowNode = start;
    ...
    for (let i = 1; i < rowSpan; i++) {
      ...
      if (insertAfterCell === null) {
        for (let j = 0; j < colSpan; j++) {
          $insertFirst(
            currentRowNode,
            $createTableCellNode(TableCellHeaderStates.NO_STATUS).append($createCellParagraphNode()),
          );
        }
      } else {
        for (let j = 0; j < colSpan; j++) {
...
export function $getColumnHeaderCellsInRange(table: TableNode, colMin: number, colMax: number) {
  const rowNodes = table.getChildren();
  var actualColIndex: number = 0;
  var ret: TableCellNode[]|undefined = undefined;
...
export function $getRowHeaderCellsInRange(table: TableNode, rowMin: number, rowMax: number) {
  const rowNodes = table.getChildren();
  var actualRowIndex: number = 0;
  var ret: TableCellNode[]|undefined = undefined;
