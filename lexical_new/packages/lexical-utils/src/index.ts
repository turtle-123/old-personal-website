...
import { NodeSelection, RangeSelection, TextNode} from 'lexical';

import invariant from 'shared/invariant';
import { DecoratorBlockNode } from '@lexical/decorators';
 ...
 *  
 */
export function getBlockNodesInSelection(selection: RangeSelection) {
  const elementNodes = getElementNodesInSelection(selection);
  const ret = new Set<ElementNode>();
 ...
 */
export function getElementNodesInSelection(selection: RangeSelection) {
...
type SegmentedNodesReturnType = {
  blockNodes: Set<ElementNode>, 
  elementNodes: Set<ElementNode>, 
  textNodes: Set<TextNode>,
  decoratorNodes: Set<DecoratorBlockNode>,
  types: Set<string>,
  activeCell: TableCellNode|undefined,
  root: LexicalNode|null
};


const isTableCellNode = (node:LexicalNode) => Boolean(node.getType()==="tablecell");
 ...
 * - text nodes
 */
export function segmentNodesForToolbarUpdate(selection: RangeSelection|NodeSelection){
  const nodes = selection.getNodes();
  const closestRoot = $findMatchingParent(nodes[0],(n)=>Boolean(n instanceof RootNode || (n.isShadowRoot && n.isShadowRoot())));
  const ret:SegmentedNodesReturnType = {
    blockNodes: new Set(), 
  ...
  const findActiveCell = (node: ElementNode) => {
    const temp = $findMatchingParent(node,(node) => isTableCellNode(node));
    if (temp&&isTableCellNode(temp)) ret.activeCell = temp as TableCellNode;
    else return;
  }
  const handleElementNode = (node: ElementNode) => {
    ...
    if (!!!node.isInline()) ret.blockNodes.add(node);
    else {
      const nearestBlockNode = $findMatchingParent(node,n=>$isElementNode(n)&&!!!n.isInline());
      if (nearestBlockNode&&!!!SEEN_NODES.has(nearestBlockNode.getKey())) handleElementNode(nearestBlockNode as ElementNode);
    }
  ...
  const handleDecoratorNode = (node: DecoratorBlockNode) => {
  ...
  for (let i = 0; i < nodes.length; i++){
    ...
    if (nodes[i] instanceof TextNode) {
      if ($isTextNode(nodes[i])) ret.textNodes.add(nodes[i] as TextNode);
      const parent = nodes[i].getParentOrThrow();
      const nearestElementNode = $findMatchingParent(nodes[i],n=>n instanceof ElementNode) as ElementNode|null;
      if (nearestElementNode) handleElementNode(nearestElementNode);
    } 
    else if (nodes[i] instanceof ElementNode) handleElementNode(nodes[i] as ElementNode);
    else if (nodes[i] instanceof DecoratorBlockNode) handleDecoratorNode(nodes[i] as DecoratorBlockNode);
    else if ($isLineBreakNode(nodes[i])) {
      const parent = nodes[i].getParentOrThrow();
      if (parent instanceof ElementNode) handleElementNode(parent as ElementNode);
    }
  }
  return ret;
}
