...
const SET_BLOCK_STYLE_LIST = new Set([
  'list',
  'heading',
  'paragraph'
]);
...
export function $setBlocksType(
  ...
  for (let i = 0; i < nodes.length; i++) {
    ...
    if (SET_BLOCK_STYLE_LIST.has(node.__key)&&SET_BLOCK_STYLE_LIST.has(targetElement.__key)) {
      (targetElement as ListNode|ParagraphNode|HeadingNode).setBlockStyle(structuredClone((node as ListNode|ParagraphNode|HeadingNode).getBlockStyle())); 
    }
