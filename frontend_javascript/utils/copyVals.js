const path = require('path');
const fs = require('fs');
const regex = /\[data-.*?\]/g
fs.promises.readFile(path.resolve(__dirname,'copyVal.txt'),{encoding:'utf-8'})
.then((res) => {
  const matches = res.match(regex);
  return fs.promises.writeFile(path.resolve(__dirname,'copyVal.json'),JSON.stringify(Array.from(matches)));
})
.then(() => {
  process.exit();
})

const a = [
  "[data-codemirror=\"html\"]",
  "[data-codemirror=\"css\"]",
  "[data-editor-wrapper]",
  "[data-editor-wrapper]",
  "[data-article-title]",
  "[data-article-description]",
  "[data-article-image]",
  "[data-article-version]",
  "[data-article-builder]",
  "[data-preview-mobile]",
  "[data-preview-tablet]",
  "[data-preview-desktop]",
  "[data-preview-mobile]",
  "[data-preview-tablet]",
  "[data-preview-desktop]",
  "[data-code-output-div]",
  "[data-article-builder]",
  "[data-editor-wrapper]",
  "[data-submit=\"save\"]",
  "[data-output-code]",
  "[data-article-builder]",
  "[data-delete-section]",
  "[data-preview-article]",
  "[data-edit-article]",
  "[data-article-builder-final]",
  "[data-codemirror]",
  "[data-codemirror]",
  "[data-editor-wrapper]",
  "[data-stop]",
  "[data-transform]",
  "[data-grad-container]",
  "[data-grad-container]",
  "[data-stop]",
  "[data-stop-container]",
  "[data-stop-list]",
  "[data-stop-container]",
  "[data-stop]",
  "[data-stop-container]",
  "[data-add-stop]",
  "[data-delete-stop]",
  "[data-transform]",
  "[data-transform]",
  "[data-transform]",
  "[data-delete-transform]",
  "[data-rich-text-editor]",
  "[data-option]",
  "[data-edit-poll-question]",
  "[data-question-type]",
  "[data-option-middle]",
  "[data-option]",
  "[data-option]",
  "[data-question-type]",
  "[data-question-content]",
  "[data-option]",
  "[data-delete-option]",
  "[data-option-type]",
  "[data-delete-poll]",
  "[data-question-type]",
  "[data-hide-question]",
  "[data-add-question]",
  "[data-option]",
  "[data-option]",
  "[data-option]",
  "[data-option]",
  "[data-option]",
  "[data-option]",
  "[data-option]",
  "[data-question-type]",
  "[data-edit-poll-question]",
  "[data-question-type]",
  "[data-question-type]",
  "[data-rich-text-editor]",
  "[data-poll-question]",
  "[data-question-content]",
  "[data-pixi-container]",
  "[data-hide-show-section]",
  "[data-add-line-dash-segment]",
  "[data-remove-line-dash-segment]",
  "[data-remove-line-dash-segment]",
  "[data-line-dash-segment]",
  "[data-scroll]",
  "[data-path-operation]",
  "[data-path-operation]",
  "[data-added-element]",
  "[data-delete-operation]",
  "[data-line-dash-details]",
  "[data-add-line-dash]",
  "[data-remove-line-dash]",
  "[data-add-line-dash-segment]",
  "[data-remove-line-dash-segment]",
  "[data-drag-container]",
  "[data-drag-el]",
  "[data-rich-text-editor]",
  "[data-type=\"full\"]",
  "[data-rich-text-editor]",
  "[data-type=\"article\"]",
  "[data-rich-text-editor]",
  "[data-type=\"poll-question\"]",
  "[data-rich-text-editor]",
  "[data-type=\"poll-short-blog\"]",
  "[data-rich-text-editor]",
  "[data-type=\"poll-option\"]",
  "[data-rich-text-editor]",
  "[data-type=\"range-description\"]",
  "[data-error]",
  "[data-previous=\"true\"]",
  "[data-next=\"true\"]",
  "[data-photo-text]",
  "[data-index=\"'.concat(String(currentIndexStr)).concat('\"]",
  "[data-photo-text]",
  "[data-index=\"'.concat(String(newIndex)).concat('\"]",
  "[data-previous=\"true\"]",
  "[data-next=\"true\"]",
  "[data-photo-text]",
  "[data-index=\"'.concat(String(currentIndexStr)).concat('\"]",
  "[data-photo-text]",
  "[data-index=\"'.concat(String(newIndex)).concat('\"]",
  "[data-wrapper=\"true\"]",
  "[data-wrapper=\"true\"]",
  "[data-media-output-wrapper]",
  "[data-error]",
  "[data-error]",
  "[data-image-input]",
  "[data-audio-input]",
  "[data-video-input]",
  "[data-multiple]",
  "[data-preview]",
  "[data-loader]",
  "[data-actions]",
  "[data-switch-camera]",
  "[data-multiple]",
  "[data-preview]",
  "[data-loader]",
  "[data-time]",
  "[data-actions]",
  "[data-multiple]",
  "[data-preview]",
  "[data-loader]",
  "[data-time]",
  "[data-actions]",
  "[data-switch-camera]",
  "[data-image-input]",
  "[data-audio-input]",
  "[data-video-input]",
  "[data-highlight-js-form]",
  "[data-katex-input]",
  "[data-form]",
  "[data-warn-form-progress]",
  "[data-editor-wrapper]",
  "[data-article-builder]",
  "[data-article-builder]",
  "[data-delete-section]",
  "[data-preview-mobile]",
  "[data-preview-tablet]",
  "[data-preview-desktop]",
  "[data-preview-mobile]",
  "[data-preview-tablet]",
  "[data-preview-desktop]",
  "[data-rich-text-editor]",
  "[data-rich-text-editor]",
  "[data-rich-text-editor]",
  "[data-delete-section]",
  "[data-esc]",
  "[data-show-geography=\"true\"]",
  "[data-close-map-window]",
  "[data-delete-geometry]",
  "[data-delete-geometry]",
  "[data-close-map-window]",
  "[data-show-geography=\"true\"]",
  "[data-show-geography=\"true\"]",
  "[data-show-geography=\"false\"]",
  "[data-hx-disable]",
  "[data-indicator]",
  "[data-indicator]",
  "[data-text]",
  "[data-text]",
  "[data-dialog]",
  "[data-close-backdrop]",
  "[data-vis-switch]",
  "[data-regenerate-value]",
  "[data-copy-input]",
  "[data-increase]",
  "[data-decrease]",
  "[data-default]",
  "[data-toggle]",
  "[data-combobox-option]",
  "[data-audio-wrapper]",
  "[data-video-wrapper]",
  "[data-sound]",
  "[data-on]",
  "[data-off]",
  "[data-on]",
  "[data-off]",
  "[data-main-controls]",
  "[data-ellapsed]",
  "[data-remaining]",
  "[data-time]",
  "[data-warn-text]",
  "[data-error-text]",
  "[data-time]",
  "[data-mute-button]",
  "[data-closed-captions]",
  "[data-play]",
  "[data-pause]",
  "[data-replay]",
  "[data-time]",
  "[data-mute-button]",
  "[data-sound]",
  "[data-playback]",
  "[data-closed-captions]",
  "[data-play]",
  "[data-pause]",
  "[data-replay]",
  "[data-time]",
  "[data-mute-button]",
  "[data-sound]",
  "[data-playback]",
  "[data-closed-captions]",
  "[data-fullscreen]",
  "[data-close-menu]",
  "[data-close-menu]",
  "[data-popover]",
  "[data-snackbar]",
  "[data-close-snackbar]",
  "[data-enter]",
  "[data-exit]",
  "[data-fullscreen-btn]",
  "[data-scroll]",
  "[data-copy-prev]",
  "[data-nav-back]",
  "[data-nav-forward]",
  "[data-show]",
  "[data-hide]",
  "[data-arrow]",
  "[data-hide-show]",
  "[data-copy-el]",
  "[data-remove-el]",
  "[data-step-back]",
  "[data-step-forward]",
  "[data-step-back]",
  "[data-step-forward]",
  "[data-click-capture]",
  "[data-drawer-button]",
  "[data-close-drawer=\"\"]",
  "[data-rich-text-editor]",
  "[data-three-js]",
  "[data-remove=\"true\"]",
  "[data-pelem]",
  "[data-editor-wrapper]",
  "[data-editor-wrapper]",
  "[data-editor-wrapper]",
  "[data-image-input]",
  "[data-audio-input]",
  "[data-video-input]",
  "[data-drag-el]",
  "[data-codemirror]",
  "[data-article-builder-final]",
  "[data-warn-form-progress]"
].map((s) => {
  if (s.indexOf(/\]\[/)!==-1) {
    const ind = s.indexOf(/\]\[/);
    return [s.slice(0,ind+1),s.slice(ind+1)];
  }
  return s;
})
.flat(1)
.map((s) => {
  var str = s.slice(1);
  const index = str.includes('=') ? str.indexOf('=') : str.indexOf(']');
  return `"${str.slice(0,index)}"`;
})
console.log(Array.from(new Set(a)).join(',\n'))