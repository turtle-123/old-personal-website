...
import { DocOrEl } from "..";
import { clearInnerHTML } from "../shared";
...
type ShowMapObject = {el: HTMLElement, map: google.maps.Map, bounds: Bounds, markers: any[], type: "show"};
type CircleMapObject = {el: HTMLElement, map: google.maps.Map, type: "circle", polygons: {[key: string]: any }, max: number|undefined, drawingManager: any, inputDiv: HTMLElement, snackbarEl: HTMLElement|undefined};
type DrawMapObject = {el: HTMLElement, map: google.maps.Map, type: "polygon", polygons: {[key: string]: any}, max: number|undefined, drawingManager: any, inputDiv: HTMLElement, snackbarEl: HTMLElement|undefined};

/**
 * Keep only one info window open at a time
 */
var INFO_WINDOW: google.maps.InfoWindow|undefined;
 ...
 */
const MAPS:{[key: string]: ShowMapObject|DrawMapObject|CircleMapObject}  = {};
/**
 * Default Center, Zoom, and Bounds for initializing a map
 * @type {{bounds: Bounds, center: Center, zoom: number}}
 */
const DEFAULT_MAPS_OBJECT:{center:Center,bounds:Bounds,zoom:number} = {
 ...
 * @returns {boolean}
 */
const validLat = (strOrNum:string|number) => Boolean(Number(strOrNum)>=-90&&Number(strOrNum)<=90);
/**
 * Function that tells you whether a number is a valid longitudinal value
 * @param {string|number} strOrNum 
 * @returns {boolean}
 */
const validLng = (strOrNum:string|number) => Boolean(Number(strOrNum)>=-180&&Number(strOrNum)<=180);
/**
 * 
 * @param {any} s 
 ...
 */
const isNumLike = (s:string|number|any) => Boolean(typeof parseFloat(s as string)==="number" && !!!isNaN(parseFloat(s as string)))
/**
 ...
 * @returns 
 */
const validBounds = (strOrObject:string|Bounds) => {
  try{
    /**
 ...
 * @returns 
 */
const validCenter = (strOrObject:string|Center) => {
  try{
    /**
 ...
 */
function getBoundsAndCenter(map: HTMLElement){
  ...
  if(geographies.length){
    ...
    for(let i=0;i<geographies.length;i++){
      ...
      if(type==="geojson"){
        ...
        if(validBounds(String(geogBounds))){
          const parsedBounds = JSON.parse(String(geogBounds));
          if (!!!bounds) {
        ...
        if(validCenter(String(geogCenter))){
          const parsedCenter = JSON.parse(String(geogCenter));
          if (!!!center) {
    ...
    center = { lng: (center as Center).lng/centerLength, lat: (center as Center).lat/centerLength };
    if (validBounds(String(bounds)) && validCenter(String(center))) return { bounds, center };
 ...
 */
const getFillColor = ()=>{
  const el = document.getElementById('FILL_COLOR_MAP') as HTMLInputElement;
...
const getStrokeColor = () => {
  const el = document.getElementById('STROKE_COLOR_MAP') as HTMLInputElement;
  if (el && VALID_HEX_REGEX.test(el.value)) return el.value;
  else return '#ab47bc';
}
/**
 ...
 */
const getMapElementFromOverlay = (overlay: google.maps.Circle|google.maps.Polygon) => {
  /**
...
/* --------------------------------------------------------- End Utilities ---------------------- */
const DELETE_GEOMETRY_POPOVER = `<div class="delete-geometry-popup">
...
function handleDrawingDblClick(this: any,e:any){
  const position = e.latLng;
  const id = this.uid;
  if(!!!id) return;
  
    const deleteFunction = function (this: any) {
      const mapElement = getMapElementFromOverlay(this);
      if (!!!mapElement) return;
      const mapId = mapElement.id;
      if (!!!mapId) return;
      const mapObject = MAPS[mapId] as CircleMapObject|DrawMapObject;
      this.setMap(null);
  ...
  INFO_WINDOW = new google.maps.InfoWindow({
    content: DELETE_GEOMETRY_POPOVER,
    position,
    maxWidth: 400
  })
  /* @ts-ignore */
  ...
  setTimeout(() => {
    ...
    if (deleteGeometry) {
      deleteGeometry.addEventListener('click',(_)=>{
         /* @ts-ignore */
        INFO_WINDOW.close();
 ...
 * @returns 
 */
function updateCircle(this:any,){
  const id = this.uid;
  if(!!!id)return;
  const circleInput=document.getElementById(id) as HTMLInputElement;
 ...
 * @returns 
 */
function updatePolygon(this:any,){
  const id = this.uid;
  if(!!!id)return;
  const polygonInput=document.getElementById(id) as HTMLInputElement;
  if(!!!polygonInput) return;
  const arr:any[] = [];
  this.forEach((latLng: google.maps.LatLng)=>arr.push([latLng.lng(),latLng.lat()]));
  arr.push(arr[0]);
...
function handleOverlayComplete(e:any){
  if (e.type==="circle"){
    ...
    const max = (MAPS[key] as CircleMapObject|DrawMapObject).max;
    const inputDiv = (MAPS[key] as CircleMapObject|DrawMapObject).inputDiv;
    if(!!!inputDiv) {circle.setMap(null); return};
    if (max) {
      const circleInputs = Array.from(inputDiv.children);
      if (circleInputs.length>=max) {
        const snackbarEl = (MAPS[key] as CircleMapObject|DrawMapObject).snackbarEl;
        if (snackbarEl) snackbarEl?.setAttribute('aria-hidden','false');
        circle.setMap(null);
        return;
      }
    }
    const center = { lat: circle?.center?.lat(), lng: circle?.center?.lng() } as Center;
    ...
    (MAPS[key] as CircleMapObject|DrawMapObject).polygons[randomID] = circle;
    ...
    circleInput.value = JSON.stringify({r:radius,c:center});
    circleInput.setAttribute('value',circleInput.value);
    (MAPS[key] as CircleMapObject|DrawMapObject).inputDiv?.appendChild(circleInput);
    circleInput.dispatchEvent(new Event('change'));
    ...
    const arr:any[] = [];
    path.forEach((latLng:google.maps.LatLng)=>arr.push([latLng.lng(),latLng.lat()]));
    arr.push(arr[0]);
    ...
    overlay.uid =randomID;
    (MAPS[key] as CircleMapObject|DrawMapObject).polygons[randomID] = polygon;
    ...
    polygonInput.value = JSON.stringify(arr);
    polygonInput.setAttribute('value',polygonInput.value);
    (MAPS[key] as CircleMapObject|DrawMapObject).inputDiv?.appendChild(polygonInput);
 ...
 */
function setMapsStyles(mode:'light'|'dark') {
  Object.values(MAPS)
  .forEach((obj) =>{
    if (obj.map) {
    ...
    if ((obj as DrawMapObject|CircleMapObject).drawingManager&&obj.type==="circle"||obj.type==="polygon") {
      obj.drawingManager.setOptions(getDrawingManagerOptions(obj.type))
    }
    if ((obj as DrawMapObject|CircleMapObject).polygons) {
      Object.values((obj as DrawMapObject|CircleMapObject).polygons).forEach((overlay) => {
        overlay.setOptions({
          fillColor: getFillColor(),
          fillOpacity: 0.3,
          strokeColor: getStrokeColor(),
 ...
 */
function handleLightModeChangeMaps(_e:Event) {
 ...
 */
async function addGeographies(mapObject:ShowMapObject) {
  ...
  if (section) {
    ...
    if(Array.isArray(elementsToAdd)) {
      elementsToAdd.forEach(async (el)=>{
        ...
        if (bounds&&validBounds(bounds)) {
          mapObject.map.fitBounds(JSON.parse(bounds));
        }
      })
    }
  }
 ...
 */
function removeGeographies(map:HTMLElement) {
  const key = map.id;
  const MAP = (MAPS[key as keyof typeof MAPS] as DrawMapObject|CircleMapObject|ShowMapObject);
  if (MAP&&MAP.map){
    MAP.map?.data?.forEach((feature)=>MAP.map.data.remove(feature));
    if ((MAP as ShowMapObject)?.markers?.length){
      (MAP as ShowMapObject).markers.forEach((marker:google.maps.Marker) => marker.setMap(null));
      (MAP as ShowMapObject).markers= [];
    }
  }
}
/**
 ...
 */
async function handleTabChangeMaps(e:any) {
  /**
  ...
  if (tabPanels&& tabPanels.children) {
    ...
    if(children[from]&&children[to]&&section){
      ...
      if(Array.isArray(geographiesToRemove)&&map){
        geographiesToRemove.forEach((geog:HTMLElement)=>geog.setAttribute('data-show-geography','false'));
        removeGeographies(map);
      }
      if(Array.isArray(geographiesToAdd)&&map){
        geographiesToAdd.forEach((span:HTMLElement)=>span.setAttribute('data-show-geography','true'));
        const { bounds, center } = getBoundsAndCenter(map);
        const key = map.id;
        if (MAPS[key]&&MAPS[key].map){
          MAPS[key]?.map?.fitBounds(bounds as Bounds); // [,padding]
          MAPS[key]?.map?.setCenter(center);
          await addGeographies(MAPS[key] as ShowMapObject);
        }
      }
    }
  }
 ...
 */
function getDrawingManagerOptions(type:"circle"|"polygon") {
  const options = {
 ...
 */
const getMapOptions = (mapId:string,center:Center,bounds:Bounds) => {
  const initial:any = {
    zoom: 4,
 ...
 */
const getDrawingMapProperties = (el:HTMLElement) => {
  /**
  ...
  if (validBounds(boundsStr as string)) bounds=JSON.parse(boundsStr as string);
  /**
   * @type {undefined|Center}
   */
  var center;
  const centerStr = el.getAttribute('data-center');
  if (validCenter(centerStr as string)) center=JSON.parse(centerStr as string);
  /**
   * @type {HTMLElement|undefined}
   */
  var inputDiv;
 ...
 */
export async function setGoogleMaps(docOrEl:DocOrEl) {
  const showMaps = Array.from(docOrEl.querySelectorAll('div[id^="google_map"]'));
  const drawMaps = Array.from(docOrEl.querySelectorAll('div[id^="draw_google_map"]'));
  const drawCircleMaps = Array.from(docOrEl.querySelectorAll('div[id^="circle_google_map"]'));
  if (showMaps.length || drawMaps.length || drawCircleMaps.length){
    ...
    if (showMaps.length){
      for(let i = 0; i < showMaps.length; i++){
        ...
        if (MAPS[key]===undefined) {
          ...
          /*@ts-ignore*/
          const newMap = new Map(showMaps[i],getMapOptions(key,center));
          newMap.fitBounds(bounds);
          newMap.setCenter(center);
          /*@ts-ignore*/
          MAPS[key].map = newMap;
          /*@ts-ignore*/
          await addGeographies(MAPS[key]);
        }
      }
    }
    if (drawMaps.length && drawing){
      for (let i = 0; i <drawMaps.length;i++){
        const key = drawMaps[i].id;
        /*@ts-ignore*/
        if (MAPS[key] === undefined) {
          /*@ts-ignore*/
          const { center, max, bounds, inputDiv, snackbarEl } = getDrawingMapProperties(drawMaps[i]);
          if(!!!inputDiv) return;
          const newMap = new Map(drawMaps[i],getMapOptions(key,center,bounds));
    ...
    if (drawCircleMaps.length && drawing){
      for (let i = 0; i < drawCircleMaps.length;i++){
        const key = drawCircleMaps[i].id;
        /*@ts-ignore*/
        if (MAPS[key] === undefined){
          /*@ts-ignore*/
          const { center, max, bounds, inputDiv, snackbarEl } = getDrawingMapProperties(drawCircleMaps[i]);
          if(!!!inputDiv) return;
          const newMap = new Map(drawCircleMaps[i],getMapOptions(key,center,bounds));
...
export type setGoogleMapsType = typeof setGoogleMaps;
window.setGoogleMaps = setGoogleMaps;
/**
 ...
 */
export function removeGoogleMaps(docOrEl:DocOrEl){
  const maps = Array.from(docOrEl.querySelectorAll<HTMLDivElement>('div[id^="google_map"], div[id^="draw_google_map"], div[id^="circle_google_map"]'));
  maps.forEach((map) => {
    clearInnerHTML(map);
    delete MAPS[map.id];
  });
}
export type removeGoogleMapsType = typeof removeGoogleMaps;
window.removeGoogleMaps = removeGoogleMaps;
...
window.editGeographyStyles = editGeographyStyles;

export type editGeographyStylesType = typeof editGeographyStyles;
