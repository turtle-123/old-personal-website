 ...
 */
import { createCommand, VALID_HEX_REGEX, $getRoot, $getSelection, $isRangeSelection, RootNode, FOCUS_COMMAND, COMMAND_PRIORITY_CRITICAL, BLUR_COMMAND, $isTextNode, $isLeafNode, $setSelection } from './lexicalLexical.js';
import { mergeRegister, $restoreEditorState, $dfs } from './lexical-utilsLexicalUtils.js';
import { $isOverflowNode, $createOverflowNode } from './lexical-overflowLexicalOverflow.js';
import { trimTextContentFromAnchor } from './lexical-selectionLexicalSelection.js';
...
function getDeviceType() {
  ...
  if (mobile) return 'mobile';else return 'tablet';
}
 ...
 */
const SET_EDITOR_COMMAND = createCommand('SET_EDITOR_COMMAND');
...
function getCurrentEditorID() {
  ...
  if (input && input.value) return input.value;else return null;
}
/**
...
function getCurrentEditor() {
  ...
  if (typeof id === "string" && EDITOR_INSTANCES[id]) return EDITOR_INSTANCES[id];else return null;
}
/**
...
function disableAndSelect(button, disable, select) {
  if (disable) disableButton(button);else removeDisabledButton(button);
  if (select) selectButton(button);else unselectButton(button);
}
/**
...
function getLightModeDarkMode() {
  ...
  if (html) {
    ...
    if (mode === "light" || mode === "dark") return mode;else return "dark";
  } else return 'dark';
}
/**
...
function getDefaultTextColor() {
  ...
  const input = document.getElementById(id);
  if (input && input.value && VALID_HEX_REGEX.test(input.value)) return input.value;else return '#FFFFFF';
}
...
function setSelectOptionsStyle(button, val) {
  ...
  if (button && button.nextElementSibling) {
    ...
    if (Array.isArray(options)) {
      options.forEach(btn => {
        if (val === btn.getAttribute('data-val')) btn.setAttribute('aria-selected', 'true');else btn.setAttribute('aria-selected', 'false');
      });
    }
  }
}
...
function setMarkRadioGroupChecked(fieldset, val) {
  if (fieldset) {
    ...
    inputs.forEach(input => {
      if (input.getAttribute('value') === val) input.checked = true;else if (!!!val && input.getAttribute('value') === 'none') input.checked = true;else input.checked = false;
    });
  }
}
/**
...
const MAKE_TRANSPARENT = createCommand('MAKE_TRANSPARENT');
const TEXT_COLOR_CHANGE = createCommand('TEXT_COLOR_CHANGE');
const BACKGROUND_COLOR_CHANGE = createCommand('BACKGROUND_COLOR_CHANGE');
const CLEAR_TEXT_FORMATTING = createCommand('CLEAR_TEXT_FORMATTING');
const LAUNCH_TEXT_INSERTION_COMMAND = createCommand('LAUNCH_TEXT_INSERTION_COMMAND');
const REGISTER_TOOLBAR_LISTENERS = createCommand('REGISTER_TOOLBAR_LISTENERS');
const UPDATE_ON_SET_HTML = createCommand('UPDATE_ON_SET_HTML');
...
const TEXT_LENGTH = {};
const EDITOR_STATE = {};
function $addCharCounter(editor, {
  min,
  max
}, initial = false) {
...
function registerMaxLengthEditable(editor, {
  min,
  max,
  showCounter = true
}) {
  ...
  if (showCounter) $addCharCounter(editor, {
    min,
    max
  }, true);
  return mergeRegister(editor.registerUpdateListener(({
    ...
    if (TEXT_LENGTH[editor._config.namespace].lastLength === null || textLengthAboveThreshold) {
      const offset = findOffset(TEXT_LENGTH[editor._config.namespace]?.text || '', Number(max), s => s.length);
      editor.update(() => {
    ...
    if (el) {
      ...
      if (maxSpan) {
        ...
        if (!!!SHOWING_MIN_SPAN) maxSpan.setAttribute('aria-hidden', 'false');else maxSpan.setAttribute('aria-hidden', 'true');
      }
    }
...
function getCsrfToken() {
  ...
  if (csrf) return csrf.value;else return 'csrf-not-found';
}
...
export { $rootTextContent, BACKGROUND_COLOR_CHANGE, CLEAR_TEXT_FORMATTING, GET_BACKGROUND_COLOR_DIALOG, IS_DESKTOP, IS_PHONE, IS_REAL_BROWSER, IS_TABLET, LAUNCH_TEXT_INSERTION_COMMAND, MAKE_TRANSPARENT, REGISTER_TOOLBAR_LISTENERS, SCROLL_INTO_VIEW_OPTIONS, SET_EDITOR_COMMAND, TEXT_COLOR_CHANGE, UPDATE_ON_SET_HTML, addSwipeDownListener, addSwipeLeftListener, addSwipeRightListener, addSwipeUpListener, calculateZoomLevel, closeDialog, closeLoadingDialog, disableAndSelect, disableButton, editorCanChangeSize, gcd, getCsrfToken, getCurrentEditor, getCurrentEditorID, getDOMRangeRect, getDOMRangeRectAbsolute, getDefaultBackground, getDefaultFontFamily, getDefaultTextColor, getDeviceType, getEditorButtons, getEditorInstances, getFileListForFileInput, getLightModeDarkMode, getNoColor, handleCloseLexDialog, handleEditorFocus, handleOpenLexDialog, isHTMLElementCustom, isIOS, isMobileDevice, isTouchDevice, mergePrevious, openDialog, openLoadingDialog, q, registerMaxLengthEditable, registerStickyNavbar, removeDisabledButton, selectButton, setColorInput, setCurrentEditor, setEditorInstance, setHeadingNodeButtons, setMarkRadioGroupChecked, setSelectButtonText, setSelectInput, setSelectOptionsStyle, unselectButton };
