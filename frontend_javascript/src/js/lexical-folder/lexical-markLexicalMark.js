 ...
 */
import { ElementNode, $applyNodeReplacement, $isElementNode, $isRangeSelection, $isTextNode, $getSelection } from './lexicalLexical.js';
import { addClassNamesToElement, removeClassNamesFromElement, $findMatchingParent, mergeRegister, registerNestedElementResolver, $dfs } from './lexical-utilsLexicalUtils.js';
...
function $removeMarkNodesInSelection(selection) {
  ...
  for (let node of nodes) {
    ...
    if (MARK_NODE) $unwrapMarkNode(MARK_NODE);
...
function $handleMarkNodes({
  ...
  if ($isRangeSelection(selection)) {
    ...
    if (IS_COLLAPSED) {
      const nodes = selection.getNodes();
      if (nodes.length) {
        const markNode = $findMatchingParent(nodes[0], node => $isMarkNode(node));
        if (markNode) {
...
function registerMarkNodes(editor) {
  if (editor.hasNode(MarkNode)) {
    ...
    return mergeRegister(registerNestedElementResolver(editor, MarkNode, from => $createMarkNode(from.getIDs()), (from, to) => {
...
export { $createMarkNode, $getMarkIDs, $handleMarkNodes, $isMarkNode, $unwrapMarkNode, $wrapSelectionInMarkNode, MarkNode, registerMarkNodes };
