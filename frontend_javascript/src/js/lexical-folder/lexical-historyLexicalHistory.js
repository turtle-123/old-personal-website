 ...
 */
import { mergeRegister } from './lexical-utilsLexicalUtils.js';
import { CAN_REDO_COMMAND, COMMAND_PRIORITY_LOW, CAN_UNDO_COMMAND, UNDO_COMMAND, COMMAND_PRIORITY_EDITOR, REDO_COMMAND, CLEAR_EDITOR_COMMAND, CLEAR_HISTORY_COMMAND, $isRangeSelection, $isTextNode, $isRootNode } from './lexicalLexical.js';
import { q, removeDisabledButton, disableButton } from './lexical-sharedLexicalShared.js';
import { UPDATE_TOOLBAR_COMMAND } from './lexical-toolbarLexicalToolbar.js';
...
function registerHistoryCustomEditable(editor) {
  return mergeRegister(editor.registerCommand(CAN_REDO_COMMAND, bool => {
    ...
    const button = q(wrapper, 'REDO_COMMAND', undefined);
    if (button) {
      if (bool) removeDisabledButton(button);else disableButton(button);
    }
    editor.dispatchCommand(UPDATE_TOOLBAR_COMMAND, undefined);
    return false;
  }, COMMAND_PRIORITY_LOW), editor.registerCommand(CAN_UNDO_COMMAND, bool => {
    const CONTENT_EDITABLE = editor.getRootElement();
    if (!!!CONTENT_EDITABLE) return false;
    const wrapper = CONTENT_EDITABLE.closest('div.lexical-wrapper');
    if (!!!wrapper) return false;
    const button = q(wrapper, 'UNDO_COMMAND', undefined);
    if (button) {
      if (bool) removeDisabledButton(button);else disableButton(button);
    }
    editor.dispatchCommand(UPDATE_TOOLBAR_COMMAND, undefined);
    return false;
  }, COMMAND_PRIORITY_LOW), registerHistory(editor, createEmptyHistoryState(), HISTORY_DELAY));
}
...
function registerHistory(editor, historyState, delay) {
  ...
  const unregisterCommandListener = mergeRegister(editor.registerCommand(UNDO_COMMAND, () => {
    ...
    return true;
  }, COMMAND_PRIORITY_EDITOR), editor.registerCommand(CLEAR_EDITOR_COMMAND, payload => {
    if (payload === true) clearHistory(historyState);
    return false;
...
export { createEmptyHistoryState, registerHistory, registerHistoryCustomEditable };
