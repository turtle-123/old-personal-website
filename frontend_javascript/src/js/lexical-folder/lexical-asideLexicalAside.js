 ...
 */
import { ElementNode, isHTMLElement, $applyNodeReplacement, $createParagraphNode, $isParagraphNode, createCommand, $getSelection, BLOCK_STYLE_DEFAULT, $isRangeSelection, $isRootNode, COMMAND_PRIORITY_EDITOR, INSERT_PARAGRAPH_COMMAND, COMMAND_PRIORITY_CRITICAL, KEY_ENTER_COMMAND, $isTextNode, COMMAND_PRIORITY_HIGH } from './lexicalLexical.js';
import { mergeRegister, $findMatchingParent } from './lexical-utilsLexicalUtils.js';
import { REGISTER_TOOLBAR_LISTENERS, isTouchDevice } from './lexical-sharedLexicalShared.js';
...
class AsideNode extends ElementNode {
  ...
  exportDOM(editor) {
    const {
      element
    } = super.exportDOM(editor); // See LexicalNode.ts -> should call this element createDOM method
...
const INSERT_ASIDE_COMMAND = createCommand('INSERT_ASIDE_ELEMENT');
  ...
  if (wrapper) {
    ...
    insertAsideButtons.forEach(button => {
      ...
      if (dataMouseDown !== null) {
        if (!!!isTouchDevice()) button.addEventListener('mousedown', getInsertAside(editor));else button.addEventListener('touchstart', getInsertAside(editor));
      } else {
    ...
    editor.registerCommand(INSERT_PARAGRAPH_COMMAND, () => {
      ...
      if ($isRangeSelection(selection) && selection.isCollapsed() && editor.isEditable()) {
        ...
        if (paragraphNode) {
          ...
          if (asideNode.getChildrenSize() >= 3 && $isParagraphNode(lastChild) && $isParagraphNode(previousChild) && lastChild.isEmpty() && previousChild.isEmpty()) {
            lastChild.remove();
...
export { $createAsideNode, $isAsideNode, AsideNode, INSERT_ASIDE_COMMAND, registerAsideEditable };
